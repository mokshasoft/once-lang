...
### Limitation 1: Jump Target Calculation

The current semantics model uses absolute jump targets (e.g., `j 15` sets `pc = 15`). The code generator computes these targets relative to each IR construct's start position. This works correctly for standalone constructs but creates issues when code is composed via concatenation.

**Example:** In `case [h,k] ∘ f`, the case code computes `right-branch = 4 + len(h)` relative to the case code's start (index 0). But when concatenated after f's code, the case code starts at index `len(f)`, making the jump target incorrect.

**Impact:** Mutual recursion proofs for `compile-compose-correct`, `compile-pair-correct`, and `compile-case-correct` cannot be completed without either:
1. Restricting composition to IRs without internal jumps
2. Modifying code generation to use PC-relative jumps
3. Implementing a link-time address resolution phase

**Status:** These remain postulated. The postulates are sound for a correctly-implemented backend with proper address calculation.

### Limitation 2: Cross-Program Calls

The `jalr` instruction in `apply` transfers control to a code pointer stored in a closure. This code pointer points to thunk code generated by `curry`, which is a separate program from `apply`.

Our model's `run prog s` function fetches instructions from `prog`, not from memory addresses. When `jalr` jumps to address 6 (the thunk's start within the curry program), it actually fetches instruction 6 from the *apply* program, not the curry program.

**Impact:** `run-apply-seq` cannot be proven without a more sophisticated memory/code model where:
- Programs are loaded at specific base addresses
- `fetch` reads from memory at `pc`, not `prog[pc]`

**Status:** `run-apply-seq` and `compile-apply-correct` remain postulated.
