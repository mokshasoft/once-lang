...
**86 of the 95 modules now compile under `--safe`** (verified 2026-07-16): the
...
| **L3.4b adequacy of `NF` for `_≈c_` (2026-07-16, all `--safe`)** | `NbEPMonAdq1`–`NbEPMonAdq9` (the world-realization homomorphism layer: `mult`/`multInv` isos, `join-split`, `⊙P-realC`, the Kelly/pentagon/hexagon realizations, `node-perm-real`), `NbEPMonAdq10`/`NbEPMonAdq11` (the split-monad splice laws `withSpˡ-splice`/`appSp-splice`/`withSpʳ-splice` — the deepest chains), `NbEPMonAdq12` (the Kripke gluing relation `R` by type recursion), `NbEPMonAdq13` (Kripke monotonicity `R-vmap`), `NbEPMonAdq14` (the reify/reflect boundary `R-reify`/`R-reflectNe`/`R-reflectTy`), `NbEPMonAdq16` (`R-join`: the split-monad join preserves `R`), `NbEPMonAdq17` (the generic `Tree` splice layer: `withSpˡ-Tree`/`withSpʳ-Tree`/`bindSp-Tree`/`Tree-resp`), `NbEPMonAdq15` (**THE FUNDAMENTAL LEMMA `fund : R A v t → R B (evalV f v) (f ∘c t)` — all 12 SMCC combinators, transport-free**), `NbEPMonAdq18` (**the capstone: `adequacy : f ≈c NF f` and `decide : NF f ≡ NF g → f ≈c g` — completeness of the normalizer and the decision procedure for `_≈c_`, a theorem of `--safe` Agda**) |
