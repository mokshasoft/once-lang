...
------------------------------------------------------------------------
-- Once.Arith.Backend.RiscV64.ExecArith  (Plan 0.54 Phase B / Option 2)
--
-- riscv64 instance of the generic block fold + memory effect. Unlike x86-64,
-- riscv64 lowers `sp` in the prologue and addresses scratch as `sp + 8·slot`, so
-- it USES the shared in-frame witness: `InFrame (Xmov-r-m sc _) = 8·slot < N`
-- (`N = 8·required-scratch` = the reserved frame). The frontier is `sp + N`
-- (= the entry sp), and `scratch-below` discharges `sp + 8·slot < sp + N` from
-- the slot bound (`+-monoʳ-<`). SAME obligation as x86-64, different witness.
------------------------------------------------------------------------

module Once.Arith.Backend.RiscV64.ExecArith where
...
module _ (val : XInstr → State → Reg → Word) (N : ℕ) where
  ...
  frontier : State → ℕ
  frontier s = readReg (regs s) sp + N

  scratch-below : ∀ s sc (src : XReg) fr → frontier s ≡ fr → 0 < fr → InFrame N (Xmov-r-m sc src) →
                  scratch-addr s sc < fr
  scratch-below s sc src fr f≡ _ inf rewrite sym f≡ = +-monoʳ-< (readReg (regs s) sp) inf
  ...
  exec1 i s = mkstate (step-of i (val i s) (regs s)) (mem-effect i s) (suc (pc s)) (halted s)
  ...
  frontier-inv i s fr f≡ _ _ =
    trans (cong (_+ N) (sym (a-sp (step-of-preserves i (val i s) (regs s))))) f≡
