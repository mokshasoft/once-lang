...
--
-- The memory analog of register-confinement, over the REAL riscv64 memory
-- model (`Once.CCC.Target.RiscV64.Semantics.Memory = Word → Maybe Word`).
--
-- The arith block's scratch slots live BELOW `sp` (`-N(sp)`); CCC's live
-- data is at/above `sp`. So a scratch write (address `< rsp`) preserves every
...
open import Once.CCC.Target.RiscV64.Semantics using (Memory; readMem; writeMem; Word)
