...
-- the 7 CCC registers AND on all memory at/above the entry `%rsp` frontier.
--
-- Both halves compose (transitively), so a whole arith block — a sequence of
-- steps each writing only non-`ccc` registers and only scratch (`< rsp`) —
...
open import Once.CCC.Target.X86-64.Semantics using (State; RegFile; Memory; Word)
open State
open import Once.Arith.Backend.X86-64.Preserve using (AgreeCCC; agree-refl-ccc; AgreeCCC-trans)
open import Once.Arith.Backend.X86-64.MemPreserve using (AgreeMemFrom)
