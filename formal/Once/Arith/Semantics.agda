...
eval-cmp (Lt {Γ} {Δ} {τ} e₁ e₂) env =
  let (env₁ , env₂) = splitEnv {Γ} {Δ} env
  in lt τ (eval-arith e₁ env₁) (eval-arith e₂ env₂)

eval-cmp (Eq {Γ} {Δ} {τ} e₁ e₂) env =
  let (env₁ , env₂) = splitEnv {Γ} {Δ} env
  in eq τ (eval-arith e₁ env₁) (eval-arith e₂ env₂)

eval-cmp (Le {Γ} {Δ} {τ} e₁ e₂) env =
  let (env₁ , env₂) = splitEnv {Γ} {Δ} env
      v₁ = eval-arith e₁ env₁
      v₂ = eval-arith e₂ env₂
  in lt τ v₁ v₂ Data.Bool.∨ eq τ v₁ v₂

eval-cmp (Gt {Γ} {Δ} {τ} e₁ e₂) env =
  let (env₁ , env₂) = splitEnv {Γ} {Δ} env
  in lt τ (eval-arith e₂ env₂) (eval-arith e₁ env₁)

eval-cmp (Ge {Γ} {Δ} {τ} e₁ e₂) env =
  let (env₁ , env₂) = splitEnv {Γ} {Δ} env
      v₁ = eval-arith e₁ env₁
      v₂ = eval-arith e₂ env₂
  in lt τ v₂ v₁ Data.Bool.∨ eq τ v₁ v₂

eval-cmp (Ne {Γ} {Δ} {τ} e₁ e₂) env =
  let (env₁ , env₂) = splitEnv {Γ} {Δ} env
  in Data.Bool.not (eq τ (eval-arith e₁ env₁) (eval-arith e₂ env₂))
