------------------------------------------------------------------------
-- Once.CCC.Target.X86.Correct.WholeProgram
--
...
--
-- TERMINATION:
--   Termination is proven separately in Once.CCC.Termination.
--   The dispatcher postulate is validated by this orthogonal proof.
------------------------------------------------------------------------

module Once.CCC.Target.X86.Correct.WholeProgram where
...
open import Once.CCC.Target.X86.Correct.StackInstantiation using (slots; StackCapacity; rsp-bound-to-capacity; ir-stack-requirement; ir-output-capacity; thunk-setup-consumed-slots; thunk-setup-cap≤thunk-consumed+ir-req)
...
open import Once.CCC.Target.X86.Correct.StarBase
  using (IRStarResultV; ClosureWFOutput; no-closure; has-closure;
         ...
         ir-result-valid)
open import Once.CCC.Target.X86.Correct.MemoryValid using (ValidAt; valid-closure-env; ClosureAtS; closure-at-s)
...
open import Once.CCC.Target.X86.Correct.ClosureContext
  using (ApplyMemoryLayout; run-apply-with-full-wf; CurryOutputWF)

-- Import modular runner for delegation
open import Once.CCC.Target.X86.Correct.MutualIR as Modular
...
open import Once.CCC.Target.X86.Correct.IRSize using (ir-size)
open import Data.Nat.Induction using (<-wellFounded)

-- Import curry proof with memory result
open import Once.CCC.Target.X86.Correct.IR.Curry using (run-curry-star; CurryMemoryResult; CurryExecResult)
open import Once.CCC.Target.X86.Correct.IR.Curry using (closure-addr; code-ptr; env-addr; rax-eq; mem-env; mem-cp; v-env; code-ptr-is-thunk)
-- CurryExecResult field accessors
open import Once.CCC.Target.X86.Correct.IR.Curry using (exec-star; exec-halted; exec-pc; exec-r14; exec-r15; exec-rbp; exec-stack-inv; exec-capacity; exec-rbp-inv)
