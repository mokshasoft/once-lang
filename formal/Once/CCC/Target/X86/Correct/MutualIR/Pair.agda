------------------------------------------------------------------------
-- Once.CCC.Target.X86.Correct.MutualIR.Pair
--
...
open import Once.CCC.Target.X86.Correct.StackInstantiation using (slots; slot-size; pair-alloc; StackCapacity; slots-mono-≤; ir-stack-requirement; ir-rsp-delta; pair-setup-consumed-slots; pair-setup≤pair-req; pair-inner-requirement; output-slots; output-slots≤pair-req; pair-rbp-slot; pair-rbp-slot≤pair-setup; pair-rbp-frame-≥-r15-frame; make-frame-at-slot; saved-regs-size; frame-size)
open import Data.Nat.Properties using (≤-<-trans; ≤-trans; <-trans; <-≤-trans; <⇒≤; m∸n≤m; m≤n⇒m∸n≡0; ≰⇒>; m≤m+n; m≤m⊔n; m≤n⊔m)
open import Once.CCC.Target.X86.Correct.ArithmeticLemmas using (rsp-min-pair-fits-frame; word-fits-thunk-bound-strict)
...
module Once.CCC.Target.X86.Correct.MutualIR.Pair
  (bound : ℕ)
...
open import Once.CCC.Target.X86.Correct.IR.Pair
...
open import Once.CCC.Target.X86.Correct.IR.Pair using (module PairContext)
open import Once.CCC.Target.X86.Correct.IR.Pair using (module PairFinalResult)
open import Once.CCC.Target.X86.Correct.IR.Pair using (module PairSetupResultV)
open import Once.CCC.Target.X86.Correct.IR.Pair using (module PairMiddleResultV)
...
run-pair-star-v {A} {B} {C} f g f<bound g<bound prefix suffix caller-sp x s h-false pc-eq input-valid stack-inv cap-in rbp-inv =
    ...
    where
      ...
      rbp-inv-s2 = Once.CCC.Target.X86.Correct.StarBase.rbp-inv-preserved-unchanged s1 s2 rbp-inv-s1 rsp-s2-eq-s1 rbp-s2-eq-s1
