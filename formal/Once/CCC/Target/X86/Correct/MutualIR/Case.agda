------------------------------------------------------------------------
-- Once.CCC.Target.X86.Correct.MutualIR.Case
--
...
open import Once.CCC.Target.X86.Correct.StackInstantiation using (slots; StackCapacity; ir-stack-requirement)
...
module Once.CCC.Target.X86.Correct.MutualIR.Case
  (bound : ℕ)
...
open import Once.CCC.Target.X86.Correct.Star using (Star; refl*; step*; star-trans)
open import Once.CCC.Target.X86.Correct.FetchStep using (step-exec)
open import Once.CCC.Target.X86.Correct.StarBase using (IRStarResultV; rbp-inv-preserved-unchanged; ClosureWFOutput)
open import Once.CCC.Target.X86.Correct.MemoryValid
...
open import Once.CCC.Target.X86.Correct.StackInstantiation
  using (slots; slot-size; StackCapacity; ir-stack-requirement; ir-output-capacity;
         ...
         rsp-sufficient)
open import Once.CCC.Target.X86.Layout using (InStack; InHeap; InCode; heap-offset)
...
-- Import Case helpers
open import Once.CCC.Target.X86.Correct.IR.Case
...
open import Once.CCC.Target.X86.Correct.IR.Case using (module CaseInlSetupResult; module CaseCleanupResult; module CaseInrSetupResult)
...
run-case-star-direct-inl {A} {B} {C} f g f<bound prefix suffix caller-sp a s h-false pc-eq input-valid stack-inv cap-in rbp-inv =
  ...
  where
    ...
    stack-inv-final = stack-inv-preserved-unchanged s s-final stack-inv r15-final rsp-final
      where
        open import Once.CCC.Target.X86.Correct.StackInvariant using (stack-inv-preserved-unchanged)
    ...
    rbp-inv-final = record
      ...
      where
        ...
        open import Once.CCC.Target.X86.Layout using (StackPointer) renaming (addr to sp-addr)
