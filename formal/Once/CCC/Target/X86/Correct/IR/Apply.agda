------------------------------------------------------------------------
-- Once.CCC.Target.X86.Correct.IR.Apply
--
...
module Once.CCC.Target.X86.Correct.IR.Apply where
...
open import Once.CCC.Target.X86.Correct.ExecLemmas using (fetch-at-prefix-end; just-injective)
open import Once.CCC.Target.X86.Correct.InstrExec using (execPop)
...
open import Once.CCC.Target.X86.Correct.ArithmeticLemmas using (word-fits-thunk-bound)
...
open import Once.CCC.Target.X86.Layout using (init-frame-slot-at-base) renaming (addr to sp-addr)
...
run-apply-with-wf {E} {A} {B} prefix suffix code-ptr env semantics arg arg-addr s
  ...
  where
    ...
    open import Once.CCC.Target.X86.Correct.StackInstantiation using (capacity-from-larger; thunk-setup-capacity)
...
open import Once.CCC.Target.X86.Correct.StackInvariant using (RbpInvariant)
...
run-apply-to-ir-result {E} {A} {B} prefix suffix code-ptr env semantics arg arg-addr s
  ...
  where
    ...
    open import Once.CCC.Target.X86.Correct.StackInstantiation using (thunk-setup-capacity)
