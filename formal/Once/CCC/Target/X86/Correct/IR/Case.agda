------------------------------------------------------------------------
-- Once.CCC.Target.X86.Correct.IR.Case
--
...
module Once.CCC.Target.X86.Correct.IR.Case where
...
open import Once.CCC.Target.X86.Correct.Star using (Star; refl*; step*; star-trans; star-step2; star-step3; star-step6; star-step7)
open import Once.CCC.Target.X86.Correct.FetchStep using (step-exec; fetch-append-skip)
open import Once.CCC.Fetch using (fetch-0; fetch-1; fetch-2; fetch-3; fetch-4; fetch-5; fetch-append-right)
...
open import Once.CCC.Target.X86.Correct.StarBase using (IRStarResultV)
open import Once.CCC.Target.X86.Correct.MemoryValid using (ValidAt)
...
open import Once.CCC.Target.X86.Correct.RegisterLemmas using (readMem-writeMem-diff)
...
open import Once.CCC.Target.X86.Correct.CompileLength using (length-++; compile-length-correct)
...
case-inl-setup-star {A} {B} {C} f g prefix suffix a s val-addr
  ...
  where
    ...
    mem-at-rbp6 = subst (λ addr → readMem (memory s6) addr ≡ just orig-rbp) (sym rbp6) push-wrote-rbp
      where
        open import Once.CCC.Target.X86.Correct.RegisterLemmas using (readMem-writeMem-same)
    ...
    slot-size<rsp = rsp-sufficient cap-1
      where
        open import Once.CCC.Target.X86.Correct.StackInstantiation using (rsp-sufficient)
    ...
    mem-r15-6 = readMem-writeMem-diff orig-mem push-addr orig-r15 orig-rbp push-addr≢r15
      where
        ...
        open import Once.CCC.Target.X86.Layout using (slot-addr; init-slot-at-base)
