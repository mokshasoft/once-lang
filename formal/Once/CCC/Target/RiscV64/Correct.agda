...
------------------------------------------------------------------------
-- Once.CCC.Target.RiscV64.Correct
--
-- COMPILER CORRECTNESS THEOREM for RISC-V 64
--
-- The FULL correctness property we want to prove:
--
--   ∀ ir x rv64-state →
--     let program = compile-ir ir
--         rv64-final = exec program rv64-state
--     in a1 rv64-final represents (eval ir x)
--
-- This decomposes into two layers:
--
--   Layer 1: IR → AbstractTrace (via Dispatcher/PairWF)
--   Layer 2: AbstractTrace → RISC-V (via DirectSimulation)
--
-- Current status:
...
open import Once.CCC.Target.RiscV64.Types using (Type; ⟦_⟧)
...
--   │           ▼                             ▼                       │
--   │     ┌────────────┐    ═══════     ┌──────────────┐               │
--   │     │final-state │ RV64Corresponds│ RISC-V State │               │
--   │     │(LocState)  │                │ (result)     │               │
--   │     └────────────┘               └──────────────┘               │
--   │                                                                 │
--   │  Result: a1 holds address of value satisfying                   │
--   │          eval primSem ir x                                      │
--   └─────────────────────────────────────────────────────────────────┘
--
------------------------------------------------------------------------