...
module Simulation {FS : FrameSemantics} where
  ...
  record X86State : Set where
    constructor mkX86
    field
      eax-val : StoredValue FS     -- Output register (maps to x86 eax)
      ecx-val : StoredValue FS     -- Input1 register (maps to x86 ecx)
      cur-frame : Frame            -- Current frame (maps to ebp)
      stack-slot : ℕ               -- Stack slot index (maps to esp offset)
      x86-mem : ValueLocation FS → Maybe (StoredValue FS)    -- Memory: address → value
  ...
  writeX86Mem : (ValueLocation FS → Maybe (StoredValue FS)) →
  ...
  exec-x86-load-eax-with-value : Maybe (StoredValue FS) → X86State → X86State
  ...
  exec-x86-load-ecx-with-value : Maybe (StoredValue FS) → X86State → X86State
  ...
  -- Plan 0.13.2: register holds StoredValue; indirect memory access must
  -- unwrap via sv-as-loc. Mirrors X86-64's helpers (Bridge B).
  readSV-at : X86State → Maybe (ValueLocation FS) → Maybe (StoredValue FS)
  readSV-at xs (just loc) = x86-mem xs loc
  readSV-at xs nothing    = nothing

  readSV-at-suc : X86State → Maybe (ValueLocation FS) → Maybe (StoredValue FS)
  readSV-at-suc xs (just loc) = x86-mem xs (sucLoc loc)
  readSV-at-suc xs nothing    = nothing

  x86-read-sv : X86State → StoredValue FS → Maybe (StoredValue FS)
  x86-read-sv xs sv = readSV-at xs (sv-as-loc sv)

  x86-read-sv-suc : X86State → StoredValue FS → Maybe (StoredValue FS)
  x86-read-sv-suc xs sv = readSV-at-suc xs (sv-as-loc sv)

  writeSV-at : X86State → Maybe (ValueLocation FS) → StoredValue FS → X86State
  writeSV-at xs (just loc) v = record xs { x86-mem = writeX86Mem (x86-mem xs) loc v }
  writeSV-at xs nothing    _ = record xs { x86-halted = true }

  writeSV-at-suc : X86State → Maybe (ValueLocation FS) → StoredValue FS → X86State
  writeSV-at-suc xs (just loc) v = record xs { x86-mem = writeX86Mem (x86-mem xs) (sucLoc loc) v }
  writeSV-at-suc xs nothing    _ = record xs { x86-halted = true }

  x86-write-sv : X86State → StoredValue FS → StoredValue FS → X86State
  x86-write-sv xs sv v = writeSV-at xs (sv-as-loc sv) v

  x86-write-sv-suc : X86State → StoredValue FS → StoredValue FS → X86State
  x86-write-sv-suc xs sv v = writeSV-at-suc xs (sv-as-loc sv) v
  ...
  postulate
    ...
    exec-x86-call-sym-rax    : String → X86State → StoredValue FS
  ...
  exec-x86 (mov (reg eax) (mem (base ecx))) xs _ =
    exec-x86-load-eax-with-value (x86-read-sv xs (ecx-val xs)) xs

  -- load-indirect-suc: mov eax, [ecx + slot-size] → eax' = *(sucLoc ecx)
  exec-x86 (mov (reg eax) (mem (base+disp ecx d))) xs _ =
    exec-x86-load-eax-with-value (x86-read-sv-suc xs (ecx-val xs)) xs
  ...
  -- Plan 0.13.2: eax-val is StoredValue; wrap slot address in SV-Ptr.
  exec-x86 (lea eax (base+disp ebp d)) xs frame =
    record xs { eax-val = SV-Ptr (slotLoc frame (disp-to-slot d)) }
  ...
  writeX86Mem-stack-corresponds : ∀ (ls : LocState FS) (xs : X86State) (f : Frame) (k : ℕ) (val : StoredValue FS) →
  ...
  writeX86Mem-corresponds : ∀ (ls : LocState FS) (xs : X86State) (loc : ValueLocation FS) (val : StoredValue FS) →
    (∀ l → x86-mem xs l ≡ readLoc ls l) →
    eax-val xs ≡ val →
  ...
  load-eax-corresponds : ∀ (mv : Maybe (StoredValue FS)) (ls : LocState FS) (xs : X86State) (alloc : AllocState {FS}) →
  ...
  load-ecx-corresponds : ∀ (mv : Maybe (StoredValue FS)) (ls : LocState FS) (xs : X86State) (alloc : AllocState {FS}) →
    Corresponds ls xs alloc →
  ...
  load-from-slot-corresponds : ∀ (mv : Maybe (StoredValue FS)) (ls : LocState FS) (xs : X86State) (alloc : AllocState {FS}) →
    Corresponds ls xs alloc →
  ...
  postulate
    ...
    load-tag-lit-codegen-faithful-32 :
      ∀ (n : ℕ) ls xs alloc → halted ls ≡ false → Corresponds ls xs alloc →
      Corresponds (proj₁ (exec-abstract (instr-load-tag-lit n) ls alloc))
                  (exec-prog (compile-abstract (instr-load-tag-lit n)) xs (current-frame alloc))
                  (proj₂ (exec-abstract (instr-load-tag-lit n) ls alloc))

    alloc-heap-codegen-faithful-32 :
      ∀ (n : ℕ) ls xs alloc → halted ls ≡ false → Corresponds ls xs alloc →
      Corresponds (proj₁ (exec-abstract (instr-alloc-heap n) ls alloc))
                  (exec-prog (compile-abstract (instr-alloc-heap n)) xs (current-frame alloc))
                  (proj₂ (exec-abstract (instr-alloc-heap n) ls alloc))

    -- Input2 ↔ edx isn't tracked by X86-32 Corresponds (X86State has no
    -- edx field). The proofs that abstract Input2-touching instrs
    -- correspond to edx-emitting codegen go through these postulates.
    instr-sim-mov-input2-to-output-32 : ∀ ls xs alloc → halted ls ≡ false → Corresponds ls xs alloc →
    ...
    instr-sim-mov-output-to-input2-32 : ∀ ls xs alloc → halted ls ≡ false → Corresponds ls xs alloc →
    ...
    -- Plan 0.13.1 Phase 1: case-on-tag — both sides halt (X86-32 mirror).
    case-codegen-faithful-phase1-32 :
  ...
  -- Shared body for mov-to-output / mov-input2-to-output:
  -- both write Input1 into Output. Extracted to a helper so each
  -- instr-sim clause has its own LHS (avoids ambiguous mutual-clause
  -- continuations; see memory [[mutual-block-syntax]]).
  private
    mov-output-from-input1-sim : ∀ ls xs alloc → halted ls ≡ false → Corresponds ls xs alloc →
      let s' = record ls { regs = writeReg (regs ls) Output (readReg (regs ls) Input1) }
      in Corresponds s'
                     (record xs { eax-val = ecx-val xs })
                     alloc
    mov-output-from-input1-sim ls xs alloc not-halted corr =
    ...
    mov-input1-from-output-sim : ∀ ls xs alloc → halted ls ≡ false → Corresponds ls xs alloc →
      let s' = record ls { regs = writeReg (regs ls) Input1 (readReg (regs ls) Output) }
      in Corresponds s'
                     (record xs { ecx-val = eax-val xs })
                     alloc
    mov-input1-from-output-sim ls xs alloc not-halted corr =
  ...
  -- mov-to-output: Output := Input1. emits `mov eax, ecx`.
  instr-sim mov-to-output ls xs alloc not-halted corr
    with x86-halted xs | xs-not-halted ls xs alloc not-halted corr
  ... | true | ()
  ... | false | _ = mov-output-from-input1-sim ls xs alloc not-halted corr

  -- mov-input2-to-output emits `mov eax, edx` — edx is not tracked by
  -- X86State, so route through a named postulate.
  instr-sim mov-input2-to-output ls xs alloc not-halted corr =
    instr-sim-mov-input2-to-output-32 ls xs alloc not-halted corr

  -- mov-to-input: Input1 := Output. emits `mov ecx, eax`.
  instr-sim mov-to-input ls xs alloc not-halted corr
    with x86-halted xs | xs-not-halted ls xs alloc not-halted corr
  ... | true | ()
  ... | false | _ = mov-input1-from-output-sim ls xs alloc not-halted corr

  -- mov-output-to-input2 emits `mov edx, eax` — edx untracked, postulate.
  instr-sim mov-output-to-input2 ls xs alloc not-halted corr =
    instr-sim-mov-output-to-input2-32 ls xs alloc not-halted corr

  -- load-indirect: Output := *Input1
  -- Bridge B: both abstract and x86 dispatch on `sv-as-loc` of Input1
  -- (parallel `with` keeps the just/nothing branches in lock-step).
  instr-sim load-indirect ls xs alloc not-halted corr
    with x86-halted xs | xs-not-halted ls xs alloc not-halted corr
  ... | true | ()
  ... | false | _
    with sv-as-loc (ecx-val xs) | sv-as-loc (readReg (regs ls) Input1)
       | cong sv-as-loc (ecx-eq corr)
  ... | just loc | just .loc | refl =
    let abs-read   = readLoc ls loc
        mem-read-eq : x86-mem xs loc ≡ abs-read
  ...
  ... | just loc | just .loc | refl =
    let abs-read   = readLoc ls (sucLoc loc)
        mem-read-eq : x86-mem xs (sucLoc loc) ≡ abs-read
  ...
  -- store-indirect: *Input1 := Output (Bridge B: parallel sv-as-loc dispatch)
  ...
  ... | just loc | just .loc | refl =
    ...
    in record
      ...
      ; frame-eq = frame-eq corr
      ; slot-eq = trans (slot-eq corr) (slot-eq-lift alloc (cong stackSlot (sym regs-eq)))
      ; mem-eq = writeX86Mem-corresponds ls xs loc val (mem-eq corr) (eax-eq corr)
      ; halt-eq = trans (halt-eq corr) (sym (writeLoc-halted ls loc val))
      }
  ... | nothing | nothing | refl =
    record
  ...
  ... | just loc | just .loc | refl =
    ...
    in record
      ...
      ; frame-eq = frame-eq corr
      ; slot-eq = trans (slot-eq corr) (slot-eq-lift alloc (cong stackSlot (sym regs-eq)))
      ; mem-eq = writeX86Mem-corresponds ls xs sloc val (mem-eq corr) (eax-eq corr)
  ...
  ... | false | _ =
    let frame = current-frame alloc
        ...
        slot-recover = m*n/n≡m slot slot-size
    in record
    { ecx-eq = trans (ecx-eq corr) (sym (writeReg-preserves (regs ls) Output Input1 sv Input1≢Output))
    ; eax-eq = trans (cong (λ s → SV-Ptr (AtStack frame s)) slot-recover)
  ...
  instr-sim (instr-load-tag-lit n) ls xs alloc not-halted corr =
    load-tag-lit-codegen-faithful-32 n ls xs alloc not-halted corr

  instr-sim (instr-alloc-heap n) ls xs alloc not-halted corr =
    alloc-heap-codegen-faithful-32 n ls xs alloc not-halted corr

  instr-sim (instr-case-on-tag f g) ls xs alloc not-halted corr =
    case-codegen-faithful-phase1-32 f g ls xs alloc not-halted corr
