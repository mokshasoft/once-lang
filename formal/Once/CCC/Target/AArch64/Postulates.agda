{-# OPTIONS --sized-types #-}
------------------------------------------------------------------------
-- Once.CCC.Target.AArch64.Postulates
--
...
module Once.CCC.Target.AArch64.Postulates where
...
open import Once.Target.AArch64.Syntax using (x0; x19; x20; x21; x29; x30; Program; sub-sp; stp; sp+imm; mov-from-sp; ret)
open import Once.Target.AArch64.Semantics using (State; readReg; readSP; readMem; exec)
open import Once.Target.AArch64.Semantics using () renaming (module State to St)
open St using (regs; memory; halted; pc)
open import Once.CCC.Target.AArch64.Correct.Star using (Star)
open import Once.CCC.Target.AArch64.Correct.StackInvariant using (StackInvariant; X29Invariant)
open import Once.CCC.Target.AArch64.Correct.ClosureWellFormed using (ThunkResult)
open import Once.CCC.Target.AArch64.CodeGen using (compile-aarch64; compile-length)
...
--
-- NEEDED BY: Once.CCC.Target.AArch64.Correct.MutualIR (inl, inr, pair, case, curry)
--
-- JUSTIFICATION:
...
--   - Once.CCC.Target.AArch64.Correct.ThunkProof (construct-closure-wf)
--   - Once.CCC.Target.AArch64.Correct.MutualIR (run-curry-star-direct)
--
-- ELIMINATION:
...
--     open import Once.CCC.Target.AArch64.Correct.MutualIR using (curry-thunk-correct-impl)
--
------------------------------------------------------------------------
...
--
-- PREVIOUSLY NEEDED BY:
--   - Once.CCC.Target.AArch64.Correct.IR.Apply (but was imported, never used)
--
-- ELIMINATION:
...
--
-- NEEDED BY: Once.CCC.Target.AArch64.Correct.MutualIR (run-apply-star-direct)
--
