{-# OPTIONS --sized-types #-}
------------------------------------------------------------------------
-- Once.CCC.Target.AArch64.Correct.ClosureContext
--
...
module Once.CCC.Target.AArch64.Correct.ClosureContext where
...
open import Once.CCC.Target.AArch64.Correct.ClosureWellFormed
  using (ClosureWellFormed; ThunkResult;
         ...
         thunk-x20; thunk-x29; thunk-stack-inv; thunk-sp-bound)
...
-- NOTE: This section is commented out due to pattern matching issues with infix
-- operators in Agda. It's not currently needed for the basic ClosureContext
-- infrastructure.
...
record ApplyMemoryLayout {A B : Type} (prog : Program) (s : State)
  ...
  field
    -- Pair layout: x0 points to (closure-addr, encode arg)
    mem-fst : readMem (memory s) (readReg (regs s) x0) ≡ just closure-addr
    mem-snd : readMem (memory s) (readReg (regs s) x0 +ℕ 8) ≡ just (encode arg)
...
-- NOTE: Commented out for Phase 2 - API mismatch with ClosureWellFormed version
-- TODO: Fix in later phase when integrating with Apply
{-
-- Import the proven run-apply-with-wf
open import Once.CCC.Target.AArch64.Correct.IR.Apply as ApplyProof
...
run-apply-with-full-wf : ∀ {A B} (prefix suffix : Program)
  ...
  ∃[ s' ] (Star prog s s'
          ...
          × readReg (regs s') x0 ≡ encode {B} (semantics arg)
          × StackInvariant s'
          × readSP (regs s') > 16)
run-apply-with-full-wf {A} {B} prefix suffix code-ptr env-addr closure-addr
                       semantics arg s wf mem-layout h-eq pc-eq stack-inv sp>16 =
  let (s' , star , h' , pc' , x0' , x19' , x29' , stack' , sp') =
        run-apply-with-wf prefix suffix code-ptr env-addr semantics arg s wf h-eq pc-eq stack-inv sp>16
          (closure-addr , mem-fst mem-layout , mem-snd mem-layout ,
           mem-env mem-layout , mem-cp mem-layout)
  in s' , star , h' , pc' , x0' , stack' , sp'
-}
