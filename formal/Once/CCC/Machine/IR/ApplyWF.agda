...
module ApplyWFImpl {FS : FrameSemantics} (program-bound : ℕ)
  ...
  run-apply {m} {A} {B} {k} x input-loc s alloc input-valid-wf input-before not-halted rdi-eq =
    ...
    where
      ...
      result-loc-dispatch (at-reg loc _ _ _ _ _) = loc  -- Plan 0.58: at-reg carries a witness loc
      ...
      -- Plan 0.58: body register-resident ⇒ apply's result rides through in-register.
      -- The reg-specific Output rax is scaffolded (same class as apply's other !!s).
      ... | at-reg _ _ _ _ _ _ = at-reg result-loc result-valid-wf' result-before' SMP.!!
                                       cont-preserves-validity' cont-preserves-result'
