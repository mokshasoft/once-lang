...
-- Proof Module
--
-- Parameterized by FrameSemantics, program-bound, and PrimSem.
------------------------------------------------------------------------

module RecSchemeProofImpl {FS : FrameSemantics} (program-bound : ℕ) (primSem : PrimSem) where
  ...
  CataIH {F} {B} wf alg x =
    ...
    → BeforeFrontier alloc result-loc
    → ValidAtWF Heap alloc (sem-cata wf (λ fa → eval primSem alg fa) x) result-loc s
  ...
  --    - Show eval primSem (Cata wf alg) x = sem-cata wf (eval ps alg) x
