...
module ComposeWFImpl {FS : FrameSemantics} (program-bound : ℕ) where
  ...
  run-compose : ∀ {A B C} (mIn : AllocMode) (f : IR A B) (g : IR B C)
    ...
    halted s ≡ false →
    -- Plan 0.58: was `readReg (regs s) Input1 ≡ SV-Ptr input-loc`; now the
    -- `InputPlace` hand-off (so `run-compose` itself satisfies the generalized
    -- `RecDispatcherWF` when plugged into the recursion, and threads its own
    -- input placement to the `f`-run).
  ...
  run-compose mIn f g rec-wf x input-loc s alloc input-valid-wf input-before not-halted rdi-eq =
    ...
    where
      ...
      -- Plan 0.58: `s₁'` writes exactly `readReg Output` (what the real
      -- `mov-to-input` moves), so `s₁'-eq-output` is `refl` and the hand-off
      -- carries whatever placement f produced (pointer OR register sentinel).
      -- Defined here (before FFacts) so the FFacts field `inter-place-f` can
      -- mention it. Depends only on `s₁`, not on `inter-loc`.
      s₁' = record s₁ { regs = writeReg (regs s₁) Input1 (readReg (regs s₁) Output) }
      ...
      record FFacts : Set where
        field
          ...
          -- Plan 0.58: placement-generic hand-off (DUAL of ResultPlace's
          -- at-loc/at-reg on the output side). `s₁'` writes exactly what the
          -- real `mov-to-input` moves — `readReg Output` — so f's result rax
          -- (a pointer for at-loc, the sentinel for at-reg) yields the
          -- `InputPlace` that g's precondition now consumes. No `SV-Ptr`
          -- assumption; no spill.
          inter-place-f  : InputPlace (eval f x) inter-loc-f s₁'
      ...
      ... | at-loc loc valid before rax _ _ = record
              ...
              ; inter-valid-f  = valid
              ; inter-place-f  =
                  in-ptr (trans (writeReg-same (regs s₁) Input1 (readReg (regs s₁) Output)) rax)
              }
      ... | at-reg loc valid before rax _ _ = record
              { inter-loc-f    = loc
              ; inter-before-f = before
              ; inter-valid-f  = valid
              ; inter-place-f  =
                  in-reg (trans (writeReg-same (regs s₁) Input1 (readReg (regs s₁) Output)) rax)
              }
      ... | unit-result = record
              ...
              ; inter-place-f  =
                  in-ptr (trans (writeReg-same (regs s₁) Input1 (readReg (regs s₁) Output)) unit-inter-rax)
              }
        where
          postulate
      ...
      open FFacts f-facts using ()
        renaming (inter-loc-f to inter-loc;
                  ...
                  inter-place-f to inter-place-f')
      ...
      g-result-pair = rec-wf mMid g (∘-g-smaller f g) (eval f x) inter-loc s₁' alloc₁
                        inter-valid-wf' inter-before-f' not-halted₁ inter-place-f'
      ...
      -- Plan 0.58: `s₁'` is DEFINED as this write, so the equation is `refl` —
      -- the old proof went via `inter-rax-f'` (Output ≡ SV-Ptr), the pointer
      -- assumption we removed.
