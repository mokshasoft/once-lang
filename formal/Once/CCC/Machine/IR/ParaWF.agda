...
open import Data.Nat using (ℕ; zero; suc; _<_; _≤_; z≤n; s≤s) renaming (_+_ to _+ℕ_; _*_ to _*ℕ_)
...
module ParaWFImpl {FS : FrameSemantics} (program-bound : ℕ) where
  ...
  run-para-core {F} {A} wf alg rec-wf mIn x input-loc s alloc input-valid-wf input-before not-halted rdi-eq =
    Heap ,
    mk-IRResultAWF-via-bump
      s' alloc' para-trace (mkBump 1 0) refl
      SMP.!!
      refl
      ...
      (record
        ...
        ; bump-fits-stack-budget = ≤-trans (s≤s z≤n) (m≤n+m pair-slots (product-depth wf +ℕ sum-depth wf *ℕ 2 +ℕ ir-stack-requirement alg))
