...
-- Import extensionality for writeLoc-commute-stack proof
...
module MemOps {FS : FrameSemantics} where
  ...
  -- writeStackMem-commute: Writes to different slots in the same frame commute
  -- Proven by functional extensionality (pointwise equality)
  writeStackMem-commute : ∀ (mem : StackMem FS) (f : Frame) (k1 k2 : ℕ) (v1 v2 : ValueLocation FS) →
    k1 ≢ k2 →
    writeStackMem (writeStackMem mem f k1 v1) f k2 v2 ≡
    writeStackMem (writeStackMem mem f k2 v2) f k1 v1
  writeStackMem-commute mem f k1 k2 v1 v2 k1≢k2 =
    extensionality (λ f' → extensionality (λ k' → pointwise f' k'))
    where
      open import Data.Empty using (⊥-elim)

      pointwise : ∀ f' k' →
        writeStackMem (writeStackMem mem f k1 v1) f k2 v2 f' k' ≡
        writeStackMem (writeStackMem mem f k2 v2) f k1 v1 f' k'
      pointwise f' k' with f ≟F f' | k2 ≟ k' | k1 ≟ k'
      ... | yes refl | yes refl | yes k1≡k2 = ⊥-elim (k1≢k2 k1≡k2)
      ... | yes refl | yes refl | no _ with f ≟F f | k2 ≟ k2
      ...   | yes _ | yes _ = refl
      ...   | yes _ | no k2≢k2 = ⊥-elim (k2≢k2 refl)
      ...   | no f≢f | _ = ⊥-elim (f≢f refl)
      pointwise f' k' | yes refl | no _ | yes refl with f ≟F f | k1 ≟ k1
      ...   | yes _ | yes _ = refl
      ...   | yes _ | no k1≢k1 = ⊥-elim (k1≢k1 refl)
      ...   | no f≢f | _ = ⊥-elim (f≢f refl)
      pointwise f' k' | yes refl | no k2≢k' | no k1≢k' with f ≟F f | k1 ≟ k' | f ≟F f | k2 ≟ k'
      ...   | yes _ | yes k1≡k' | _ | _ = ⊥-elim (k1≢k' k1≡k')
      ...   | yes _ | no _ | yes _ | yes k2≡k' = ⊥-elim (k2≢k' k2≡k')
      ...   | yes _ | no _ | yes _ | no _ = refl
      ...   | yes _ | no _ | no f≢f | _ = ⊥-elim (f≢f refl)
      ...   | no f≢f | _ | _ | _ = ⊥-elim (f≢f refl)
      pointwise f' k' | no f≢f' | _ | _ with f ≟F f' | k1 ≟ k' | f ≟F f' | k2 ≟ k'
      ...   | yes f≡f' | _ | _ | _ = ⊥-elim (f≢f' f≡f')
      ...   | no _ | _ | yes f≡f' | _ = ⊥-elim (f≢f' f≡f')
      ...   | no _ | _ | no _ | _ = refl

  -- writeStackMem-commute-diff-frame: Writes to different frames commute
  writeStackMem-commute-diff-frame : ∀ (mem : StackMem FS) (f1 f2 : Frame) (k1 k2 : ℕ) (v1 v2 : ValueLocation FS) →
    f1 ≢ f2 →
    writeStackMem (writeStackMem mem f1 k1 v1) f2 k2 v2 ≡
    writeStackMem (writeStackMem mem f2 k2 v2) f1 k1 v1
  writeStackMem-commute-diff-frame mem f1 f2 k1 k2 v1 v2 f1≢f2 =
    extensionality (λ f' → extensionality (λ k' → pointwise f' k'))
    where
      open import Data.Empty using (⊥-elim)

      pointwise : ∀ f' k' →
        writeStackMem (writeStackMem mem f1 k1 v1) f2 k2 v2 f' k' ≡
        writeStackMem (writeStackMem mem f2 k2 v2) f1 k1 v1 f' k'
      pointwise f' k' with f1 ≟F f' | f2 ≟F f'
      ... | no f1≢f' | no f2≢f' with f1 ≟F f' | k1 ≟ k' | f2 ≟F f' | k2 ≟ k'
      ...   | yes f1≡f' | _ | _ | _ = ⊥-elim (f1≢f' f1≡f')
      ...   | no _ | _ | yes f2≡f' | _ = ⊥-elim (f2≢f' f2≡f')
      ...   | no _ | _ | no _ | _ = refl
      pointwise f' k' | yes refl | no f2≢f' with k1 ≟ k'
      ...   | yes refl with f1 ≟F f1 | k1 ≟ k1
      ...     | yes _ | yes _ = refl
      ...     | yes _ | no k1≢k1 = ⊥-elim (k1≢k1 refl)
      ...     | no f1≢f1 | _ = ⊥-elim (f1≢f1 refl)
      pointwise f' k' | yes refl | no f2≢f' | no k1≢k' with f1 ≟F f' | k1 ≟ k' | f2 ≟F f' | k2 ≟ k'
      ...   | yes _ | yes k1≡k' | _ | _ = ⊥-elim (k1≢k' k1≡k')
      ...   | yes _ | no _ | yes f2≡f1 | _ = ⊥-elim (f1≢f2 (sym f2≡f1))
      ...   | yes _ | no _ | no _ | _ = refl
      ...   | no f1≢f1 | _ | _ | _ = ⊥-elim (f1≢f1 refl)
      pointwise f' k' | no f1≢f' | yes refl with k2 ≟ k'
      ...   | yes refl with f2 ≟F f2 | k2 ≟ k2
      ...     | yes _ | yes _ = refl
      ...     | yes _ | no k2≢k2 = ⊥-elim (k2≢k2 refl)
      ...     | no f2≢f2 | _ = ⊥-elim (f2≢f2 refl)
      pointwise f' k' | no f1≢f' | yes refl | no k2≢k' with f2 ≟F f' | k2 ≟ k' | f1 ≟F f' | k1 ≟ k'
      ...   | yes _ | yes k2≡k' | _ | _ = ⊥-elim (k2≢k' k2≡k')
      ...   | yes _ | no _ | yes f1≡f2 | _ = ⊥-elim (f1≢f2 f1≡f2)
      ...   | yes _ | no _ | no _ | _ = refl
      ...   | no f2≢f2 | _ | _ | _ = ⊥-elim (f2≢f2 refl)
      pointwise f' k' | yes f1≡f' | yes f2≡f' = ⊥-elim (f1≢f2 (trans f1≡f' (sym f2≡f')))

  -- writeLoc-commute-stack: Writes to different stack slots in same frame commute
  ...
  writeLoc-commute-stack s' f k1 k2 v1 v2 k1≢k2 =
    cong (λ sm → record s' { stackMem = sm })
         (writeStackMem-commute (stackMem s') f k1 k2 v1 v2 k1≢k2)
