...
module TwoCellC {FS : FrameSemantics} (program-bound : ℕ) where
  ...
  obs-correct-curry {A} {B} {C} body n l prog base _ cr span mIn x s alloc cl n≤ nh inp k =
    record
  ...
  obs-correct-Ana {F} wf {A} coalg n l prog base _ cr span mIn x s alloc cl n≤ nh inp k =
    record
      { traces-agree   = cong (take k) (sym (denot-[] k))
      ; value-realized =
          realized 10 TCB.fs10 Heap (falloc TCB.fs10) TCB.run TCB.nh10 refl refl refl place
