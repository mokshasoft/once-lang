...
RI0 : ∀ (c : C.FunCtx) (p : PolyCtx) (s : SigEffectCtx) (nm : String) (e : RawExpr)
  ...
  (d₂ : (ctxWithImportsAndSelfAndPolys c p s nm EffUU) ⊢ᶜ e ∶ EffUU ⨾ Ψ₂)
  (dγ : ⟦ ⟦ ∅ ⟧ᶜ ⟧ᴰ) (n : ℕ) →
  SD.⟦ realize d₁ ⟧ˢ dγ n ≡ SD.⟦ realize d₂ ⟧ˢ dγ n
...
mt-den-indep : ∀ {polys sigEffs funs ctx}
  ...
  (dγ : ⟦ ⟦ ∅ ⟧ᶜ ⟧ᴰ) (n : ℕ) →
  SD.⟦ proj₂ (MC.mainRealized-go mt me) ⟧ˢ dγ n
  ≡ SD.⟦ proj₂ (MC.mainRealized-go bt bme) ⟧ˢ dγ n
...
mt-den-indep {polys = polys} {sigEffs = sigEffs} {ctx = ctx}
  ...
  where
    ...
    dispatch2 : (w₁ : MC.MainExists rt₁) (w₂ : MC.MainExists rt₂) →
      SD.⟦ proj₂ (MC.mrg-dispatch d₁ rt₁ w₁ (funName fi ≟str "main") (ty₁ ≟T EffUU) (funIsPrimitive fi)) ⟧ˢ dγ n
      ≡ SD.⟦ proj₂ (MC.mrg-dispatch d₂ rt₂ w₂ (funName fi ≟str "main") (ty₁ ≟T EffUU) (funIsPrimitive fi)) ⟧ˢ dγ n
    ...
    dispatch : (me : MC.MainExists (AS.tcons {fi = fi} rf₁ d₁ rt₁)) (bme : MC.MainExists (AS.tcons {fi = fi} rf₂ d₂ rt₂)) →
      SD.⟦ proj₂ (MC.mainRealized-go (AS.tcons {fi = fi} rf₁ d₁ rt₁) me) ⟧ˢ dγ n
      ≡ SD.⟦ proj₂ (MC.mainRealized-go (AS.tcons {fi = fi} rf₂ d₂ rt₂) bme) ⟧ˢ dγ n
