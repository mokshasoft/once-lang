...
open IRObsCorrectFlatness {x86-64-frame-semantics} program-bound using (ir-obs-correct)
...
postulate
  ...
  -- The `just` case is the refinement — DECOMPOSED top-down below (`conc-flat-
  -- sim-just`): the recovered flat↔x86-64 cluster (Adequacy.ArchCorrectness.X86-64.
  -- FlatSimulation + the ConcFlatSim assembly) discharges it. The `nothing` case
  -- is proven here (both traces `[]`).
  conc-flat-sim-just :
    ∀ (ir : IR Unit Unit) (n : ℕ) →
