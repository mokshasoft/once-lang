...
module Once.Adequacy.ArchCorrectness.X86-64
  ...
  (x86-64-lit-fits : RB.LitFits o) where
...
x86-64-conc-flat-sim brs nothing   n = refl
x86-64-conc-flat-sim brs (just ir) n = conc-flat-sim-just brs ir n
...
asm-trace-correct-x86-64 : ∀ (brs : (ir : IR Unit Unit) → BlockRuns (ir-to-trace ir)) → FFOx.AsmTraceCorrect (FFOx.flat-trace-of ir-obs-correct brs)
asm-trace-correct-x86-64 brs m asm eq dl lr sr n =
  trans (x86-64-loader-faithful m asm eq dl lr sr n)
        (x86-64-conc-flat-sim brs (moduleToIR-emitted m) n)
...
BlockRunsHyp-x86-64 = (ir : IR Unit Unit) → BlockRuns (ir-to-trace ir)
