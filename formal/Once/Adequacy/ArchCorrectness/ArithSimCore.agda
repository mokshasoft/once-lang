...
--
-- This module had TWO bakes of 64 (`open Correct 64` and
-- `open OnceWord.Word64`), and it is the shared CORRESPONDENCE core: the
-- statement that a concrete arith interpreter agrees with `block-semM`. With
-- 64 on both sides of that statement, the x86-32 instance was proving that a
-- 64-bit model of x86-32 matches a 64-bit meaning — true, and about no real
-- machine. The width is the instance’s now.
------------------------------------------------------------------------
...
module At (tn : TargetNum) where
  ...
  module Core
    ...
    output-extract {sh} e env s-conc R-end = trans (block-value-semM tn e env) (cong just body)
      where
        ...
        bvs' : ArithAbsState.regs aPre [ xreg-idx XR0 ] ≡ just (block-semM e tn (toWord tn sh env))
        bvs' = subst (λ t → output-of t ≡ just (block-semM e tn (toWord tn sh env))) blk≡ (block-value-semM tn e env)
        rr-eq : block-semM e tn (toWord tn sh env) ≡ rr (e1 (XI.Xmov-out XR0) cPre) (arith-reg XR0)
        rr-eq = R' XR0 (block-semM e tn (toWord tn sh env)) bvs'
        body : block-semM e tn (toWord tn sh env) ≡ rr (eb (emit-program (compile-abs e)) s-conc) out-reg
    ...
    arith-block-correct : ∀ {sh} (e : MArithIR sh NInt) (env : ⟦ sh ⟧S) (s-conc : St)
      ...
      → rr (eb (emit-program (compile-abs e)) s-conc) out-reg
          ≡ block-semM e tn (toWord tn sh env)
