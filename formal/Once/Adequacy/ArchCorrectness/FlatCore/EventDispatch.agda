...
-- …and the ABSTRACT-MACHINE well-formedness layer, 1,099 lines that mention no
-- machine at all (plan 0.65 G1d). The dispatch leans on it constantly:
...
open CFC using (HeapView; HDom; slots; haddr; hfront; lo; caddr)
...
open import Once.CCC.Machine.FlatStoreWF FS using
  (FlatWF; flat-wf-step; cl-step; sv-below; svm-below; wf-regs; wf-heap; wf-stack; wf-fresh)
  ...
  mutual
    ...
    thunk-step {hv} n ev env prog fs s m b cc wf h ftq =
      ...
      ccc-step-bs n ev env prog fs s (instr-ctrl (c-thunk m b))
        (bs-c-thunk bss prog fs s m b cc h ftq lo' lo'≤lo front-lo' lo'≤rsp fits
