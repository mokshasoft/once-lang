...
mem-agree-heap : ∀ i s → (∀ sc → InStack (scratch-addr s sc)) → ∀ a → InHeap a
               → readMem (memory (EA.exec1 val-x86-64 i s)) a ≡ readMem (memory s) a
...
wf-e1 : ∀ i s → WF s → WF (EA.exec1 val-x86-64 i s)
wf-e1 i s (inStk , inHp) =
  (λ sc → subst InStack (sym (sa-inv i s sc)) (inStk sc)) ,
  (λ p → subst (λ ptr → HeapChase (memory (EA.exec1 val-x86-64 i s)) ptr p) (sym (rdi-inv i s))
               (heapchase-agree (memory s) (memory (EA.exec1 val-x86-64 i s)) (readReg (regs s) rdi) p
                                (mem-agree-heap i s inStk) (inHp p)))

pl-inv : ∀ i s → WF s → ∀ p → path-load (EA.exec1 val-x86-64 i s) p ≡ path-load s p
...
pl-inv (XI.Xmov-r-m sc' src) s (inStk , inHp) p =
  trans (pathloadgo≡plg (EA.exec1 val-x86-64 (XI.Xmov-r-m sc' src) s) (readReg (regs s) rdi) p)
