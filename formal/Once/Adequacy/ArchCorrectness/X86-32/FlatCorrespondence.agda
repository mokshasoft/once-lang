...
------------------------------------------------------------------------
-- Once.Adequacy.ArchCorrectness.X86-32.FlatCorrespondence
--
-- x86-32's instance of `FlatCore.FlatCorrespondence` (plan 0.66 X2).
--
-- THE THIRD INSTANCE, and the cheapest of the three: x86-32's `State` has
-- x86-64's exact shape (`mkstate regs memory flags pc halted`), so this is
-- x86-64's file with the register file swapped — which is the strongest
-- evidence so far that plan 0.65's extraction actually generalised rather than
-- merely typechecked twice.
--
-- The correspondence itself — `FlatCorr`, the `sim-*` lemmas, the four
...
module Once.Adequacy.ArchCorrectness.X86-32.FlatCorrespondence
  (FS : FrameSemantics)
...
open import Once.CCC.Target.X86-32.Syntax using (Reg)
...
open RegRoles x86-32-roles using (reg-of)
...
open import Once.Adequacy.ArchCorrectness.FlatCore.FlatCorrespondence
       FS slot-size word-eq float-bits-single Reg x86-32-roles X.State rreg memory xhalted
  public


------------------------------------------------------------------------
-- x86-32 REALISES `SetsRole`: the state that writes one role's register and
-- leaves memory, the halt flag and the other seven roles alone.
--
-- THE 49 CLAUSES ARE THE POINT, not an accident. While the post-state was a
-- concrete `mkstate` literal, "writing eax leaves ecx alone" was free —
-- `readReg (writeReg rf eax v) ecx` reduces because `eax` and `ecx` are
...
-- edi, 1` really does set flags, so a realiser that fixed `flags s` would only
...
-- …and x86-32 realises `SetsMem` the same way, out of `writeMem`'s two
