...
------------------------------------------------------------------------
-- Once.Adequacy.ArchCorrectness.X86-32.ConcFlatSim  (plan 0.66 X2)
--
-- Wiring the recovered flat↔x86-32 refinement cluster (FlatSimulation +
-- FlatCorrespondence + FlatComposition + StepLemmas) toward the apex
-- `x86-32-conc-flat-sim`. This module is the ASSEMBLY layer:
--
...
-- so the concrete instantiation (x86-32-frame-semantics + the heap layout) is
...
import Once.Adequacy.ArchCorrectness.X86-32.FlatSimulation as FSim
import Once.Adequacy.ArchCorrectness.X86-32.RunContext as RC
...
module Once.Adequacy.ArchCorrectness.X86-32.ConcFlatSim (o : CanonicalName)
  ...
  -- the body's reservation does not run `%esp` down into the heap frontier.
  ...
  -- `…X86-32.ResourceBounds` copies pin `x86-32-frame-semantics` and are the
  ...
  (scratch-dec-guarded : ∀ {hv : FC.HeapView FS word-eq}
                       ...
                       → 1 ≤ X.readReg (X.State.regs s) edx)
  ...
  (ret-no-wrap : ∀ {hv : FC.HeapView FS word-eq}
               ...
               -- BASE, frame AND the slot the call spent. x86-32 only needs the
               -- weaker half here; riscv64, which adds both in one `addi`,
               -- needs this one. See `Resources.ret-no-wrap`.
               → X.readReg (X.State.regs s) esp + slots (suc b) < X.W.modulus)
  (count-no-wrap : ∀ {hv : FC.HeapView FS word-eq}
                 ...
                 → X.readReg (X.State.regs s) edi + 1 < X.W.modulus)
...
open import Once.Adequacy.ArchCorrectness.X86-32.FlatSimulation o FS word-eq public
...
open import Once.CCC.Target.X86-32.Syntax using (slots; esi)
...
-- once, for every arch, comes through `EE`. What x86-32 supplies is the four
...
-- x86-32 filling `EngineInterface`'s three records. What was a 49-argument
-- module application is three values now, for the memory reason recorded in
-- `EngineInterface`; the contents are identical.
x86-32-emitter : EI.Emitter FS Reg
x86-32-emitter = record
  { Instr = XS.Instr
...
x86-32-machine : EI.Machine FS Reg x86-32-emitter
x86-32-machine = record
  { State = X.State ; rreg = xrreg ; memory = X.State.memory
  ; xhalted = X.State.halted ; xpc = X.State.pc
  -- x86-32's `call` PUSHES the return address, so an unspilled return is
  -- already in its stack cell: the link claim IS the memory claim.
  ; link-claim = x86-32-link-claim
...
-- `List XInstr × ℕ` and the field is `uncurry`'d; x86-32's block executes
-- without one, so the payload is just the block. The engine is parameterised
-- over `Payload` precisely so this is a choice an arch makes rather than a
-- shape it inherits.
...
-- fact that says so: the heap frontier is at or below `%esp`. A LIVE heap cell
-- is strictly below the frontier (`HeapView.dom-below`), hence below any address
-- at or above `%esp` — which is every stack slot the emitted code touches. So
...
-- hardware's `mov [ecx],eax` stores the address and continues. `writeLoc` now
...
------------------------------------------------------------------------
-- THE STUCK ROUTES, x86-32's half (plan 0.65 G2 item 4, slice 3).
--
...
-- names live: a `mov eax,[ecx]` faulting on an unmapped address, a `jmp`/`je`
...
stuck-c-jmp ev env prog fs s m cc h ftq fl-eq =
  ...
  where
    ...
    fetch-x86 : X.fetch (compile-trace prog) (X.State.pc s) ≡ just (jmp-l (once m))
    ...
    step-eq : X.execInstr (compile-trace prog) s (jmp-l (once m)) ≡ just s'
...
stuck-c-branch-scratch-zero {hv} ev env prog fs s m cc h ftq sc-eq fl-eq =
  ...
  where
    ...
    rbx0 : X.readReg (X.State.regs s) edx ≡ 0
    ...
    post-cmp : X.State
    post-cmp = record s { flags = X.mkflags (X.readReg (X.State.regs s) edx ≡ᵇ 0)
                                            (X.readReg (X.State.regs s) edx X.<ᵇ 0) false
                        ; pc = X.State.pc s + 1 }
    step-cmp : X.execInstr (compile-trace prog) s (cmp (reg edx) (imm 0)) ≡ just post-cmp
    step-cmp = b-cmp-reg-imm (compile-trace prog) s edx 0
...
stuck-c-branch-tag-zero ev env prog fs s m loc cc h ftq i-eq r-eq rd fl-eq =
  ...
  where
    ...
    fetch-cmp : X.fetch (compile-trace prog) (X.State.pc s)
              ≡ just (cmp (mem (base ecx)) (imm 0))
    fetch-cmp = trans (cong (X.fetch (compile-trace prog)) (pc-off cc))
                      (fetch-block-head prog (fpc fs) (instr-ctrl (c-branch-tag-zero m)) ftq)
    post-cmp : X.State
    post-cmp = record s { flags = X.mkflags (0 ≡ᵇ 0) (0 X.<ᵇ 0) false
                        ; pc = X.State.pc s + 1 }
    step-cmp : X.execInstr (compile-trace prog) s (cmp (mem (base ecx)) (imm 0))
             ≡ just post-cmp
    -- the core hands the read at `rreg s in1-reg + 0` (a DISPLACEMENT, which
    -- x86-64's `[rdi+0]` and riscv64's `ld t1, 0(t0)` produce and x86-32's
    -- `cmp [ecx], 0` does not); convert it here, next to the addressing mode.
    rd' : X.readMem (X.State.memory s) (X.effectiveAddr s (base ecx)) ≡ just 0
    rd' = subst (λ a → X.readMem (X.State.memory s) a ≡ just 0)
                (+-identityʳ (X.readReg (X.State.regs s) ecx)) rd
    step-cmp = execInstr-cmp-mi (compile-trace prog) s (base ecx) 0 rd'
    fetch-je : X.fetch (compile-trace prog) (X.State.pc post-cmp) ≡ just (je (once m))
    fetch-je = trans (cong (λ p → X.fetch (compile-trace prog) (p + 1)) (pc-off cc))
...
-- …and the supply record itself. Same role as `x86-32-block-steps`: the field
-- types come FROM the record, so an interface weakened to fit one arch breaks
-- here rather than silently.
x86-32-stuck-steps : EE.StuckSteps
x86-32-stuck-steps = record
...
postulate
  ...
  -- closure pointer into `%ebx` (`instr-save-closure-reg`) after the `curry`
  ...
  arith-sigop-contract : ∀ {hv : HeapView} (env : RTx.ArithEnv val-x86-32) prog fs s {A B} (si : SigOpInfo A B)
                       ...
                       → Σ (List XInstr) (λ pl → env (once-symbol-path (SigOpInfo.name si)) ≡ just pl
                           × CompiledCorr hv prog (flat-exec-instr (instr-sigop si) prog fs)
                               (dispatch-arith val-x86-32 pl s))
...
-- `FlatCore.EventDispatch`. x86-32 supplies the record below and gets the
-- whole induction back.
------------------------------------------------------------------------
x86-32-supply : EE.Supply
x86-32-supply = record
  { bss = x86-32-block-steps ; sts = x86-32-stuck-steps
