...
------------------------------------------------------------------------
-- Once.Adequacy.ArchCorrectness.X86-32.ResourceBounds
--
-- The THIRD instance (plan 0.66 X2). Same family, same classes, same reasons —
-- the only differences are the register names, the modulus (2³²), and the float
-- encoder, all of which are exactly the things that SHOULD differ.
--
-- The x86-32 RESOURCE BOUNDS, given names so they can be threaded as
...
-- TWO telescopes — `…ArchCorrectness.X86-32`'s, which consumes it, and
...
-- it asserts the adequacy of `step-budget-x86-32`, which is an UNDEFINED
-- postulated `ℕ → ℕ` in `Once.Adequacy.CPU.X86-32`. That is a hole in the
...
module Once.Adequacy.ArchCorrectness.X86-32.ResourceBounds (o : CanonicalName) where
...
HeapRoom =
  ...
  → FlatMachine.fetch {x86-32-frame-semantics} prog
      (FlatMachine.fpc {x86-32-frame-semantics} fs) ≡ just (instr-alloc-heap n)
  → FCx.hfront hv + slots n ≤ FCx.lo hv

------------------------------------------------------------------------
-- STACK EXHAUSTION: at an emitted `c-thunk m b` the body's reservation does
-- not run `%esp` down into the heap's allocation frontier.
--
...
--
-- WHY `hfront + slots b ≤ %esp` AND NOT THE TWO CONSEQUENCES SEPARATELY. The
-- `c-thunk` block-step needs `slots b ≤ %esp` (the `sub` does not underflow)
-- and `hfront ≤ %esp ∸ slots b` (the reserved frame stays above the heap).
...
-- %esp` (the push does not underflow) and `hfront ≤ %esp ∸ slot-size` (the
...
ScratchDecGuarded =
  ...
  → FSimx.CompiledCorr o x86-32-frame-semantics refl hv prog fs s
  → FlatMachine.fetch {x86-32-frame-semantics} prog
      (FlatMachine.fpc {x86-32-frame-semantics} fs) ≡ just (instr-reg-op scratch-dec)
...
-- (`esp` frame release, `esi` heap bump, the `eax`/`ecx` index scaling) or
-- the observable COUNTER (`edi`). A user `Int` addition never reaches this
...
-- (`ArchCorrectness.X86-32` → `ArchCorrectness` → `Compiler` → `Certified`)
-- is written once and does not change as fields are added.
record AddrNoWrap : Set₁ where
  field
    -- `c-ret b` → `add esp, slots b`: releasing the frame stays in range.
    ...
    ret-no-wrap :
      ...
      → FSimx.CompiledCorr o x86-32-frame-semantics refl hv prog fs s
      → FlatMachine.fetch {x86-32-frame-semantics} prog
          (FlatMachine.fpc {x86-32-frame-semantics} fs) ≡ just (instr-ctrl (c-ret b))
      → X.readReg (X.State.regs s) esp + slots (suc b) < W.modulus

    -- `count-inc` → `add edi, 1`: THE ONE NON-ADDRESS SITE. `%edi` is the
    ...
    count-no-wrap :
      ...
      → FSimx.CompiledCorr o x86-32-frame-semantics refl hv prog fs s
      → FlatMachine.fetch {x86-32-frame-semantics} prog
          (FlatMachine.fpc {x86-32-frame-semantics} fs) ≡ just (instr-reg-op count-inc)
      → X.readReg (X.State.regs s) edi + 1 < W.modulus
...
record LitFits : Set₁ where
  field
    -- `instr-load-tag-lit n` → `mov eax, imm n`: sum/loop tags.
    tag-fits :
      ∀ {hv : FCx.HeapView x86-32-frame-semantics refl}
        (prog : AbstractTrace) (fs : FlatMachine.FlatState {x86-32-frame-semantics})
        (s : X.State) (n : ℕ)
      ...
      → FSimx.CompiledCorr o x86-32-frame-semantics refl hv prog fs s
      → FlatMachine.fetch {x86-32-frame-semantics} prog
          (FlatMachine.fpc {x86-32-frame-semantics} fs) ≡ just (instr-load-tag-lit n)
      → n < W.modulus

    -- `instr-load-const fits-int v` → `mov eax, imm v`: THE user literal.
    lit-fits :
      ∀ {hv : FCx.HeapView x86-32-frame-semantics refl}
        (prog : AbstractTrace) (fs : FlatMachine.FlatState {x86-32-frame-semantics})
        (s : X.State) (v : Carrier)
    ...
    -- only for symmetry with the other two arches; it is the one field of this
    -- family that could be discharged today.
    float-fits :
      ∀ {hv : FCx.HeapView x86-32-frame-semantics refl}
        (prog : AbstractTrace) (fs : FlatMachine.FlatState {x86-32-frame-semantics})
        (s : X.State) (v : Dyadic)
