...
sound-body : ∀ {name params toks d rest bnd} →
  parseFunBodyB name params toks ≡ just (d , rest , bnd) →
  ParsesFunBody name params toks d rest
sound-body {name} {params} {toks} h with eqHead toks in eh
... | false with () ← h
...
complete-body : ∀ {name params toks d rest} → ParsesFunBody name params toks d rest →
  Σ[ bnd ∈ (length rest < length toks) ] parseFunBodyB name params toks ≡ just (d , rest , bnd)
...
sound-fundef {name} {toks} h with parseParamsB toks in pp
... | (params , toks'' , paramsBnd) with parseFunBodyB name params toks'' in fb
...   | nothing with () ← h
...   | just (d , rest , bodyBnd) with refl ← just-injective h =
        pfd-mk (subst₂ (ParsesParams toks) (cong proj₁ pp) (cong (λ z → proj₁ (proj₂ z)) pp) (sound-params toks))
               (sound-body fb)
...
complete-fundef (pfd-mk pp pb) with complete-params pp
