...
module Once.Surface.Desugar (⟦_⟧ : Type → Set) where
...
module DesugarDef (CI : ContractInterface) where
  module C = IRDef CI
  open C
  ...
  --
  -- Contract is machine-independent; semantics is embedded in IR.Prim.
  --
  ...
  --
  postulate
    makeContract : ∀ {A B} (sem : ⟦ A ⟧ → ⟦ B ⟧) → Contract A B

  prim-desugar : ∀ {A B} → String → (sem : ⟦ A ⟧ → ⟦ B ⟧) → C.IR A B
  prim-desugar name sem = C.Prim name sem (makeContract sem)
