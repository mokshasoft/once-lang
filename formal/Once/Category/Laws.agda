...
--
-- Import structural coercions and coherence from Core
open import Once.Semantics.IR
  using (coerce-struct; coerce-struct⁻¹;
         coerce-struct-round-trip; coerce-struct⁻¹-round-trip;
         fmap-struct-coherence; fmap-struct-coherence′;
         sem-fmap-Type)
...
-- | Structural coercions equal subst-based coercions
--
-- Postulated because proving this requires pattern matching on the
-- complex proof terms generated by sem-functor-coherence's rewrite.
-- Both coercions are mathematically the identity function (the types
-- are definitionally equal for polynomial functors), so this is sound.
--
postulate
  coerce-struct-eq-functor : ∀ F X (x : ⟦ ⟦ F ⟧T X ⟧)
                           → coerce-struct F X x ≡ coerce-functor F X x
  coerce-struct⁻¹-eq-functor : ∀ F X (y : ⟦ F ⟧F ⟦ X ⟧)
                             → coerce-struct⁻¹ F X y ≡ coerce-functor⁻¹ F X y

-- | coerce⁻¹ ∘ sem-fmap ∘ coerce ≡ fmap-Type (for Type-level input)
--
-- Proof: Use structural coherence and transport to subst-based coercions.
--
fmap-coerce-coherence : ∀ F {X Y : Type} (f : ⟦ X ⟧ → ⟦ Y ⟧) (x : ⟦ ⟦ F ⟧T X ⟧)
                      → coerce-functor⁻¹ F Y (sem-fmap F f (coerce-functor F X x)) ≡ fmap-Type F f x
fmap-coerce-coherence F {X} {Y} f x =
  trans (sym (coerce-struct⁻¹-eq-functor F Y _))
        (trans (cong (coerce-struct⁻¹ F Y) (cong (sem-fmap F f) (sym (coerce-struct-eq-functor F X x))))
               (trans (fmap-struct-coherence F f x)
                      (sym (fmap-Type-eq F f x))))
...
fmap-coerce-coherence′ F {X} {Y} f y =
  trans (sym (coerce-struct⁻¹-eq-functor F Y _))
        (trans (fmap-struct-coherence′ F f y)
               (trans (cong (sem-fmap-Type F f) (coerce-struct⁻¹-eq-functor F X y))
                      (sym (fmap-Type-eq F f _))))
