...
eval-pair-unique : ∀ {A B C} (h : IR C (A * B)) (x : ⟦ C ⟧)
                 → eval (⟨ fst ∘ h , snd ∘ h ⟩ Heap) x ≡ eval h x
...
eval-case-unique : ∀ {A B C} (h : IR (A + B) C) (x : ⟦ A + B ⟧)
                 → eval [ h ∘ inl Heap , h ∘ inr Heap ] x ≡ eval h x
...
eval-curry-apply : ∀ {A B C} (f : IR (A * B) C) (x : ⟦ A * B ⟧)
                 → eval (apply ∘ ⟨ curry f Heap ∘ fst , snd ⟩ Heap) x ≡ eval f x
...
eval-curry-eta : ∀ {A B C} (g : IR A (B ⇒ C)) (a : ⟦ A ⟧) (b : ⟦ B ⟧)
               → Closure.semantics (eval (curry (apply ∘ ⟨ g ∘ fst , snd ⟩ Heap) Heap) a) b ≡ Closure.semantics (eval g a) b
