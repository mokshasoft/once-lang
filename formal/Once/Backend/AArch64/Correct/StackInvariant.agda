...
-- | X29 invariant: sp ≤ x29 (stack pointer at or below frame pointer)
record X29Invariant (s : State) : Set where
  field
    sp≤x29 : readSP (regs s) ≤ readReg (regs s) x29

open X29Invariant public

-- | Derive address disjointness from X29Invariant
-- If sp ≤ x29 and sp > 16, then (sp - 16) < sp ≤ x29, so (sp - 16) ≢ x29
...
x29-addr-diff-from-invariant s x29-inv sp>16 = diff-1 , diff-2
  where
    sp-val = readSP (regs s)
    x29-val = readReg (regs s) x29
    new-sp = sp-val ∸ 16

    sp≤x29' : sp-val ≤ x29-val
    sp≤x29' = X29Invariant.sp≤x29 x29-inv
...
x29-inv-preserved-unchanged : ∀ (s s' : State) →
  ...
  readSP (regs s') ≡ readSP (regs s) →
  X29Invariant s'
x29-inv-preserved-unchanged s s' x29-inv x29-eq sp-eq = record
  { sp≤x29 = subst₂ _≤_ (sym sp-eq) (sym x29-eq) (X29Invariant.sp≤x29 x29-inv)
  }
...
x29-inv-preserved-sp-decreased : ∀ (s s' : State) →
  ...
  readSP (regs s') ≤ readSP (regs s) →
  X29Invariant s'
x29-inv-preserved-sp-decreased s s' x29-inv x29-eq sp-le = record
  { sp≤x29 = ≤-trans sp-le (subst₂ _≤_ refl (sym x29-eq) (X29Invariant.sp≤x29 x29-inv))
  }
