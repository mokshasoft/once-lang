...
--
-- Mutual block for IR code generation correctness proofs.
--
-- This module contains ONLY the mutually recursive functions.
-- Pre-mutual helpers are in PreMutual.agda.
--
-- Main export: run-ir-at-offset
--   Proves that executing IR at any program offset terminates with
--   correct result in rax, preserving callee-saved registers.
--
-- Level 3b - depends on PreMutual
------------------------------------------------------------------------

module Once.Backend.X86.Correct.MutualIR where

-- Re-export everything from PreMutual
open import Once.Backend.X86.Correct.PreMutual public

-- Import helper modules (compiled separately for faster incremental builds)
open import Once.Backend.X86.Correct.PairHelpers public
  using ( add-7-2; commute-9
        ; pair-setup-push-r14; pair-setup-push-r15; pair-setup-push-rbp
        ; pair-setup-frame; pair-setup-sub; pair-setup-base; pair-setup-save
        ; pair-store-f; pair-restore-input
        ; pair-store-g; pair-return; pair-restore-rsp
        ; pair-pop-rbp; pair-pop-r15; pair-pop-r14
        ; step-count-eq
        )
open import Once.Backend.X86.Correct.CurryHelpers public
  using ( closure-code-ptr-x86; closure-env-x86
        ; curry-sub-rsp; curry-mov-env; curry-mov-rax-rsp
        ; curry-lea-r9; curry-mov-code-ptr; curry-jmp; curry-end-label
        ; apply-load-closure; apply-load-arg; apply-load-env
        ; apply-load-code; apply-mov-arg; apply-call
        ; thunk-sub-rsp; thunk-mov-env; thunk-mov-arg; thunk-mov-rdi-rsp; thunk-ret
        ; curry-pc-final; curry-pc-after-jmp
        )
...
mutual
  ...
  run-ir-at-offset-pair {A} {B} {C} f g prefix suffix x s h-false pc-eq rdi-eq stack-inv rsp>16 =
    ...
    where
      ...
      -- Note: add-7-2 and commute-9 imported from PairHelpers
      ...
      -- Note: step-count-eq imported from PairHelpers

      exec-all : exec (compile-length ⟨ f , g ⟩) prog s ≡ just s-final
      exec-all = subst (λ n → exec n prog s ≡ just s-final) (step-count-eq len-f len-g) exec-1-5
  ...
  -- Note: closure-code-ptr-x86 and closure-env-x86 imported from CurryHelpers
  ------------------------------------------------------------------------
