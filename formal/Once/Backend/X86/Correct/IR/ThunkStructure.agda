...
-- Updated for frame pointer handling in thunk
------------------------------------------------------------------------
...
-- The thunk setup instructions (positions 6-12)
...
thunk-i1 : Instr
thunk-i1 = push (reg rbp)

thunk-i2 : Instr
thunk-i2 = mov (reg rbp) (reg rsp)

thunk-i3 : Instr
thunk-i3 = sub (reg rsp) (imm 16)

thunk-i4 : Instr
thunk-i4 = mov (mem (base rsp)) (reg r12)

thunk-i5 : Instr
thunk-i5 = mov (mem (base+disp rsp 8)) (reg rdi)

thunk-i6 : Instr
thunk-i6 = mov (reg rdi) (reg rsp)

-- The thunk setup (7 instructions, positions 6-12)
...
curry-tail {A} {B} {C} f =
  ...
  in mov (reg rsp) (reg rbp) ∷ pop rbp ∷ ret ∷ label end-label-pos ∷ []

------------------------------------------------------------------------
-- Curry structure theorem: compile-x86 (curry f Heap) has the expected form
------------------------------------------------------------------------

curry-structure : ∀ {A B C} (f : IR (A * B) C) →
  compile-x86 (curry f Heap) ≡
...
--
-- Given: prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
...
thunk-prog-structure : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      thunk-after-i0 = thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷
...
thunk-prog-structure {A} {B} {C} f prefix suffix =
  let ccs = curry-closure-setup f
      ...
      -- curry-structure: compile-x86 (curry f Heap) = ccs ++ cts ++ code-f ++ cta
      -- cts = i0 ∷ i1 ∷ i2 ∷ i3 ∷ i4 ∷ i5 ∷ i6 ∷ []

      -- Step 1: Expand prog
      step1 : prefix ++ compile-x86 (curry f Heap) ++ suffix ≡
      ...
      cts-expand : cts ≡ thunk-i0 ∷ thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ []
      cts-expand = refl

      step3 : (prefix ++ ccs) ++ (cts ++ code-f ++ cta) ++ suffix ≡
              (prefix ++ ccs) ++ thunk-i0 ∷ ((thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ []) ++ code-f ++ cta) ++ suffix
      step3 = refl

      -- Step 4: Simplify the tail
      step4 : (prefix ++ ccs) ++ thunk-i0 ∷ ((thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ []) ++ code-f ++ cta) ++ suffix ≡
              (prefix ++ ccs) ++ thunk-i0 ∷ (thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ code-f ++ cta ++ suffix)
      step4 = cong ((prefix ++ ccs) ++_)
                   (cong (thunk-i0 ∷_)
                         (trans (++-assoc (thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ []) (code-f ++ cta) suffix)
                                (cong (λ xs → thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ xs)
...
thunk-prog-structure-i1 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      thunk-after-i1 = thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷
                       compile-x86 f ++ curry-tail f ++ suffix
  in
...
thunk-prog-structure-i1 {A} {B} {C} f prefix suffix =
  let base = thunk-prog-structure f prefix suffix
      ccs = curry-closure-setup f
      rest = thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
...
thunk-prog-structure-i2 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      thunk-after-i2 = thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷
                       compile-x86 f ++ curry-tail f ++ suffix
  in
...
thunk-prog-structure-i2 {A} {B} {C} f prefix suffix =
  let base = thunk-prog-structure-i1 f prefix suffix
      ccs = curry-closure-setup f
      prefix-i1 = prefix ++ ccs ++ thunk-i0 ∷ []
      rest = thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
...
thunk-prog-structure-i3 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      thunk-after-i3 = thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
  in
...
thunk-prog-structure-i3 {A} {B} {C} f prefix suffix =
  let base = thunk-prog-structure-i2 f prefix suffix
      ccs = curry-closure-setup f
      prefix-i2 = prefix ++ ccs ++ thunk-i0 ∷ thunk-i1 ∷ []
      rest = thunk-i3 ∷ thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
...
thunk-prog-structure-i4 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      thunk-after-i4 = thunk-i5 ∷ thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
  in
...
thunk-prog-structure-i4 {A} {B} {C} f prefix suffix =
  let base = thunk-prog-structure-i3 f prefix suffix
      ccs = curry-closure-setup f
      prefix-i3 = prefix ++ ccs ++ thunk-i0 ∷ thunk-i1 ∷ thunk-i2 ∷ []
      rest = thunk-i4 ∷ thunk-i5 ∷ thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
...
thunk-prog-structure-i5 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      thunk-after-i5 = thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
  in
...
thunk-prog-structure-i5 {A} {B} {C} f prefix suffix =
  let base = thunk-prog-structure-i4 f prefix suffix
      ccs = curry-closure-setup f
      prefix-i4 = prefix ++ ccs ++ thunk-i0 ∷ thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ []
      rest = thunk-i5 ∷ thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
...
thunk-prog-structure-i6 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      thunk-after-i6 = compile-x86 f ++ curry-tail f ++ suffix
  in
...
thunk-prog-structure-i6 {A} {B} {C} f prefix suffix =
  let base = thunk-prog-structure-i5 f prefix suffix
      ccs = curry-closure-setup f
      prefix-i5 = prefix ++ ccs ++ thunk-i0 ∷ thunk-i1 ∷ thunk-i2 ∷ thunk-i3 ∷ thunk-i4 ∷ []
      rest = thunk-i6 ∷ compile-x86 f ++ curry-tail f ++ suffix
...
--
-- With frame pointer handling, the ret instruction comes after:
-- - compile-x86 f
-- - mov rsp, rbp (cleanup)
-- - pop rbp (cleanup)
-- Position: length prefix + 6 + 7 + compile-length f + 2 = length prefix + 15 + compile-length f
------------------------------------------------------------------------
...
thunk-prog-structure-ret : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      -- prefix up to ret = prefix ++ closure-setup ++ thunk-setup ++ code-f ++ mov rsp rbp ++ pop rbp
      prefix-ret = prefix ++ ccs ++ curry-thunk-setup ++ compile-x86 f ++
                   mov (reg rsp) (reg rbp) ∷ pop rbp ∷ []
      thunk-after-ret = label (16 +ℕ len-f) ∷ suffix
  in
...
thunk-prog-structure-ret {A} {B} {C} f prefix suffix =
  let ccs = curry-closure-setup f
      ...
      -- curry-structure: compile-x86 (curry f Heap) = ccs ++ cts ++ code-f ++ cta
      -- cta = mov rsp rbp ∷ pop rbp ∷ ret ∷ label (16+len-f) ∷ []
      ...
      -- cta ++ suffix = mov rsp rbp ∷ pop rbp ∷ ret ∷ label (16+len-f) ∷ suffix
      -- We need to split out the first two instructions to reach ret
      cta-expand : cta ++ suffix ≡ mov (reg rsp) (reg rbp) ∷ pop rbp ∷ ret ∷ label (16 +ℕ len-f) ∷ suffix
      cta-expand = refl

      -- Split the cleanup instructions
      cleanup-split : mov (reg rsp) (reg rbp) ∷ pop rbp ∷ ret ∷ label (16 +ℕ len-f) ∷ suffix ≡
                      (mov (reg rsp) (reg rbp) ∷ pop rbp ∷ []) ++ (ret ∷ label (16 +ℕ len-f) ∷ suffix)
      cleanup-split = refl
      ...
      step5a : (prefix ++ ccs ++ cts ++ code-f) ++ (cta ++ suffix) ≡
               ((prefix ++ ccs ++ cts ++ code-f) ++ (mov (reg rsp) (reg rbp) ∷ pop rbp ∷ [])) ++ (ret ∷ label (16 +ℕ len-f) ∷ suffix)
      step5a = trans (cong ((prefix ++ ccs ++ cts ++ code-f) ++_) cta-expand)
                     (trans (cong ((prefix ++ ccs ++ cts ++ code-f) ++_) cleanup-split)
                            (sym (++-assoc (prefix ++ ccs ++ cts ++ code-f) (mov (reg rsp) (reg rbp) ∷ pop rbp ∷ [])
                                           (ret ∷ label (16 +ℕ len-f) ∷ suffix))))

      -- Now we need to re-associate (prefix ++ (ccs ++ (cts ++ code-f))) ++ cleanup
      -- to prefix ++ (ccs ++ (cts ++ (code-f ++ cleanup)))
      cleanup = mov (reg rsp) (reg rbp) ∷ pop rbp ∷ []
      ...
      step5b : ((prefix ++ ccs ++ cts ++ code-f) ++ cleanup) ++ (ret ∷ label (16 +ℕ len-f) ∷ suffix) ≡
               (prefix ++ ccs ++ cts ++ code-f ++ mov (reg rsp) (reg rbp) ∷ pop rbp ∷ []) ++ (ret ∷ label (16 +ℕ len-f) ∷ suffix)
      step5b = cong (_++ (ret ∷ label (16 +ℕ len-f) ∷ suffix)) all-assoc
...
curry-prefix-length {A} {B} {C} f =
  let step1 : length (curry-closure-setup f ++ curry-thunk-setup ++ compile-x86 f) ≡
      ...
      step2 : 6 +ℕ length (curry-thunk-setup ++ compile-x86 f) ≡
              6 +ℕ (7 +ℕ length (compile-x86 f))
      step2 = cong (6 +ℕ_) (List-length-++ curry-thunk-setup {compile-x86 f})

      -- 6 + (7 + n) = 13 + n by computation
      step3 : 6 +ℕ (7 +ℕ length (compile-x86 f)) ≡ 13 +ℕ length (compile-x86 f)
      step3 = refl

      step4 : 13 +ℕ length (compile-x86 f) ≡ 13 +ℕ compile-length f
      step4 = cong (13 +ℕ_) (compile-length-correct f)
...
prefix-ret-length {A} {B} {C} f prefix =
  ...
  ≡⟨ cong (length prefix +ℕ_) (List-length-++ (curry-closure-setup f ++ curry-thunk-setup ++ compile-x86 f)) ⟩
    length prefix +ℕ (length (curry-closure-setup f ++ curry-thunk-setup ++ compile-x86 f) +ℕ 2)
  ≡⟨ cong (λ x → length prefix +ℕ (x +ℕ 2)) (curry-prefix-length {A} {B} {C} f) ⟩
    length prefix +ℕ ((13 +ℕ compile-length f) +ℕ 2)
  ≡⟨ cong (length prefix +ℕ_) (+-assoc 13 (compile-length f) 2) ⟩
    length prefix +ℕ (13 +ℕ (compile-length f +ℕ 2))
  ≡⟨ cong (λ x → length prefix +ℕ (13 +ℕ x)) (Data.Nat.Properties.+-comm (compile-length f) 2) ⟩
    length prefix +ℕ (13 +ℕ (2 +ℕ compile-length f))
  ≡⟨ cong (length prefix +ℕ_) (sym (+-assoc 13 2 (compile-length f))) ⟩
    length prefix +ℕ (15 +ℕ compile-length f)
  ≡⟨ sym (+-assoc (length prefix) 15 (compile-length f)) ⟩
    (length prefix +ℕ 15) +ℕ compile-length f
  ∎
...
fetch-ret {A} {B} {C} f prefix suffix =
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      prefix-ret = prefix ++ ccs ++ curry-thunk-setup ++ compile-x86 f ++
                   mov (reg rsp) (reg rbp) ∷ pop rbp ∷ []
      thunk-after-ret = label (16 +ℕ compile-length f) ∷ suffix

      prog-eq : prog ≡ prefix-ret ++ ret ∷ thunk-after-ret
      prog-eq = thunk-prog-structure-ret f prefix suffix

      len-eq : length prefix-ret ≡ length prefix +ℕ 15 +ℕ compile-length f
...
thunk-prog-structure-cleanup-i0 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      after-cleanup-i0 = pop rbp ∷ ret ∷ label (16 +ℕ compile-length f) ∷ suffix
  in
...
thunk-prog-structure-cleanup-i0 {A} {B} {C} f prefix suffix =
  let ccs = curry-closure-setup f
      ...
      -- Start: prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      --            = prefix ++ (ccs ++ cts ++ code-f ++ cta) ++ suffix
      step1 : prefix ++ compile-x86 (curry f Heap) ++ suffix ≡
              prefix ++ (ccs ++ cts ++ code-f ++ cta) ++ suffix
      step1 = cong (λ x → prefix ++ x ++ suffix) (curry-structure f)
      ...
      -- cta ++ suffix = mov rsp rbp ∷ pop rbp ∷ ret ∷ label ∷ suffix
      cta-expand : cta ++ suffix ≡ cleanup-i0 ∷ pop rbp ∷ ret ∷ label (16 +ℕ len-f) ∷ suffix
      cta-expand = refl

      step5 : (prefix ++ ccs ++ cts ++ code-f) ++ (cta ++ suffix) ≡
              (prefix ++ ccs ++ cts ++ code-f) ++ cleanup-i0 ∷ pop rbp ∷ ret ∷ label (16 +ℕ len-f) ∷ suffix
...
prefix-cleanup-length {A} {B} {C} f prefix =
  ...
  ≡⟨ cong (length prefix +ℕ_) (curry-prefix-length f) ⟩
    length prefix +ℕ (13 +ℕ compile-length f)
  ≡⟨ sym (+-assoc (length prefix) 13 (compile-length f)) ⟩
    (length prefix +ℕ 13) +ℕ compile-length f
  ∎
...
fetch-cleanup-i0 {A} {B} {C} f prefix suffix =
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      len-eq : length prefix-cleanup ≡ length prefix +ℕ 13 +ℕ compile-length f
...
thunk-prog-structure-cleanup-i1 : ∀ {A B C} (f : IR (A * B) C) (prefix suffix : Program) →
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      after-cleanup-i1 = ret ∷ label (16 +ℕ compile-length f) ∷ suffix
  in
...
thunk-prog-structure-cleanup-i1 {A} {B} {C} f prefix suffix =
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      -- after-cleanup-i0 = pop rbp ∷ ret ∷ label ∷ suffix = cleanup-i1 ∷ (ret ∷ label ∷ suffix)
...
prefix-cleanup-i1-length {A} {B} {C} f prefix =
  let ccs = curry-closure-setup f
      ...
      -- Length of tail = 6 + 7 + len-f + 1 = 14 + len-f
      tail-len : length tail ≡ 14 +ℕ len-f
      tail-len =
        begin
          length (ccs ++ cts ++ code-f ++ cleanup-i0 ∷ [])
        ≡⟨ List-length-++ ccs {cts ++ code-f ++ cleanup-i0 ∷ []} ⟩
          6 +ℕ length (cts ++ code-f ++ cleanup-i0 ∷ [])
        ≡⟨ cong (6 +ℕ_) (List-length-++ cts {code-f ++ cleanup-i0 ∷ []}) ⟩
          6 +ℕ (7 +ℕ length (code-f ++ cleanup-i0 ∷ []))
        ≡⟨ cong (λ x → 6 +ℕ (7 +ℕ x)) (List-length-++ code-f {cleanup-i0 ∷ []}) ⟩
          6 +ℕ (7 +ℕ (length code-f +ℕ 1))
        ≡⟨ cong (λ x → 6 +ℕ (7 +ℕ (x +ℕ 1))) (compile-length-correct f) ⟩
          6 +ℕ (7 +ℕ (len-f +ℕ 1))
        ≡⟨⟩  -- Computation: 6 + 7 = 13
          13 +ℕ (len-f +ℕ 1)
        ≡⟨ cong (13 +ℕ_) (Data.Nat.Properties.+-comm len-f 1) ⟩
          13 +ℕ (1 +ℕ len-f)
        ≡⟨⟩  -- Computation: 13 + 1 = 14
          14 +ℕ len-f
        ∎
...
fetch-cleanup-i1 {A} {B} {C} f prefix suffix =
  let prog = prefix ++ compile-x86 (curry f Heap) ++ suffix
      ...
      len-eq : length prefix-cleanup-i1 ≡ length prefix +ℕ 14 +ℕ compile-length f
