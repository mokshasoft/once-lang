...
assemble-compose-result {A} {B} {C} f g prefix suffix x s s1 s2 s3 r1 tr r3 s2-eq = record
  ...
  ; ir-capacity = IRStarResult.ir-capacity r3
...
assemble-compose-result-v {A} {B} {C} f g prefix suffix x s s1 s2 s3 r1 tr r3 s2-eq = record
  { ir-star = star-all
  ; ir-halted = h3
  ; ir-pc = pc3
  ...
  ; ir-mem-heap = mem-heap-3
  ; ir-stack-inv = stack-inv-3
  ; ir-capacity = IRStarResultV.ir-capacity r3
