...
run-inr-star-v {A} {B} prefix suffix x s h-false pc-eq input-valid stack-inv rsp-sufficient rbp-inv addr-neq-1 addr-neq-2 =
    s4 , record
    { ir-star = star-proof
    ; ir-halted = h4
    ; ir-pc = pc4
  ...
  where
    ...
    mem-at-0-preserved : readMem (memory s4) 0 ≡ readMem (memory s) 0
    mem-at-0-preserved =
      let cap = rsp-bound-to-capacity 2 s (rsp-in-stack-after-stack-op s) rsp-sufficient
          (tag-addr-in-stack , val-addr-in-stack) = alloc-2-slots-addrs-in-stack s cap
          after-tag-write = stackAddr-write-preserves-zero (memory s1) new-rsp 1 tag-addr-in-stack
          after-val-write = stackAddr-write-preserves-zero (memory s2) (new-rsp +ℕ slot-size) orig-rdi val-addr-in-stack
      in trans after-val-write after-tag-write
...
run-inr-star-v-auto {A} {B} prefix suffix x s h-false pc-eq input-valid stack-inv cap rbp-inv =
  run-inr-star-v prefix suffix x s h-false pc-eq input-valid stack-inv (StackCapacity.rsp-sufficient cap) rbp-inv
