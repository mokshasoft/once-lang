...
exec-pair-final : ∀ {A B C} (f : IR C A) (g : IR C B)
  ...
  let ctx = make-pair-context f g prefix suffix in
  let open PairContext ctx in
  -- Preconditions from previous phases
  halted s3 ≡ false →
...
exec-pair-final {A} {B} {C} f g prefix suffix s s3 h3 pc3 = record
    ...
    where
      ...
      -- ========== State definitions ==========
      -- Instruction 1: mov [r15+8], rax - store g result
      s4 : State
      s4 = record s3 { memory = writeMem (memory s3) (readReg (regs s3) r15 +ℕ 8) (readReg (regs s3) rax)
                     ; pc = pc s3 +ℕ 1 }

      -- Instruction 2: mov rax, r15 - return pair pointer
      s5 : State
      s5 = record s4 { regs = writeReg (regs s4) rax (readReg (regs s4) r15)
                     ; pc = pc s4 +ℕ 1 }

      -- Instruction 3: mov rsp, rbp - restore stack pointer
      s6 : State
      s6 = record s5 { regs = writeReg (regs s5) rsp (readReg (regs s5) rbp)
                     ; pc = pc s5 +ℕ 1 }

      -- For pop instructions, postulate memory preconditions
      postulate
      ...
      -- Instruction 4: pop rbp
      s7 : State
      s7 = record s6 { regs = writeReg (writeReg (regs s6) rbp (readReg (regs s) rbp)) rsp (readReg (regs s6) rsp +ℕ 8)
                     ; pc = pc s6 +ℕ 1 }

      -- Instruction 5: pop r15
      s8 : State
      s8 = record s7 { regs = writeReg (writeReg (regs s7) r15 (readReg (regs s) r15)) rsp (readReg (regs s7) rsp +ℕ 8)
                     ; pc = pc s7 +ℕ 1 }

      -- Instruction 6: pop r14
      s9 : State
      s9 = record s8 { regs = writeReg (writeReg (regs s8) r14 (readReg (regs s) r14)) rsp (readReg (regs s8) rsp +ℕ 8)
                     ; pc = pc s8 +ℕ 1 }
      ...
      fetch4 = subst (λ n → fetch prog-final n ≡ just store-g-instr)
                     (sym pc3) (fetch-at-prefix-end prefix-final store-g-instr _)
      ...
      fetch5 = subst₂ (λ p n → fetch p n ≡ just return-pair-instr)
                      (sym prog-eq-i2) (trans len-prefix-i2 (sym pc4))
                      (fetch-at-prefix-end (prefix-final ++ store-g-instr ∷ []) return-pair-instr _)
      ...
      fetch6 = subst₂ (λ p n → fetch p n ≡ just restore-rsp)
                      (sym prog-eq-i3) (trans len-prefix-i3 (sym pc5))
                      (fetch-at-prefix-end (prefix-final ++ store-g-instr ∷ return-pair-instr ∷ []) restore-rsp _)
      ...
      fetch7 = subst₂ (λ p n → fetch p n ≡ just final-pop-rbp)
                      (sym prog-eq-i4) (trans len-prefix-i4 (sym pc6))
                      (fetch-at-prefix-end (prefix-final ++ store-g-instr ∷ return-pair-instr ∷ restore-rsp ∷ []) final-pop-rbp _)
      ...
      fetch8 = subst₂ (λ p n → fetch p n ≡ just final-pop-r15)
                      (sym prog-eq-i5) (trans len-prefix-i5 (sym pc7))
                      (fetch-at-prefix-end (prefix-final ++ store-g-instr ∷ return-pair-instr ∷ restore-rsp ∷ final-pop-rbp ∷ []) final-pop-r15 _)
      ...
      fetch9 = subst₂ (λ p n → fetch p n ≡ just final-pop-r14)
                      (sym prog-eq-i6) (trans len-prefix-i6 (sym pc8))
                      (fetch-at-prefix-end (prefix-final ++ store-g-instr ∷ return-pair-instr ∷ restore-rsp ∷ final-pop-rbp ∷ final-pop-r15 ∷ []) final-pop-r14 suffix)

      -- ========== Step proofs ==========
      step4 : step prog-final s3 ≡ just s4
      step4 = trans (step-exec prog-final s3 store-g-instr h3 fetch4)
                    (execMov-mem-disp-reg prog-final s3 r15 rax 8)

      step5 : step prog-final s4 ≡ just s5
      step5 = trans (step-exec prog-final s4 return-pair-instr h4 fetch5)
                    (execMov-reg-reg s4 rax r15)

      step6 : step prog-final s5 ≡ just s6
      step6 = trans (step-exec prog-final s5 restore-rsp h5 fetch6)
                    (execMov-reg-reg s5 rsp rbp)

      -- Pop instruction step proofs
      step7 : step prog-final s6 ≡ just s7
      step7 = trans (step-exec prog-final s6 final-pop-rbp h6 fetch7)
                    (execPop prog-final s6 rbp (readReg (regs s) rbp) pop-rbp-mem)
      ...
      pop-r15-mem' = subst (λ addr → readMem (memory s7) addr ≡ just (readReg (regs s) r15))
                           (sym rsp-s7) pop-r15-mem

      step8 : step prog-final s7 ≡ just s8
      step8 = trans (step-exec prog-final s7 final-pop-r15 h7 fetch8)
                    (execPop prog-final s7 r15 (readReg (regs s) r15) pop-r15-mem')

      rsp-s8 : readReg (regs s8) rsp ≡ readReg (regs s6) rsp +ℕ 16
      rsp-s8 = trans (readReg-writeReg-same (writeReg (regs s7) r15 (readReg (regs s) r15)) rsp (readReg (regs s7) rsp +ℕ 8))
               (trans (cong (_+ℕ 8) rsp-s7) (+-assoc (readReg (regs s6) rsp) 8 8))

      pop-r14-mem' : readMem (memory s8) (readReg (regs s8) rsp) ≡ just (readReg (regs s) r14)
      pop-r14-mem' = subst (λ addr → readMem (memory s8) addr ≡ just (readReg (regs s) r14))
                           (sym rsp-s8) pop-r14-mem

      step9 : step prog-final s8 ≡ just s9
      step9 = trans (step-exec prog-final s8 final-pop-r14 h8 fetch9)
                    (execPop prog-final s8 r14 (readReg (regs s) r14) pop-r14-mem')

      -- ========== Combine all 6 steps ==========
      exec-6-final : exec 6 prog-final s3 ≡ just s9
      exec-6-final = exec-six-steps-nonhalt prog-final s3 s4 s5 s6 s7 s8 s9
