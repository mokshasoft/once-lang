...
run-inl-star-v : ∀ {A B} (prefix suffix : Program) (x : ⟦ A ⟧) (s : State) →
  ...
  RbpInvariant s →
  let prog = prefix ++ compile-instr (inl {A} {B}) ++ suffix
...
run-inl-star-v {A} {B} prefix suffix x s h-false pc-eq input-valid stack-inv cap rbp-inv =
  ...
  where
    ...
    -- The program
    prog : Program
    prog = prefix ++ compile-instr (inl {A} {B}) ++ suffix
    ...
    -- Preserve input validity using Ownership model - see input-valid-preserved below
    -- result-valid is defined after input-valid-preserved (line ~470)
