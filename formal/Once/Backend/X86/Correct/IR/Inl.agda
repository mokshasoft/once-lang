...
run-inl-star {A} {B} prefix suffix x s h-false pc-eq rdi-eq stack-inv rsp-sufficient rbp-inv =
  ...
  where
    ...
    -- CLEAN DERIVATION: Use proven capacity-after-alloc-2-slots
...
run-inl-star-v {A} {B} prefix suffix x s h-false pc-eq input-valid stack-inv rsp-sufficient rbp-inv addr-neq-1 addr-neq-2 =
    s4 , record
    { ir-star = star-proof
    ; ir-halted = h4
    ; ir-pc = pc4
  ...
  where
    ...
    -- Capacity derived via region-based proof
    rsp-change : readReg (regs s4) rsp ≡ readReg (regs s) rsp ∸ slots 2
    rsp-change = rsp-s4
