...
--
-- Full end-to-end trace proof for curry-apply.
-- Proves execution of apply ∘ ⟨curry fst, id⟩ on unit input
-- without using any postulates for the execution itself.
--
-- This module was extracted from Once.Backend.X86.Correct to improve
-- compilation time. It is ~1170 lines of validation code.
------------------------------------------------------------------------
...
-- Import RegisterLemmas (for readReg-writeReg-same)
open import Once.Backend.X86.Correct.RegisterLemmas
  using (readReg-writeReg-same)

-- Import FetchStep (for step-exec and exec* lemmas)
open import Once.Backend.X86.Correct.FetchStep
  using (step-exec;
         execPush-reg; execMov-reg-reg; execMov-mem-base-reg;
         execMov-mem-disp-reg; execMov-reg-mem;
         execSub-reg-imm; execLea; execJmp; execLabel;
         execCall-reg; execPop; execRet)

-- Import ExecLemmas (for exec-step-helper, step-implies-not-halted, exec-one-step)
open import Once.Backend.X86.Correct.ExecLemmas
  using (exec-step-helper; step-implies-not-halted; exec-one-step)
...
------------------------------------------------------------------------
-- Bridge postulate: exec extend for halted states
--
-- This is the same postulate from Correct.agda.
-- If exec n reaches halted s', exec (n+m) also gives s'.
------------------------------------------------------------------------
postulate
...
------------------------------------------------------------------------
-- E2E Trace: Full proof for apply ∘ ⟨curry fst, id⟩
--
-- Execution trace:
--   0-7: Pair setup (compile-x86 ⟨...⟩ prologue)
--   7-12: Curry closure creation
--   12→20: jmp skips thunk (instructions 13-19)
--   20-29: Complete pairing + epilogue
--   30: Transfer to apply
--   31-36: Apply setup + call
--   36→13: call jumps to thunk
--   13-19: Thunk execution + ret (halt)
--
