...
x86-compose-combine {A} {B} {C} f g prefix suffix x s s₁ s₂ s₃ orig-cap rsp-transfer g-corr transfer-star f-corr = record
  ...
  where
    ...
    cap-in-split : StackCapacity s (ir-rsp-delta (f ∘ g) + ir-output-capacity (f ∘ g))
    cap-in-split = subst (StackCapacity s) (ir-requirement-split (f ∘ g)) orig-cap

    compose-capacity : StackCapacity s₃ (ir-output-capacity (f ∘ g))
    compose-capacity = capacity-after-delta s s₃ (ir-rsp-delta (f ∘ g)) (ir-output-capacity (f ∘ g)) cap-in-split rsp-compose
