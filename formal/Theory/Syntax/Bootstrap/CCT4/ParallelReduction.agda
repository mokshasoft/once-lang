------------------------------------------------------------------------
-- Theory.Syntax.Bootstrap.CCT4.ParallelReduction
--
-- Parallel reduction on Takahashi's CCT4 syntax, plus bridges.
-- Extends CCT3's pattern with νOut / νIn / ana constructors.
------------------------------------------------------------------------

{-# OPTIONS --no-positivity-check #-}

module Theory.Syntax.Bootstrap.CCT4.ParallelReduction where

open import Theory.Syntax.Bootstrap.CCT4
...
data _⟹_ : ∀ {A B} → Term A B → Term A B → Set where
  ...
  ⟹-ana  : ∀ {F A} {coalg coalg' : Term A (F A)} →
  ...
  ⟹-νout-νin : ∀ {F} → (νOut {F} ∘ νIn  {F}) ⟹ id
  ⟹-νin-νout : ∀ {F} → (νIn  {F} ∘ νOut {F}) ⟹ id
  ⟹-ana-β    : ∀ {F A} {coalg coalg' : Term A (F A)} →
               coalg ⟹ coalg' →
               (νOut {F} ∘ ana {F} coalg) ⟹
...
⟹→⟶* (⟹-cata-β palg)   =
  ⟶*-trans (⟶*-∘-l _ (⟶*-cata (⟹→⟶* palg)))
           (cata-β ∷ done)
⟹→⟶* ⟹-νout-νin         = νout-νin ∷ done
⟹→⟶* ⟹-νin-νout         = νin-νout ∷ done
⟹→⟶* (⟹-ana-β pc)       =
  ⟶*-trans (⟶*-∘-r _ (⟶*-ana (⟹→⟶* pc)))
           (ana-β ∷ done)
