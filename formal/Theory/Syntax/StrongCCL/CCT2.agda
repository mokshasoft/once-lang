------------------------------------------------------------------------
-- Theory.Syntax.StrongCCL.CCT2
--
-- Bicartesian Closed Category (BCC) at the CCT2 level, combining:
--   - term algebra + CCTB/CCT1 rules from Hardin 1989 strong CCL
--   - coproduct rules in the categorical-combinator style of
--     Seely 1984, "Locally Cartesian Closed Categories and Type Theory"
--
-- The Seely1984 layer contributes 6 CCT2 rules:
--   β:  case-inl, case-inr, eta-case
--   s:  eta-case-gen, case-dist, initial-unique
--
-- Same term algebra as Hardin1989 (Curien combinators) extended with
-- Void, ⊎, initial, inl, inr, [_,_]. Within the same "categorical
-- lineage" as Hardin — seamlessly composable because both use the
-- same term algebra.
--
-- The rules are parameterized ONCE in Theory.Syntax.StrongCCL.BaseRules.CCT2
-- and instantiated here.
--
-- This module builds:
--   (a) the Systems.CCT2.CCT2Structure instance whose `_≈_` is the
...
module Theory.Syntax.StrongCCL.CCT2 where
...
------------------------------------------------------------------------
-- Terms (Curien combinators + exponentials + coproducts)
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- β/η/s rules — inherited from PARAMETERIZED modules at their
-- origin levels.
------------------------------------------------------------------------
...
------------------------------------------------------------------------
-- Union of β-rules across all levels.
------------------------------------------------------------------------
...
module βη-Closure =
  CCT2-Close Ty _×_ _⇒_ _⊎_ Term _∘_ ⟨_,_⟩ curry [_,_] _⟶βη-rules_
...
-- Initial-unique holds between any two morphisms f, g : Void → A:
-- both reduce to initial by the initial-unique structural rule, so
-- they are joinable, hence ≈-equivalent via symmetry.
initial-unique-≈ : ∀ {A} {f g : Term Void A} → f ≈ g
initial-unique-≈ {A} {f} {g} =
