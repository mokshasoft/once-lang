...
### ⬜ THE LEVER: SHORTEN THE TELESCOPE, DO NOT SHRINK ITS CONTENTS

[[agda-cost-is-context-depth]] measured that big context TYPES and long
lookups are **not** the mechanism — depth is. So bundling several slots
into one trades a dimension the cost does not care about for the one it
does. **Cutting 4 slots off the deep modules predicts 8.4× time and 2.4×
memory.**

★ The obvious candidates are the FORDING slots. `_⊢ty_`/`_⊢_∷_`'s index
has FIVE components (`Judge/Ielim`'s own header), so a row can carry up
to five `El (⌜Id⌝ …)` slots — and the Knot already knows how to collapse
those: pair the index, exactly as `pair sort depth` does and as
`tmp/VecASpike` did with `Σ' U (El ⌜Nat⌝)`.

⚠ ALL 58 MODULES ARE GENERATED by `tools/gen-knot.py`, so this is ONE
generator change for the whole corpus — and it is testable on ONE module
first.

⚠ NOT ATTEMPTED. And the two depth-15 points differ by 1.8× (56.2 s vs
31.7 s), so depth is not the only variable; the exponent is solid, the
per-module prediction is not.
