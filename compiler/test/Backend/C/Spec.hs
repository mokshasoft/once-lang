...
allocationTests = testGroup "allocation"
  [ testCase "all allocation strategies produce same output" $ do
      ...
      results <- forM strategies $ \allocFlag -> do
        ...
        (compilerCode, _, compilerErr) <- runOnce $
          ["build", "--exe", "--interp", "../interpretations/linux"] ++ allocArg ++ [dir ++ "/hello.once", "-o", dir ++ "/hello"]
  ...
  , testCase "function allocation annotation overrides --alloc flag" $ do
      ...
      (compilerCode, _, compilerErr) <- runOnce
        ["build", "--exe", "--interp", "../interpretations/linux", "--alloc", "heap", dir ++ "/hello.once", "-o", dir ++ "/hello"]
  ...
  , testCase "strategies produce different codegen" $ do
      ...
      -- Generate with const
      (code1, _, _) <- runOnce
        ["build", "--exe", "--interp", "../interpretations/linux", "--alloc", "const", dir ++ "/hello.once", "-o", dir ++ "/const"]
      ...
      -- Generate with stack
      (code2, _, _) <- runOnce
        ["build", "--exe", "--interp", "../interpretations/linux", "--alloc", "stack", dir ++ "/hello.once", "-o", dir ++ "/stack"]
  ...
  , testCase "heap allocation uses once_heap_string" $ do
      ...
      (exitCode, _, _) <- runOnce
        ["build", "--exe", "--interp", "../interpretations/linux", dir ++ "/hello.once", "-o", dir ++ "/heap"]
