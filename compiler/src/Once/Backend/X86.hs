...
-- | Environment for local variables: maps variable names to stack offsets
-- Offsets are negative (relative to %rbp)
type LocalEnv = M.Map Name Int

-- | Code generation state: environment + next stack offset + label counter
data GenState = GenState
  { gsEnv        :: LocalEnv   -- ^ Variable -> stack offset mapping
  , gsNextOffset :: Int        -- ^ Next available stack offset (grows negative)
  , gsLabelCtr   :: Int        -- ^ Label counter for unique labels
  }
...
generateGAS name ty ir =
  let initState = GenState M.empty (-8) 0  -- Start at -8(%rbp) for first local
      (code, finalState) = generateIRGas' ir "%rdi" "%rax" initState
      -- Calculate total stack space needed (align to 16 bytes)
      stackSize = ((negate (gsNextOffset finalState) + 15) `div` 16) * 16
  in T.unlines $
    [ "# Generated by Once compiler"
    , "# GAS syntax (AT&T)"
    , ""
    ...
    gasPrologueWithStack stackSize ++
    [ "    # Function body" ] ++
    code ++
    gasEpilogueWithStack stackSize

-- | GAS function prologue (legacy, unused)
gasPrologue :: [Text]
gasPrologue =
  [ "    # Prologue"
  , "    pushq %rbp"
...
-- | GAS function prologue with stack allocation
gasPrologueWithStack :: Int -> [Text]
gasPrologueWithStack stackSize =
  [ "    # Prologue"
  , "    pushq %rbp"
  , "    movq %rsp, %rbp"
  ...
  (if stackSize > 0
   then ["    subq $" <> tshow stackSize <> ", %rsp"]
   else []) ++
  [ "" ]

-- | GAS function epilogue (legacy, unused)
gasEpilogue :: [Text]
gasEpilogue =
  [ ""
  , "    # Epilogue"
...
-- | GAS function epilogue with stack cleanup
gasEpilogueWithStack :: Int -> [Text]
gasEpilogueWithStack stackSize =
  [ "" , "    # Epilogue" ] ++
  (if stackSize > 0
   then ["    addq $" <> tshow stackSize <> ", %rsp"]
   else []) ++
  [ "    popq %r15"
  , "    popq %r14"
  , "    popq %r13"
...
------------------------------------------------------------------------
-- State-aware GAS generation (with proper local variable handling)
------------------------------------------------------------------------

-- | Generate GAS instructions for IR with state tracking
-- Returns (code, updated state)
generateIRGas' :: IR -> Text -> Text -> GenState -> ([Text], GenState)
generateIRGas' ir inputReg outputReg st = case ir of
  Id _ ->
    if inputReg == outputReg
      then (["    # id (no-op)"], st)
      else (["    movq " <> inputReg <> ", " <> outputReg], st)
  ...
  Pair f g ->
    let (code1, st1) = generateIRGas' f inputReg "%r15" st
        (code2, st2) = generateIRGas' g inputReg "%r15" st1
    in ([ "    # pair"
        , "    subq $16, %rsp"
        , "    movq %rsp, %r14"
        ] ++
  ...
  Compose g f ->
    let (code1, st1) = generateIRGas' f inputReg "%r15" st
        (code2, st2) = generateIRGas' g "%r15" outputReg st1
    in ([ "    # compose" ] ++ code1 ++ code2, st2)
  ...
  Case l r ->
    let lbl = tshow (gsLabelCtr st)
        lblElse = ".case_else_" <> lbl
        lblEnd = ".case_end_" <> lbl
        st' = st { gsLabelCtr = gsLabelCtr st + 1 }
        (codeL, st1) = generateIRGas' l "%r15" outputReg st'
        (codeR, st2) = generateIRGas' r "%r15" outputReg st1
    in ([ "    # case"
        , "    movq (" <> inputReg <> "), %r15"
        , "    cmpq $0, %r15"
        , "    jne " <> lblElse
  ...
  LocalVar n ->
    case M.lookup n (gsEnv st) of
      Just offset ->
        ([ "    # local var " <> n
         , "    movq " <> tshow offset <> "(%rbp), " <> outputReg
         ], st)
      Nothing ->
        -- Fallback: treat as global variable (shouldn't happen)
        ([ "    # ERROR: unknown local var " <> n
         , "    movq $0, " <> outputReg
         ], st)
  ...
  Let x e1 e2 ->
    -- Allocate stack slot for x
    let offset = gsNextOffset st
        env' = M.insert x offset (gsEnv st)
        st' = st { gsEnv = env', gsNextOffset = offset - 8 }
        -- Generate code for e1, result goes to %rax
        (code1, st1) = generateIRGas' e1 inputReg "%rax" st'
        -- Store result in stack slot
        storeCode = [ "    movq %rax, " <> tshow offset <> "(%rbp)" ]
        -- Generate code for e2 with updated environment
        (code2, st2) = generateIRGas' e2 inputReg outputReg st1
    in ([ "    # let " <> x ] ++
        code1 ++
        storeCode ++
        code2, st2)
