...
--
-- For IR containing primitives, we use Haskell-based code generation
-- that calls into interpretation functions (once_<primitive>).
--
-- Supported targets:
--   - AArch64 (ARM64)
--   - x86-64
...
containsPrimitives ir = case ir of
  ...
  H.Terminal _ -> False
  H.Initial _ -> False
  H.Curry f -> containsPrimitives f
  H.Apply _ _ -> False
  H.Fold _ -> False
  H.Unfold _ -> False
  H.Prim {} -> True
