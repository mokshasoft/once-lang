                ...
                Right ir -> do
                  ...
                  case mode of
                    ...
                    Executable -> do
                      ...
                      -- Load interpretation C code if provided
                      interpCode <- case buildInterp opts of
                        Nothing -> pure ""
                        Just interpPath -> TIO.readFile (interpPath </> "syscalls.c")

                      let sourcePath = outputBase ++ ".c"
                          source' = generateExecutable name ty optimizedIR primitives interpCode
                      TIO.writeFile sourcePath source'
...
generateExecutable :: Text -> Type -> Once.IR.IR -> [(Text, Type)] -> Text -> Text
generateExecutable name ty ir primitives interpCode = T.unlines
