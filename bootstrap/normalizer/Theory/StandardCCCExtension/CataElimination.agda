...
-- Corollary: The full reduction terminates
cata-terminates-⟶* : ∀ {A B} (t : Term A B) {X} (alg : Term (⟦ TermF ⟧F X) X) →
                     ∃[ r ] ((cata TermF alg ∘ encode t) ⟶* r)
cata-terminates-⟶* t alg with cata-terminates t alg
... | r , red = r , ⟶*cata→⟶* red

------------------------------------------------------------------------
-- Key Property: Cata Reduction Depth Bound
--
-- The number of cata-beta reductions is bounded by the encoding depth.
-- This is important for confluence: we can reason that cata reductions
-- form a well-founded relation on encoded terms.
--
-- The encoding depth is finite for any term (structural recursion on t).
-- Each cata-beta consumes one In layer. The full proof would use
-- well-founded recursion, but the termination property follows from
-- the finiteness of encoded term structure.
------------------------------------------------------------------------
