------------------------------------------------------------------------
-- OCP-0009 · KNOT — ★★★ `flatK`'s ADEQUACY, ALL 30 `RTm` ROWS.
--
-- ⚠ GENERATED by `tools/gen-knot.py`.  Do not hand-edit.
--
--     agree : ielim KnotD i flatMeths ⌈t⌉ ⟶* ⌈ b2n (flat? t) ⌉
--
...
{-# OPTIONS --safe #-}
module DirectedHoTT.Examples.Knot.FlatAgree where
...
open import DirectedHoTT.Spec.Variance using ( 𝔹; true; false; flat? )
...
open import DirectedHoTT.Spec.Variance using ( stkC? )
...
------------------------------------------------------------------------
head-red : {Γ : Cx} (k : ℕ) {mth : RTm Γ} → sel k (flatMeths {Γ}) ⟶* mth →
           (i p : RTm Γ) {u : RTm Γ} →
           app (app (app mth i) p)
               (iihs KnotD flatMeths (isingle i) (ilookupD KnotD k) p) ⟶* u →
           ielim KnotD i flatMeths (icon k p) ⟶* u
head-red k sp i p h = ihead-red KnotD flatMeths k i p sp h
...
agree : {Γ Θ : Cx} (i : RTm Θ) (t : RTm Γ) →
        ielim KnotD i flatMeths (enTm {Γ} {Θ} t) ⟶* num (b2n (flat? t))
...
agree i (⌜Hom⌝ y0 y1 y2) =
  ...
  » ⟶*-castₗ (cong₂ (λ z w → stkCK (pair sTm (snd z)) (fst w))
                    ...
                    (wk-single {v = IHS} PAY))
      (⟶*-ielimᵗ (step (βfst _ _) done) » stkC-agree _ y0)
  where
    PAY : RTm _
    PAY = (pair (enTm y0) (pair (enTm y1) (pair (enTm y2) (pair (idrefl ⌜Nat⌝ sTm) unit))))
    IHS : RTm _
    IHS = iihs KnotD flatMeths (isingle i)
...
------------------------------------------------------------------------
flatK-agree : {Γ Θ : Cx} (i : RTm Θ) (t : RTm Γ) →
            flatK i (enTm {Γ} {Θ} t) ⟶* num (b2n (flat? t))
flatK-agree i t = agree i t
