...
if __name__ == "__main__":
    ...
    assert not _miss, (
        "emitted rows apply %r with no _WRAP_LEDGER entry.  Every wrapper the "
        "emitter can produce owes an adequacy (commutation) lemma or a written "
        "reason it does not." % _miss)
    _stale = sorted(set(_WRAP_LEDGER) - set(_seen))
    assert not _stale, ("_WRAP_LEDGER names %r, which no emitted row uses." % _stale)
    _awrap = sorted(_seen)
    print("  wrapper ledger: %d wrapper(s), %d still OWED"
