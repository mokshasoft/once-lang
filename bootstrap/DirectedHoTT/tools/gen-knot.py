...
def gen_subtie():
    ...
    X = {"cTm-cMu": "row-cMu", "cTm-elim": "row-elim",
         "cTm-ielim": "row-ielim", "cTm-cIMu": "row-cIMu", "cTm-var": "row-var"}
    gen = [nm for nm, _, f in rows
           if nm not in X and not any(x[0] == "rec" and x[1] != "sTm" for x in f)]
    imp = "  using ( " + "; ".join("row-" + n[4:] for n in gen) + " )"
    L = [SUBTIE_HDR % ("".join("        ; %s\n" % c for c in ctors), imp)]
    L.append("""
sub-agree : {Γ Δ Θ : Cx} {σ : Sub Γ Δ} {s : RTm Θ} →
    ...
    for nm, decl, f in rows:
        ...
        if nm in X:
            sub = [j for j in recs if f[j][1] == "sTm" and "lit" not in str(f[j][2])]
            ih = "".join(" (λ h' → sub-agree h' %s)" % an[j] for j in sub)
            L.append("sub-agree h %s = %s h %s%s"
                     % (_pat(c, nargs, an), X[nm], " ".join(an[j] for j in nargs), ih))
        else:
            ih = "".join(" (λ h' → sub-agree h' %s)" % an[j] for j in recs)
            L.append("sub-agree h %s = row-%s h %s%s"
                     % (_pat(c, nargs, an), nm[4:], " ".join(an[j] for j in nargs), ih))
    return "\n".join(L) + "\n"
    ...
    for k, nm, decl, f in rows:
        ...
        for r, j in enumerate(recs):
            ...
            ihs.append(
                ...
                "             subAtK sTm (num (len %s)) (num (len %s)) σ' (enTm %s)\n"
                "             ⟶* enTm (subTm %s %s)) →"
                % (_extS(d), _cx("Γ", d), _cx("Δ", d), an[j], _extS(d), an[j]))
        chains = []
        for j, fl in enumerate(f):
            if fl[0] == "rec":
                ...
                body = ("(⟶*-appˡ (⟶*-appˡ %s) »\n"
                        ...
                        "     ih%d %s)" % (_fstatR(r), _fstatR(j), nsucs, r, _repArgS(d)))
            elif fl[0] == "nat":
        ...
        L.append(
            ...
            "          Represents σ s →%s\n"
            "%s"
            "          subAtK sTm (num (len Γ)) (num (len Δ)) s (enTm {Γ} {Θ} %s)\n"
            "          ⟶* enTm {Δ} {Θ} (subTm σ %s)\n"
            "row-%s {Γ} {Δ} h %s%s =\n"
...
def gen_rentie():
    rows = [(nm, decl, f) for nm, decl, f in KNOT if nm.startswith("cTm-")]
    ctors = sorted({_ctor(d) for _, d, _ in rows})
    XROWS = {"cTm-cMu": "row-cMu", "cTm-elim": "row-elim",
             "cTm-ielim": "row-ielim", "cTm-cIMu": "row-cIMu", "cTm-var": "row-var"}
    gen = [nm for nm, _, f in rows
           if nm not in XROWS and not any(x[0] == "rec" and x[1] != "sTm" for x in f)]
    imp_g = "  using ( " + "; ".join("row-" + n[4:] for n in gen) + " )"
    L = [RENTIE_HDR % ("".join("        ; %s\n" % c for c in ctors), imp_g)]
    L.append("""
ren-agree : {Γ Δ Θ : Cx} {ρ : Ren Γ Δ} {r : RTm Θ} →
    ...
    for nm, decl, f in rows:
        ...
        recs = [j for j, x in enumerate(f) if x[0] == "rec"]
        if nm in XROWS:
            # the cross-sort/given rows: only the sTm fields get an IH
            sub = [j for j in recs if f[j][1] == "sTm" and "lit" not in str(f[j][2])]
            ih = "".join(" (λ h' → ren-agree h' %s)" % an[j] for j in sub)
            L.append("ren-agree h %s = %s h %s%s"
                     % (_pat(c, nargs, an), XROWS[nm],
                        " ".join(an[j] for j in nargs), ih))
        else:
            ih = "".join(" (λ h' → ren-agree h' %s)" % an[j] for j in recs)
            L.append("ren-agree h %s = row-%s h %s%s"
                     % (_pat(c, nargs, an), nm[4:],
                        " ".join(an[j] for j in nargs), ih))
    return "\n".join(L) + "\n"
...
if __name__ == "__main__":
    ...
    open(os.path.join(out, "RenAgreeTie.agda"), "w").write(gen_rentie())
    open(os.path.join(out, "SubAgreeRows.agda"), "w").write(gen_subagree())
    open(os.path.join(out, "SubAgreeTie.agda"), "w").write(gen_subtie())
