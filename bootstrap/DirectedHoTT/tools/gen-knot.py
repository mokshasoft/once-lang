...
#   header.  Silently emitting a SUBSET would be
#   `verification-that-covers-less-than-it-claims`.
#
# ⚠ Do not write the emitted count into prose — `_FLOOR` ratchets it and
#   the `NOT EMITTED` block computes it.  Two prose copies of it went
#   stale at "65 of 73" while the real figure moved to 71.
...
FIELD_SORT.update({
    ...
    # ★ STEP 3's two reduction heads, via `Knot/KAdapt`.
    #   `fields D ms C m p` — every argument is a knot term; `sel k ms`
    #   takes a ℕ then a `Tm`.
...
------------------------------------------------------------------------
-- OCP-0009 · EXAMPLES — ★★★ `ren-agree`, THE %(N)d SAME-SORT `%(RAW)s` ROWS.
--
--     renTmAtK %(S)s ⌈Γ⌉ ⌈Δ⌉ r ⌈t⌉  ⟶*  ⌈ %(REN)s ρ t ⌉    given  RepresentsR ρ r
--
-- ★★★ SORT-PARAMETRIC.  This emitter ran only at `%(RAWTM)s` until the `Ty`
...
{-# OPTIONS --safe #-}
module DirectedHoTT.Examples.Knot.RenAgree%(TAG)s where
open import Agda.Builtin.Nat using ( zero; suc ) renaming ( Nat to ℕ )
open import normalizer.Syntax.Types using ( _≡_; refl )
open import DirectedHoTT.Spec.Syntax
...
RENTIE_HDR = r"""--- GENERATED by tools/gen-knot.py — do not edit.
------------------------------------------------------------------------
-- OCP-0009 · EXAMPLES — ★★★ `%(TIE)s`, TIED: ALL %(NALL)d `%(RAW)s` ROWS.
--
--     %(TIE)s : RepresentsR ρ r → (t : %(RAW)s Γ) →
--                 renTmAtK %(S)s ⌈Γ⌉ ⌈Δ⌉ r ⌈t⌉ ⟶* ⌈ %(REN)s ρ t ⌉
--
...
{-# OPTIONS --safe #-}
module DirectedHoTT.Examples.Knot.RenAgree%(TAG)sTie where
open import Agda.Builtin.Nat using ( zero; suc ) renaming ( Nat to ℕ )
open import DirectedHoTT.Spec.Syntax
...
open import DirectedHoTT.Examples.Knot.RenAgree%(TAG)s
%(GEN)s%(EXTRA)s"""

SUBAGREE_HDR = r"""--- GENERATED by tools/gen-knot.py — do not edit.
------------------------------------------------------------------------
-- OCP-0009 · EXAMPLES — ★★★ `sub-agree`, THE %(N)d SAME-SORT `%(RAW)s` ROWS.
--
--     subAtK %(S)s ⌈Γ⌉ ⌈Δ⌉ σ ⌈t⌉  ⟶*  ⌈ %(SUB)s σ' t ⌉   given  Represents σ' σ
--
...
{-# OPTIONS --safe #-}
module DirectedHoTT.Examples.Knot.SubAgree%(TAG)sRows where
open import Agda.Builtin.Nat using ( zero; suc ) renaming ( Nat to ℕ )
open import normalizer.Syntax.Types using ( _≡_; refl )
open import DirectedHoTT.Spec.Syntax
...
def _extS(d):
    return "σ" if d == 0 else "(extS " * d + "σ" + ")" * d

def _repArgS(d):
    return "h" if d == 0 else "(extS-Represents _ " * d + "h" + ")" * d

SUBTIE_HDR = r"""--- GENERATED by tools/gen-knot.py — do not edit.
------------------------------------------------------------------------
...
{-# OPTIONS --safe #-}
module DirectedHoTT.Examples.Knot.SubAgree%(TAG)sTie where
open import Agda.Builtin.Nat using ( zero; suc ) renaming ( Nat to ℕ )
open import DirectedHoTT.Spec.Syntax
...
open import DirectedHoTT.Examples.Knot.SubApp using ( subAtK )
open import DirectedHoTT.Examples.Knot.SubAgree using ( Represents )
open import DirectedHoTT.Examples.Knot.SubAgree%(TAG)sRows
%(GEN)s%(EXTRA)s
"""

_STIE_EXTRA = {
    "Tm": ("open import DirectedHoTT.Examples.Knot.SubAgreeX\n"
           "  using ( row-cMu; row-elim; row-ielim; row-cIMu; row-var )\n"),
    "Ty": ("-- ★ THE LAYER BELOW — the SUBSTITUTION half's `Tm` tie.\n"
...
def gen_subtie(sort):
    K = _AGREE_SORT[sort]
    X = _XROWS[sort]
    rows = [(nm, decl, f) for nm, decl, f in KNOT if nm.startswith(K["pre"])]
    ctors = sorted({_ctor(d) for _, d, _ in rows})
    gen = [nm for nm, _, f in rows if nm not in X]
    imp = "  using ( " + "; ".join("row-" + n[4:] for n in gen) + " )\n"
    L = [SUBTIE_HDR % dict(TAG=K.get("tag", ""), GEN=imp, EXTRA=_STIE_EXTRA[sort],
                           CTORS="".join("        ; %s\n" % c for c in ctors))]
    L.append("""
%s : {Γ Δ Θ : Cx} {σ : Sub Γ Δ} {s : RTm Θ} →
            Represents σ s → (t : %s Γ) →
            subAtK %s (num (len Γ)) (num (len Δ)) s (%s {Γ} {Θ} t)
            ⟶* %s {Δ} {Θ} (%s σ t)""" % (_TIES[K["s"]], K["raw"], K["s"],
                                          K["en"], K["en"], _SUBS[K["s"]]))
    for nm, decl, f in rows:
        c = _ctor(decl)
        nargs = [j for j, x in enumerate(f) if x[0] in ("rec", "nat")]
        an = _names(c, nargs)
        ih = "".join(" (λ h' → %s h' %s)" % (_TIES[f[j][1]], an[j])
                     for j in range(len(f)) if _fclass(f[j]) == "ih")
        head = X[nm] if nm in X else "row-" + nm[4:]
        L.append("%s h %s = %s h %s%s"
                 % (_TIES[K["s"]], _pat(c, nargs, an), head,
                    " ".join(an[j] for j in nargs), ih))
    return "\n".join(L) + "\n"

def gen_subagree(sort, tag, lo, hi):
    K = _AGREE_SORT[sort]
    rows = []
    for k, (nm, decl, f) in enumerate(KNOT):
        if not nm.startswith(K["pre"]): continue
        if any(x[0] == "rec" and x[1] == "sVar" for x in f): continue
        if not K["cross"] and any(x[0] == "rec" and x[1] != K["s"] for x in f):
            continue
        rows.append((k, nm, decl, f))
    assert len(rows) == K["n"], (
        "expected %d same-sort %s rows, got %d" % (K["n"], K["raw"], len(rows)))
    rows = rows[lo:hi]
    L = [SUBAGREE_HDR % dict(TAG=tag, N=K["n"], RAW=K["raw"], S=K["s"],
                             SUB=_SUBS[K["s"]],
                             CTORS="".join("        ; %s\n" % _ctor(d)
                                           for _, _, d, _ in rows))]
    for k, nm, decl, f in rows:
        ...
        for j, fl in enumerate(f):
            if fl[0] == "rec":   ent.append("(%s %s)" % (_EN[fl[1]], an[j]))
        ...
        ihs, ihslot = [], {}
        for j in recs:
            if _fclass(f[j]) != "ih": continue
            d = _depth(f[j][2]); s = f[j][1]
            ihslot[j] = len(ihs)
            ihs.append(
                "          ({Θ' : Cx} {σ' : RTm Θ'} → Represents %s σ' →\n"
                "             subAtK %s (num (len %s)) (num (len %s)) σ' (%s %s)\n"
                "             ⟶* %s (%s %s %s)) →"
                % (_extS(d), s, _cx("Γ", d), _cx("Δ", d), _EN[s], an[j],
                   _EN[s], _SUBS[s], _extS(d), an[j]))
        chains = []
        for j, fl in enumerate(f):
            cls = _fclass(fl)
            if cls in ("ih", "closed"):
                ...
                last = ("ih%d %s" % (ihslot[j], _repArgS(d))) if cls == "ih" \
                       else ("%s _ _ _ %s" % (_CLOSED_IDS[fl[1]], an[j]))
                body = ("(⟶*-appˡ (⟶*-appˡ %s) »\n"
                        "     ⟶*-appˡ (⟶*-appˡ (⟶*-ielimᵗ %s)) »\n"
                        "     ⟶*-appˡ (⟶*-appˡ (⟶*-ielimⁱ (⟶*-pairʳ %s))) »\n"
                        "     %s)" % (_fstatR(r), _fstatR(j), nsucs, last))
            elif cls in ("nat", "pinned"):
                # ★ A `ℕ` FIELD IS A **CLOSED CODE**, so `kaPick (sk-clo …)`
                #   copies it at BOTH instantiations — one projection, same
                #   as `gen_renagree`.  `Lib/ISub`'s own note says it:
                #   "AND THE κ FIELD IS NOT ALWAYS A COPY.  ⚠ A closed code
                #    is … A TAG FORD is not."
                body = _fstatR(j)
            else:
                # ⚠⚠ A FORD SLOT IS **NOT** A BARE PROJECTION HERE.  At the
                #   RENAMING instantiation `renFordMap fi b p = p`, so
                #   `kaPick` is the identity and one projection suffices —
                #   which is what `gen_renagree` emits.  At the SUBSTITUTION
                #   instantiation
                #       fordMapK fi b p = jsub (⌜Id⌝ ⌜Nat⌝ (sortMap (var vz))
                #                               (w b)) (symN fi p) (idrefl ⌜Nat⌝ b)
                #   so the projected ford feeds `symN`'s path, and TWO
                #   `jsub-refl`s fire — `symN` is itself a `jsub`.
                # ★ This is the D′ asymmetry showing up in the EMITTER: the
                #   two instantiations do not share a ford chain, and a pure
                #   clone of `gen_renagree` is wrong here.
                body = ("(⟶*-jsubᵖ (⟶*-jsubᵖ %s) »\n"
                        "     ⟶*-jsubᵖ (step (jsub-refl _ _ _ _) done) »\n"
                        "     step (jsub-refl _ _ _ _) done)" % _fstatR(j))
            chains.append("  " + "⟶*-icon (" + "⟶*-pairʳ (" * j
                          + "⟶*-pairˡ\n    " + body + ")" * (j + 1))
        ihn = " ".join("ih%d" % r for r in range(len(ihs)))
        L.append(
            "row-%s : {Γ Δ Θ : Cx} {σ : Sub Γ Δ} {s : RTm Θ} →\n"
            "          Represents σ s →%s\n"
            "%s"
            "          subAtK %s (num (len Γ)) (num (len Δ)) s (%s {Γ} {Θ} %s)\n"
            "          ⟶* %s {Δ} {Θ} (%s σ %s)\n"
            "row-%s {Γ} {Δ} h %s%s =\n"
            "  sub-head-red %d ttsd ttsd refl\n"
            "               %s (num (len Γ)) (num (len Δ)) _ %s »\n"
            "%s\n"
            % (nm[4:],
               "".join(" (%s : %s) →" % (an[j], _argty(f[j])) for j in nargs),
               "".join(x + "\n" for x in ihs),
               K["s"], _EN[K["s"]], _pat(c, nargs, an),
               _EN[K["s"]], _SUBS[K["s"]], _pat(c, nargs, an),
               nm[4:], " ".join(an[j] for j in nargs), (" " + ihn) if ihn else "",
               k, K["s"], pay, " »\n".join(chains)))
    return "\n".join(L) + "\n"

# ★ WHICH TIE A FIELD'S IH CALLS — the one for the FIELD's sort, not the
#   row's.  This is the entire content of the layering: a `cTy-` row with
#   an `sTm` child calls `ren-agree`, and no `cTm-` row ever calls back
#   (`cTm-cIMu`'s `sTy` slot is PINNED — see `_fclass`).
_TIE  = {"sTm": "ren-agree", "sTy": "ren-agree-ty"}
_TIES = {"sTm": "sub-agree", "sTy": "sub-agree-ty"}

# ⚠ THE HAND-WRITTEN ROWS, PER SORT.  `Ty` has none: `cross=True` means
#   the emitter produced all 11, including the five cross-sort ones that
#   at `Tm` still live in `Knot/RenAgreeX`.
_XROWS = {
    "Tm": {"cTm-cMu": "row-cMu", "cTm-elim": "row-elim",
           "cTm-ielim": "row-ielim", "cTm-cIMu": "row-cIMu",
           "cTm-var": "row-var"},
    "Ty": {},
}

_TIE_WHERE = {
    "Tm": ("--   `Knot/RenAgree` supplies the 25 same-sort rows (generated),\n"
    ...
    "Ty": ("--   `Knot/RenAgreeTy` supplies ALL 11 rows — there is no hand-written\n"
...
_TIE_EXTRA = {
    "Tm": ("open import DirectedHoTT.Examples.Knot.RenAgreeX\n"
           "  using ( row-cMu; row-elim; row-ielim; row-cIMu; row-var )\n"),
    "Ty": ("-- ★ THE LAYER BELOW.\n"
...
def gen_rentie(sort):
    ...
    rows = [(nm, decl, f) for nm, decl, f in KNOT if nm.startswith(K["pre"])]
    ctors = sorted({_ctor(d) for _, d, _ in rows})
    gen = [nm for nm, _, f in rows if nm not in XROWS]
    imp_g = "  using ( " + "; ".join("row-" + n[4:] for n in gen) + " )\n"
    L = [RENTIE_HDR % dict(
            TIE=_TIE[K["s"]], NALL=len(rows), RAW=K["raw"], S=K["s"],
            REN=K["ren"], TAG=K.get("tag", ""),
            WHERE=_TIE_WHERE[sort], EXTRA=_TIE_EXTRA[sort], GEN=imp_g,
            CTORS="".join("        ; %s\n" % c for c in ctors))]
    L.append("""
%s : {Γ Δ Θ : Cx} {ρ : Ren Γ Δ} {r : RTm Θ} →
            RepresentsR ρ r → (t : %s Γ) →
            renTmAtK %s (num (len Γ)) (num (len Δ)) r (%s {Γ} {Θ} t)
            ⟶* %s {Δ} {Θ} (%s ρ t)""" % (_TIE[K["s"]], K["raw"], K["s"],
                                          K["en"], K["en"], K["ren"]))
    for nm, decl, f in rows:
        c = _ctor(decl)
        nargs = [j for j, x in enumerate(f) if x[0] in ("rec", "nat")]
        an = _names(c, nargs)
        # ★ ONE IH PER `ih`-CLASS FIELD, at the FIELD's sort.  Closed and
        #   pinned slots take none — the same rule the row emitter uses,
        #   read off the same `_fclass`, so the two cannot drift.
        ih = "".join(" (λ h' → %s h' %s)" % (_TIE[f[j][1]], an[j])
                     for j in range(len(f)) if _fclass(f[j]) == "ih")
        head = XROWS[nm] if nm in XROWS else "row-" + nm[4:]
        L.append("%s h %s = %s h %s%s"
                 % (_TIE[K["s"]], _pat(c, nargs, an), head,
                    " ".join(an[j] for j in nargs), ih))
    return "\n".join(L) + "\n"

# ★★★ THE FIVE NAMES A SORT CONTRIBUTES TO AN AGREEMENT ROW.
#
# ⚠ `n` IS THE COUNT OF SAME-SORT ROWS AND IS ASSERTED, not derived — a
#   row silently changing class (a field's sort edited) would otherwise
#   just shrink the generated set and still sweep green.
#   `Tm`: 25 of 30 (`var`, `elim`, `ielim`, `cMu`, `cIMu` are cross-sort).
#   `Ty`:  6 of 11 (`El`, `Hom`, `Id`, `Mu`, `IMu` are cross-sort) —
#          `Pi`/`Sg` plus the four leaves `base`/`U`/`Unit`/`Nat`.
_AGREE_SORT = {
    # ⚠ `cross` IS A CONSUMER SWITCH, NOT A CAPABILITY ONE.  The emitter
    #   handles cross-sort rows at BOTH sorts as of this change; `Tm` keeps
    #   `cross=False` only because `Knot/RenAgreeX` already carries its
    #   four by hand and they are green.  ⬜ Retiring those four (leaving
    #   `RenAgreeX` just the GIVEN `row-var`) is a follow-up: it deletes
    #   ~120 lines of hand proof and is a pure win, but it churns a green
    #   tree and is not what opening the `Ty` sort needs.
    "Tm": dict(pre="cTm-", s="sTm", en="enTm", ren="renTm", raw="RTm",
               n=25, cross=False, tag=""),
    "Ty": dict(pre="cTy-", s="sTy", en="enTy", ren="renTy", raw="RTy",
               n=11, cross=True, tag="Ty"),
}

# ★ per-sort: the encoder, and the argument's Agda type.  ⚠ THE CLOSED
#   SORTS TAKE NO CONTEXT — `Desc`, not `Desc Γ` — which is exactly why
#   they need no `RepresentsR` and their "IH" is an identity.
_EN  = {"sTm": "enTm", "sTy": "enTy", "sDesc": "enDesc", "sIDesc": "enIDesc"}
_RAWS = {"sTm": "RTm", "sTy": "RTy", "sDesc": "Desc", "sIDesc": "IDesc"}
# ★★★ A CLOSED SORT HAS NO VARIABLES, so renaming it is the IDENTITY and
#   the descent's last step is a `Knot/RenClosed` lemma instead of an IH.
_CLOSED_ID = {"sDesc": "ren-Desc-id", "sIDesc": "ren-IDesc-id"}
# ⚠ THE SUBSTITUTION TWIN, and it is a DIFFERENT LEMMA — `Knot/SubClosed`,
#   not `Knot/RenClosed`.  Membership is the same at both halves, so
#   `_fclass` may keep reading `_CLOSED_ID`; only the name emitted differs.
_CLOSED_IDS = {"sDesc": "sub-Desc-id", "sIDesc": "sub-IDesc-id"}
_SUBS = {"sTm": "subTm", "sTy": "subTy"}

_RENS = {"sTm": "renTm", "sTy": "renTy"}

def _argty(fl):
    "the Agda type of a row's bound argument"
    if fl[0] == "nat": return "ℕ"
    s = fl[1]
    if s in _CLOSED_ID: return _RAWS[s]                 # closed: no context
    if fl[2][0] == "lit":                               # pinned: an ABSOLUTE depth
        return _RAWS[s] + " " + ("ε" if fl[2][1] == 0
                                 else "(ε" + " ∙" * fl[2][1] + ")")
    return _RAWS[s] + " " + _cx("Γ", _depth(fl[2]))

def _fclass(fl):
    """★★★ FIVE SHAPES, NOT THREE.  `Knot/RenAgree`'s header named three
    (ford/ℕ, rec at depth 0, rec at depth k) because at `RTm` the emitter
    only ever saw same-sort rows.  Opening `Ty` adds the two that
    `Knot/RenAgreeX` had been carrying by hand:

        pinned  — `rec` at a `lit` depth.  `sPick (s-pinned _ _) … = q`
                  hands back the original, so it is ONE PROJECTION and
                  behaves exactly like a ford.  ⚠ `_depth` CANNOT tell
                  this from depth 0 — `lit(0)` and `D` both answer 0 —
                  so the test is on the depth's CONSTRUCTOR.
        closed  — `rec` at `sDesc`/`sIDesc`: an ordinary descent whose
                  IH is `ren-Desc-id`/`ren-IDesc-id`, taking no witness.
    """
    if fl[0] == "nat": return "nat"
    if fl[0] != "rec": return "ford"
    if fl[2][0] == "lit": return "pinned"
    if fl[1] in _CLOSED_ID: return "closed"
    # ★★★ `sVar` IS THE **GIVEN** CLASS AND TAKES NO IH.  `cTm-var`'s
    #   witness is `h x` — the `RepresentsR` RELATION itself — which is
    #   the whole reason step 3's statement had to be a relation rather
    #   than an encoded substitution (`PLAN-RENAMING.md` §16.1).  There is
    #   no `ren-agree` at `sVar` to call, and there should not be.
    if fl[1] not in _TIE: return "given"
    return "ih"

def gen_renagree(sort, tag, lo, hi):
    K = _AGREE_SORT[sort]
    rows = []
    for k, (nm, decl, f) in enumerate(KNOT):
        if not nm.startswith(K["pre"]): continue
        # ⚠ THE ONLY ROW STILL EXCLUDED IS THE **GIVEN** ONE.  `cTm-var`'s
        #   method is `renVarM`, hand-written and not an `isubMethod`, so
        #   `ren-head-red`'s premise cannot even be STATED for it; what
        #   discharges it is `h x`, the relation itself.  Every other
        #   cross-sort row is now emitted — see `_fclass`.
        if any(x[0] == "rec" and x[1] == "sVar" for x in f): continue
        if not K["cross"] and any(x[0] == "rec" and x[1] != K["s"] for x in f):
            continue
        rows.append((k, nm, decl, f))
    assert len(rows) == K["n"], (
        "expected %d same-sort %s rows, got %d" % (K["n"], K["raw"], len(rows)))
    rows = rows[lo:hi]
    L = [RENAGREE_HDR % dict(TAG=tag, N=K["n"], RAW=K["raw"], S=K["s"],
                             REN=K["ren"], RAWTM="RTm",
                             CTORS="".join("        ; %s\n" % _ctor(d)
                                           for _, _, d, _ in rows))]
    for k, nm, decl, f in rows:
        ...
        for j in recs:
            ...
            ihs.append(
                ...
                "             renTmAtK %s (num (len %s)) (num (len %s)) r' (%s %s)\n"
                "             ⟶* %s (%s %s %s)) →"
                % (_extR(d), s, _cx("Γ", d), _cx("Δ", d), _EN[s], an[j],
                   _EN[s], _RENS[s], _extR(d), an[j]))
        # ---- per-slot chains -------------------------------------------
        chains = []
        for j, fl in enumerate(f):
            cls = _fclass(fl)
            if cls in ("ih", "closed"):
                r = recs.index(j); d = _depth(fl[2])
                nsucs = "(⟶*-nsuc " * d + "(step (βsnd _ _) done)" + ")" * d
                last = ("ih%d %s" % (ihslot[j], _repArg(d))) if cls == "ih" \
                       else ("%s _ _ _ %s" % (_CLOSED_ID[fl[1]], an[j]))
                body = ("(⟶*-appˡ (⟶*-appˡ %s) »\n"
                        "     ⟶*-appˡ (⟶*-appˡ (⟶*-ielimᵗ %s)) »\n"
                        "     ⟶*-appˡ (⟶*-appˡ (⟶*-ielimⁱ (⟶*-pairʳ %s))) »\n"
                        "     %s)" % (_fstatR(r), _fstatR(j), nsucs, last))
            else:
                # ford, ℕ, and ★ PINNED — all one projection.
                body = _fstatR(j)
            wrap = "⟶*-icon (" + "⟶*-pairʳ (" * j + "⟶*-pairˡ\n    " + body + ")" * (j + 1)
            chains.append("  " + wrap)
        # ⚠ ONE NAME PER **IH**, NOT PER RECURSIVE FIELD.  A closed or
        #   pinned slot is a recursive field that takes NO hypothesis, so
        #   counting `recs` here binds a pattern variable the type never
        #   quantified — `CannotEliminateWithPattern` at the row's LHS.
        ihn = " ".join("ih%d" % r for r in range(len(ihs)))
        L.append(
            "row-%s : {Γ Δ Θ : Cx} {ρ : Ren Γ Δ} {r : RTm Θ} →\n"
            "          RepresentsR ρ r →%s\n"
            "%s"
            "          renTmAtK %s (num (len Γ)) (num (len Δ)) r (%s {Γ} {Θ} %s)\n"
            "          ⟶* %s {Δ} {Θ} (%s ρ %s)\n"
            "row-%s {Γ} {Δ} h %s%s =\n"
            "  ren-head-red %d ttsd ttsd refl\n"
            "               %s (num (len Γ)) (num (len Δ)) _ %s »\n"
            "%s\n"
            % (nm[4:], "".join(" (%s : %s) →" % (an[j], _argty(f[j]))
                       for j in nargs),
               "".join(x + "\n" for x in ihs),
               K["s"], K["en"], _pat(c, nargs, an),
               K["en"], K["ren"], _pat(c, nargs, an),
               nm[4:], " ".join(an[j] for j in nargs), (" " + ihn) if ihn else "",
               k, K["s"], pay, " »\n".join(chains)))
    return "\n".join(L) + "\n"
...
def _occ_row_body(nm, decl, f, an, mvals, ctor_pat):
    ...
    return (
      ...
      "     step (β _ _) done)) »\n"
      "  occSum-red true (num (lvl x)) (ilookupD KnotD %s)%s\n"
      "    %s"
...
# ======================================================================
# ★★★ `Knot/PwAgree` — `pwK`'s ADEQUACY, ALL 30 `RTm` ROWS.
#
#     pw? (⌜Π⌝ γ δ)     = true      -- row 20, `pwOne`
#     pw? (⌜Hom⌝ C a b) = pw? C     -- row 22, `pwHom`, the ONE recursion
#     pw? _             = false     -- the other 28, `pwZero`
#
# ★★ THE CHEAPEST ADEQUACY IN THE TREE, and the two reasons are exactly
#   the two that made `Knot/IhITyAgree` cost 58 minutes, both ABSENT:
#
#     the motive is CONSTANT `Nat`, so `iinst i t Nat` IS `Nat` and no
#       row carries a cast — there is nothing for a `natⁿ` lift to do;
#     every method body is CLOSED under its three binders (`nzero`,
#       `nsuc nzero`) or reads the INNERMOST one (`fst (var vz)`), so
#       the weakening tower is ZERO RUNGS everywhere.
#
#   ⇒ a default row is three βs and stop.  No fold (`occSum-red`), no
#   `⟶*-cast`, no naturality.  Contrast `gen_occagree`, whose bulk is
#   the `maxℕ`/`_∨_` reassociation its constant motive still needs
#   because `occ` FOLDS its IH tuple; `pw` reads one slot or none.
#
# ⚠ THE SELECTION IS FOUR LEMMAS, NOT ONE.  `Knot/Pw`'s tuple is
#   SEGMENTED — `methsFrom (cdTake 20 KnotD) pwZero pwMid` with
#   `pwMid = pair pwOne (pair pwZero (pair pwHom pwTail))` — because the
#   overrides sit at rows 20/22, in the middle.  So a row is reached by
#   its segment: the leading run, one of the three middle slots, or a
#   walk PAST the prefix into `pwTail`'s own run of 30.
# ======================================================================
_BOOLAGREE_HDR = """------------------------------------------------------------------------
-- OCP-0009 · KNOT — ★★★ `%(prog)s`'s ADEQUACY, ALL 30 `RTm` ROWS.
--
-- ⚠ GENERATED by `tools/gen-knot.py`.  Do not hand-edit.
--
--     agree : ielim KnotD i %(meths)s ⌈t⌉ ⟶* ⌈ b2n (%(fn)s t) ⌉
--
...
--   them (%(nseg)d segments here), because the overrides sit in the MIDDLE
...
%(ctors)s        )
open import DirectedHoTT.Spec.Typing using ( _⟶*_; step; done; β; βfst; βsnd )
open import DirectedHoTT.Spec.Variance using ( 𝔹; true; false; %(fn)s )
open import DirectedHoTT.Lib.BoolNum using ( b2n )
open import DirectedHoTT.Lib.NatNum using ( num )
open import DirectedHoTT.Lib.RedChain using ( _»_ )
...
------------------------------------------------------------------------
head-red : {Γ : Cx} (k : ℕ) {mth : RTm Γ} → sel k (%(meths)s {Γ}) ⟶* mth →
           (i p : RTm Γ) {u : RTm Γ} →
           app (app (app mth i) p)
               (iihs KnotD %(meths)s (isingle i) (ilookupD KnotD k) p) ⟶* u →
           ielim KnotD i %(meths)s (icon k p) ⟶* u
head-red k sp i p h = ihead-red KnotD %(meths)s k i p sp h
...
agree : {Γ Θ : Cx} (i : RTm Θ) (t : RTm Γ) →
        ielim KnotD i %(meths)s (enTm {Γ} {Θ} t) ⟶* num (b2n (%(fn)s t))
"""

_BOOLAGREE_TIE = """
------------------------------------------------------------------------
-- ★★★ AT THE LEDGER'S NAME.  `%(prog)s i t = ielim KnotD i %(meths)s t`,
...
------------------------------------------------------------------------
%(prog)s-agree : {Γ Θ : Cx} (i : RTm Θ) (t : RTm Γ) →
            %(prog)s i (enTm {Γ} {Θ} t) ⟶* num (b2n (%(fn)s t))
%(prog)s-agree i t = agree i t
"""


# ---- THE SEGMENT MAP -------------------------------------------------
#
# ⚠ EACH TUPLE IS READ OFF `Knot/Pw` AND `Knot/Stk` BY HAND, and the
#   lengths are ASSERTED to sum to 53 below.  A tuple and its selector
#   disagreeing is the failure mode that type-checks: the selection would
#   land on the wrong method and the row would simply not reduce.
#
#   ("run", n, D, m)  n consecutive rows all answered by `m`, walked by
#                     `methsFrom`/`cdTake n D`
#   ("one", m)        one explicit row, a `pair` slot
_BOOL_SEGS = {
 "pw": [("run",20,"(cdTake 20 KnotD)","pwZero"),
        ("one","pwOne"), ("one","pwZero"), ("one","pwHom"),
        ("run",30,"(cdTake 30 D23)","pwZero")],
 "stkA": [("run",19,"(cdTake 19 KnotD)","pwZero"),
        ("one","stkOne"),
        ("run",1,"(cdTake 1 D20)","pwZero"),
        ("one","stkOne"), ("one","pwHom"),
        ("run",3,"(cdTake 3 D23)","pwZero"),
        ("one","stkOne"),
        ("run",10,"(cdTake 10 D27)","pwZero"),
        ("run",4,"(cdTake 4 D37)","stkOne"),
        ("run",12,"(cdTake 12 D41)","pwZero")],
 "stkC": [("run",19,"(cdTake 19 KnotD)","pwZero"),
        ("one","stkOne"),
        ("run",1,"(cdTake 1 D20)","pwZero"),
        ("one","stkOne"), ("one","stkCHom"),
        ("run",3,"(cdTake 3 D23)","pwZero"),
        ("one","stkOne"),
        ("run",10,"(cdTake 10 D27)","pwZero"),
        ("one","pwZero"),
        ("run",3,"(cdTake 3 D38)","stkOne"),
        ("run",12,"(cdTake 12 D41)","pwZero")],
 "flat": [("run",19,"(cdTake 19 KnotD)","pwZero"),
        ("one","stkOne"),
        ("run",2,"(cdTake 2 D20)","pwZero"),
        ("one","flatHom"),
        ("run",30,"(cdTake 30 D23)","pwZero")],
}
...
def _seg_assemble(pieces, inner):
    """compose the walk.  ⚠ TWO DIFFERENT COMBINATORS: crossing a `run`
       is a `⟶*` STEP (`methsFrom-past`, composed with `»`), crossing an
       explicit slot is a CONGRUENCE (`sel-there`, which WRAPS its
       continuation).  Mixing them up is the one thing to get right."""
    e = inner
...
def _seg_sel(segs, k):
    """(the selection chain for row `k`, the METHOD it lands on)."""
    pos, pieces = 0, []
    for seg in segs:
        if seg[0] == "run":
            n, D, m = seg[1], seg[2], seg[3]
            if pos <= k < pos + n:
                j = k - pos
            ...
            m = seg[1]
            if pos == k:
                return (_seg_assemble(pieces, "sel-here _ _"), m)
            pieces.append(("there", k - pos - 1, None))
            pos += 1
    raise SystemExit("row %d is past the end of the tuple" % k)


# ---- THE FOUR PROGRAMS ------------------------------------------------
#
# ⚠ `stkC?` and `flat?` CALL ANOTHER FUNCTION at their `⌜Hom⌝` row rather
#   than reading their own IH (`Knot/Stk`'s header says why: a fold
#   cannot express it).  So their `⌜Hom⌝ method applies the CALLEE's
#   program to the payload's first field, and the row's proof is the
#   CALLEE's agreement — which is why the four form a chain
...
def gen_boolagree(key):
    ...
    # ⚠ THE SEGMENT MAP IS INDEXED BY ABSOLUTE ROW, so it is only valid
    #   while the `RTm` block sits where `Knot/Pw` and `Knot/Stk` assume.
    #   Those modules assert the same tags by `refl`; assert them here
    #   too, because a shifted block would still GENERATE, just wrong.
    ks = {nm: k for k, nm, _, _ in rows}
    assert ks["cTm-var"] == 11 and ks["cTm-cbase"] == 19 and \
           ks["cTm-cPi"] == 20 and ks["cTm-cSg"] == 21 and \
           ks["cTm-cHom"] == 22 and ks["cTm-cId"] == 26 and \
           ks["cTm-cNat"] == 37 and ks["cTm-cUnit"] == 40 and \
           rows[-1][0] == 40, "the RTm block moved — the segment maps are stale"
    ctors = sorted({_ctor(d) for _, _, d, _ in rows})
    hdr = dict(spec, nseg=len(segs), ctors="".join("        ; %s\n" % c
                                                   for c in ctors))
    L, seen = [_BOOLAGREE_HDR % hdr], {}
    for k, nm, decl, f in rows:
        c = _ctor(decl)
        nargs = [j for j, x in enumerate(f) if x[0] in ("rec", "nat")]
        an = _names(c, nargs)
        sel, mth = _seg_sel(segs, k)
        kind = _BOOL_KIND[mth]
        seen[kind] = seen.get(kind, 0) + 1
        body = ("  head-red tag%s\n    (%s) i _\n"
                "    (⟶*-appˡ (⟶*-appˡ (step (β _ _) done)) »\n"
                "     ⟶*-appˡ (step (β _ _) done) »\n"
                "     step (β _ _) done)" % (nm[1:], sel))
        if kind == "ih":
            # ★ THE ONE RECURSION.  `pwHom`'s body is `fst (var vz)` and
            #   `vz` IS the IH tuple, so the third β lands on `fst ihs`:
            #   one `βfst` off the tuple, one more to expose the first
            #   payload field, then the IH.  No `snd` peel spine — these
            #   functions read slot 0, and slot 0 needs no walk.
            body += ("\n  » step (βfst _ _) done"
    ...
    #   each method, and THAT is checkable against the meta-level
    #   definition in `Spec/Variance`.
...
def gen_szagree():
    ...
    for nm, decl, f in rows:
        ...
        tail = ("     step (β _ _) done)" if not counted else
                "     step (β _ _) done »\n"
                "     ⟶*-nsuc (szsSum-red (rowSort %s) ok %s\n     %s))" % (nm, nm, aih))
        L.append("agree i %s =\n"
...
WF_CTOR.update({
    ...
    # ★ STEP 3 — `fields` is all knot terms after the depth; `sel`'s
...
-- ★ `ι-elim`'s HEADS.  The rule is
--     elim D ms (con k p) ⟶ fields D ms (lookupD D k) (sel k ms) p
--   so it needs `fields`, `sel` AND `lookupD` — the last was already in
--   `_SUBST_CT` but this header never imported it, because no emitted
--   row had used it before.
-- ⚠ NOT optional: unlike `Knot/RedWf`'s header this block has no
...
_SUBST_CT = {"single": "singleK", "subTm": "subTmAtK",
             ...
             # ★★★ STEP 3 — `ι-elim`'s two heads, via `Knot/KAdapt`'s
...
_WRAP_LEDGER = {
    ...
    # ★★★ DISCHARGED 2026-09-06 — the `Ty` sort, and both are ONE LINE.
    ...
    # ★★★ STEP 3's TWO REDUCTION HEADS, 2026-09-17.  Both became
    #   ledger-tracked the moment `_SUBST_CT` mapped `fields`/`sel`, which
    #   is the both-ways gate working as designed: `ihsK`'s entry had
    ...
    # ★ THE FOUR NAMES THE SCANNER FLAGS IN `Knot/IExtAgree2`.  It
    #   over-approximates — it sees `subTmAtK` in a body and cannot tell
    #   a PROGRAM from a LEMMA ABOUT one.  `iinst-agree` below carries
    #   the same note and the same verdict.
    ...
    # ⬜→✅ `methTyK`'s NATURALITY CASCADE (`Knot/MethsTyAgree`).  Each is
    #   the SAME three-line lemma — a `…K` program is an `ielim` over a
    #   CLOSED method tuple, and `subTm τ tuple ≡ tuple` is the only part
    #   that does not compute.  ⚠ They are scanned because their bodies
    #   mention `ielim KnotD`, which is the point.
