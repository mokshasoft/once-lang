{-# OPTIONS --prop #-}
-- SPIKE 5: recurse nat-TI on the type SYNTAX, not the wf-derivation.
--
-- §4.2⁗ pinned the root cause: `ren⊨ r wA` is an opaque function on DERIVATIONS, so
-- `TI (ren⊨ r wA) δ` is unorderable.  But `renTy ⌜o⌝ A` on SYNTAX reduces structurally:
--     renTy r (Π̇ A B) = Π̇ (renTy r A) (renTy (extR r) B)
-- and ⊨-unique says the wf carries no information the syntax does not already determine.
-- So: state nat-TI with A : Ty Γ as an EXPLICIT argument and recurse on IT.
--
...
module poc.OCP0009.SpikeSyn where
...
postulate
  -- wkTI stays POSTULATED here: it is MI's edge, not part of the nat-TI question this spike asks.
...
------------------------------------------------------------------------
-- nat-TI recursing on the type SYNTAX A, with the wfs as ordinary arguments.
------------------------------------------------------------------------
force : ∀ {a}{A : Set a}{B : Set} → A → B → B
force _ b = b

envO   : ∀ {Γ Δ}{Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑[ o ] Θc) → CI Θc → CI Δc
-- A is EXPLICIT and FIRST — the recursion argument.
nat-TI : ∀ {Γ Δ}(A : Ty Γ){Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑[ o ] Θc)
         (wA : Δc ⊨ A)(wA' : Θc ⊨ renTy ⌜ o ⌝ A)(δ : CI Θc)
         → TI wA' δ ≡ TI wA (envO r δ)

envO done         ⟨⟩       = ⟨⟩
envO (keep r {A} wA) (δ ∷ᴱ x) = envO r δ ∷ᴱ coe (congÊl (nat-TI A r wA (ren⊨ r wA) δ)) x
envO (skip r wB) (δ ∷ᴱ x)  = envO r δ

nat-TI 𝔹 r wA wA' δ = {!!}
nat-TI ⊥̇ r wA wA' δ = {!!}
nat-TI (𝕀 t A B) r wA wA' δ = force recA (force recB {!!})
  where recA = nat-TI A r {!!} {!!} δ
        recB = nat-TI B r {!!} {!!} δ
nat-TI (Π̇ A B) r wA wA' δ = force recA (force recB {!!})
  where recA = nat-TI A r {!!} {!!} δ
        recB = λ x → nat-TI B (keep r {!!}) {!!} {!!} (δ ∷ᴱ x)
