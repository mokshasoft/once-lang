{-# OPTIONS --prop #-}
-- SPIKE 3: can the NATURALITY layer be stated so it does NOT defeat the structural checker?
--
-- SpikeWFNat.agda failed with root cause  TI (ren⊨ (wk⊑ Δc wC) wA₀) (ρ ∷ᴱ v)  — `ren⊨ r wA` is a
-- FUNCTION APPLICATION, not a subterm, so any TI-argument of that shape is unorderable.
--
-- IDEA UNDER TEST: state naturality with the RENAMED wf as an INDEPENDENT VARIABLE wA', related to
-- wA only through its type index (⊨-unique makes this equivalent to wA' = ren⊨ r wA).  Then every
-- TI argument is a VARIABLE and the opaque application disappears from the statement.
--
...
module poc.OCP0009.SpikeWFNat3 where
...
-- LOCAL OPE: `keep` carries the TARGET wf as a FIELD (wA'), so `ren⊨` never appears in an index.
-- Contrast NbEPDirDTTCh's:  keep r wA : (Δc ▷ wA) ⊑[ keep o ] (Θc ▷ ren⊨ r wA)
data _⊑'[_]_ : ∀ {Γ Δ} → Con Γ → OPE Γ Δ → Con Δ → Set where
  done' : ε ⊑'[ done ] ε
  keep' : ∀ {Γ Δ}{Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑'[ o ] Θc){A}(wA : Δc ⊨ A)
          (wA' : Θc ⊨ renTy ⌜ o ⌝ A) → (Δc ▷ wA) ⊑'[ keep o ] (Θc ▷ wA')
  skip' : ∀ {Γ Δ}{Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑'[ o ] Θc){B}(wB : Θc ⊨ B)
          → Δc ⊑'[ skip o ] (Θc ▷ wB)

envO   : ∀ {Γ Δ}{Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑'[ o ] Θc) → CI Θc → CI Δc
-- NOTE wA' is a plain variable of the RENAMED type — no `ren⊨` application anywhere in the type.
nat-TI : ∀ {Γ Δ}{Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑'[ o ] Θc){A}(wA : Δc ⊨ A)
         (wA' : Θc ⊨ renTy ⌜ o ⌝ A)(δ : CI Θc)
         → TI wA' δ ≡ TI wA (envO r δ)
-- the weakening OPE in the LOCAL family; its construction is not what this spike tests.
postulate wk⊑' : ∀ {Γ}(Δc : Con Γ){C}(wC : Δc ⊨ C) → Δc ⊑'[ skip idOPE ] (Δc ▷ wC)
...
------------------------------------------------------------------------
-- NATURALITY, stated with the renamed wf as a VARIABLE (wA'), not as (ren⊨ r wA).
------------------------------------------------------------------------
envO done'          ⟨⟩       = ⟨⟩
-- wA' is now a FIELD of keep', so this is a VARIABLE — no ren⊨ application.
envO (keep' r wA wA') (δ ∷ᴱ x) = envO r δ ∷ᴱ coe (congÊl (nat-TI r wA wA' δ)) x
envO (skip' r wB)     (δ ∷ᴱ x) = envO r δ

nat-TI r ⊨𝔹 wA'              δ = {!!}
nat-TI r ⊨⊥ wA'              δ = {!!}
nat-TI r (⊨𝕀 tb w𝔹 wA wB) wA' δ = force recA (force recB {!!})
  where recA = nat-TI r wA {!!} δ
        recB = nat-TI r wB {!!} δ
nat-TI r (⊨Π wA wB) wA'      δ = force recA {!!}
  where recA = nat-TI r wA {!!} δ
