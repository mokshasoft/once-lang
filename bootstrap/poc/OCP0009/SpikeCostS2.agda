{-# OPTIONS --safe #-}
module poc.OCP0009.SpikeCostS2 where

-- ABLATION S2: carrier=generic, step slot=Nat.
-- Same derivation text in all four; only Gamma5's shape differs.
...
REC2T : RTy (ε ∙ ∙ ∙ ∙ ∙ ∙)
REC2T = Π (El (var (vs (vs (vs (vs (vs vz)))))))
...
LStepT : RTy (ε ∙ ∙ ∙ ∙)
LStepT = Π (El (var (vs (vs (vs vz))))) (Π REC1T (Π REC2T (El (app (var (vs (vs (vs (vs (vs vz)))))) (var (vs (vs vz)))))))

Γ₅ : Ctx
Γ₅ = ((((◇ ▹ U) ▹ Π (El (var vz)) U) ▹ Π (El (var (vs vz))) Nat)
        ▹ Π (El (var (vs (vs vz)))) Nat) ▹ Nat
...
⊢lexZZrec1 =
  ⊢lam (ty-El (⊢var (there (there (there (there (there (there (there (there (here)))))))))))
