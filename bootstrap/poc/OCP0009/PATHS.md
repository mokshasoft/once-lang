...
| `NbEPDirStab` | dHoTT-12 | **CwF stability — type formers commute with substitution.** `×⁺-[] : (A ×⁺ B)[σ] ≡₁ (A[σ]) ×⁺ (B[σ])` — both sides have definitionally-equal `fam`/`act` (`×` has η, both act by `σ.homₛ`), so only the proof fields differ, closed by `funext` + `uip` (the `subst-∘` pattern). Scope: `+⁺`/`Σ⁺`/`Π⁺` need an `act`-comparison (`⊎` has no η → `act` only propositionally equal; two-implicit `funextᵢ₂` + a fibre transport). `×⁺-[]` fixes the technique. |
| `NbEPDirUniv` | dHoTT-13 | **A directed universe (`Ty⁺` reflected as a type).** A Tarski universe: `⊤⁺`/`⊥⁺` (base types), `Code` (small directed types `1`/`0`/`×`/`+`), `El` decodes a code to `Ty⁺ Γ` by recursion into `⊤⁺`/`⊥⁺`/`×⁺`/`+⁺` — **large elimination** (a code becomes a type); `𝒰` the universe (`fam _ = Code`, discrete, a genuine `Ty⁺`), `⌜_⌝` codes as terms (`Code → Tm Γ 𝒰`), `⌜⌝-tm` the reflection coherence. Dependent codes (`Σ`/`Π`) are the extension. |
