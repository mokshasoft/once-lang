...
| `NbEPDirDBSNU` | dHoTT-37 | **★ The UNIVERSE's type-normalization core — El-decoding is STRONGLY NORMALIZING (`snᵀ : (A : Ty) → SNᵀ A`), the "type-growth terminates" result, `--safe`/zero-axiom.** A real run at the universe. Verified against the kernel's `_⟶_`/`_⟶ᵀ_`: TERM reduction has NO `El` (codes `⌜Π⌝`/`⌜Σ⌝` reduce only by ξ), so term SN is STLC+products+inert-codes and dissolves to dHoTT-36 by **erasure** (`El c` erases to a fixed simple type by recursion on the finite code, making `El (⌜Π⌝ c d) ⟶ᵀ Π (El c)(El d)` erasure-INVARIANT — no induction-recursion). The universe's genuinely-new difficulty lives entirely in TYPE reduction, where `El` decodes and a type GROWS; this module proves that growth terminates. The crux: the universe is PREDICATIVE, so `El (⌜Π⌝ c d)` decodes to types over strictly SMALLER codes — a plain STRUCTURAL induction on the code (`snEl`) closes it, NO measure / WF-recursion / IR. Model faithful to `_⟶ᵀ_` (`El-⌜base⌝`/`⌜Π⌝`/`⌜Σ⌝` + `ξ-Π*`/`ξ-Σ*`), non-dependent for focus; the `atom` code models a neutral/variable of type `U` (what a substitution turns into a real code, growing the type). `NfTy`: normal types are El-decode-free (El survives only on a neutral code). Honest ceiling: the COUPLED fundamental theorem — reducibility of terms AT `El`-types, following the decoding — is the induction-recursion (Abel–Öhman–Vezzosi) that stands on this core; and typed λU + the erasure transfer for term SN are routine-but-unbuilt. |
