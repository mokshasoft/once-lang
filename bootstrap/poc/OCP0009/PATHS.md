...
| `NbEPDirDBSN` | dHoTT-35 | **(SN framework + concrete witnesses) — the normalization scaffolding `dec-conv` consumes, and SN exercised on real terms.** A self-contained intrinsically-typed STLC (`ι`/`_⇒_`) with the full funext-free substitution calculus (renaming, parallel substitution, the four fusion lemmas `ren-ren`/`sub-ren`/`ren-sub`/`sub-sub`, `sub-id`, `sub-comm`); β-reduction `_⟶_` with **`⟶-sub`** (reduction survives parallel substitution — the β case IS `sub-comm`); **`SN`** as accessibility of `_⟶_` with its preservation lemma `sn-red`. **Concrete SN witnesses** exercise the machinery: `sn-var` (variables are normal), `sn-lam-id`, and the β-redex **`sn-βredex`** (`(λx.x) y` is SN — it contracts only to `y`, its ξ-reducts ruled out by the normality of the subterms). Honest ceiling: the GENERAL theorem `Γ⊢A → SN t` is Girard–Tait reducibility — for open terms the KRIPKE form (`Red` closed under weakening, + reduction-reflection + SN both ways under renaming), a substantial standalone formalization even for STLC, and strictly harder once the universe grows types under substitution (induction-recursion à la Abel–Öhman–Vezzosi). `--safe`, zero axioms. |
