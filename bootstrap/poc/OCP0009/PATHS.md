...
| `NbEPDirStab` | dHoTT-12 | **CwF stability — type formers commute with substitution.** `×⁺-[] : (A ×⁺ B)[σ] ≡₁ (A[σ]) ×⁺ (B[σ])` — both sides have definitionally-equal `fam`/`act` (`×` has η, both act by `σ.homₛ`), so only the proof fields differ, closed by `funext` + `uip` (the `subst-∘` pattern). Scope: `+⁺`/`Σ⁺`/`Π⁺` need to compare non-η `act` fields, which hits a real **Agda elaboration limitation** (`MetaCannotDependOn` assigning a bound implicit-argument function to the implicit `act` field of a reconstructed `Ty⁺` — resists `record{}`/implicit-λ/copatterns/explicit-index alike). Blocked by tooling, not mathematics; `×⁺-[]` is the clean η representative. |
| `NbEPDirUniv` | dHoTT-13 | **A directed universe (`Ty⁺` reflected as a type).** A Tarski universe: `⊤⁺`/`⊥⁺` (base types), `Code` (small directed types `1`/`0`/`×`/`+`), `El` decodes a code to `Ty⁺ Γ` by recursion into `⊤⁺`/`⊥⁺`/`×⁺`/`+⁺` — **large elimination** (a code becomes a type); `𝒰` the universe (`fam _ = Code`, discrete, a genuine `Ty⁺`), `⌜_⌝` codes as terms (`Code → Tm Γ 𝒰`), `⌜⌝-tm` the reflection coherence. **Dependent codes**: a *small* universe closed under `Σ`/`Π` needs a syntactic De Bruijn calculus (research-grade); the tractable form is the LARGE `LCode : Set₁` carrying the types — `` `Σ A B `` decodes DIRECTLY to `Σ⁺ A B` (and `` `Π `` to `Π⁺` modulo its `Cat`/`funext` context). |
