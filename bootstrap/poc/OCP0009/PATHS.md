...
| `NbEPDirDBUniv` | dHoTT-30 | **(A3, standalone) a Tarski universe whose codes DECODE BY REDUCTION.** A universe `U` with codes (terms of type `U`) and a decoding `El`, so a type can be NAMED by a term — the ingredient the committed kernel's raw `El` lacks. Fresh self-contained mini type theory, touching nothing built. Coquand-style: `El` decodes by TYPE reduction — `El ⌜Unit⌝ ⟶ᵀ Unit`, **`El (⌜Π⌝ c d) ⟶ᵀ Π (El c)(El d)`** — and codes are genuinely DEPENDENT (`⌜Π⌝`'s codomain code lives under the decoded domain: `(Γ ▹ El c) ⊢ d ∷ U`). `Ty` = `U`/`El`/`Unit`/`Π`; typing `⊢⌜Unit⌝`/`⊢⌜Π⌝` inhabit `U`, terms land in `El c` via `⊢conv` across the decoding. Demos: `⌜Unit⌝ ∷ U` decodes to `Unit` and `tt ∷ El ⌜Unit⌝`; `⌜Π⌝ ⌜Unit⌝ ⌜Unit⌝ ∷ U` decodes to `Π Unit Unit` and `λx.x ∷ El (⌜Π⌝ ⌜Unit⌝ ⌜Unit⌝)` — a term inhabiting a NAMED type. `--safe`, zero axioms. |
| `NbEPDirDBFull` | dHoTT-31 | **The FEATURE-COMPLETE self-contained dependent type theory** — the capstone of the standalone-demo line: ONE mini type theory with Π + Σ + a universe *together*, showing them INTERACT. Codes decode by reduction to `Unit`/`Π`/`Σ`; dependent functions AND pairs; β/η for both; dependent typing throughout. **Interaction demo**: `⌜Σ⌝ ⌜Unit⌝ ⌜Unit⌝ ∷ U` decodes to `Sig Unit Unit`, and `pair tt tt` inhabits the NAMED type `El (⌜Σ⌝ ⌜Unit⌝ ⌜Unit⌝)` with a working projection + β-step — a coded dependent pair (universe and Σ composing); plus `λx.(x,x) ∷ Π Unit (Sig Unit Unit)`. `--safe`, zero axioms. |
| *(integration)* | dHoTT-32 | **INTEGRATED A1 — Σ terms folded into the COMMITTED kernel.** Not a new module: `pair`/`fst`/`snd` + Σ-β are now part of the committed `NbEPDirDBPi`/`NbEPDirDBType`, and the metatheory is re-proven to cover them — `NbEPDirDBConf` (parallel reduction + complete-development for `fst (pair a b) ⟶ a`, confluence re-proven), `NbEPDirDBInj` (Σ-injectivity mirroring `Π-inj`), `NbEPDirDBSubj` (`gen-pair`/`gen-fst`/`gen-snd` + `sr` covering the six new reduction cases). **Confluence AND subject reduction now genuinely cover pairs** — the kernel is a type-safe dependent theory with BOTH Π and Σ. Six modules extended, all `--safe`, zero-axiom, full chain re-verified. |
| *(integration)* | dHoTT-33 | **INTEGRATED A3 — a universe folded into the COMMITTED kernel.** A Tarski `U` with codes (`⌜base⌝`/`⌜Π⌝`/`⌜Σ⌝`) and El-decoding (`El (⌜Π⌝ c d) ⟶ᵀ Π (El c)(El d)`), now part of the committed `NbEPDirDBPi`/`NbEPDirDBType` (`U` on `RTy`, codes on `RTm`). The substantive part: `NbEPDirDBInj`'s TYPE confluence now has El-decode REDEXES, so the type complete-development `_⁺ᵀ` develops them (`El (⌜Π⌝ c d) ⁺ᵀ = Π (El c⁺)(El d⁺)`) and type confluence is re-proven. `NbEPDirDBSubj` needed **`conv-ctx`** (context conversion, derived from the substitution lemma) for the `ξ-⌜Π⌝ˡ` case (reducing the domain code changes the codomain's context). **Confluence AND subject reduction now cover the universe.** The committed kernel is a type-safe dependent theory with Π, Σ, AND a universe. All `--safe`, zero-axiom. |
| `NbEPDirDBDec` | dHoTT-34 | **(C1) DECIDABLE CONVERSION, modulo normalization — the decision engine powered by confluence.** The design's headline (§1: definitional equality decided by NbE), split into (1) normalization and (2) the engine. This delivers (2) in full — the part that USES the confluence result. **`conv-normal-≡`**: convertible normal terms are syntactically EQUAL, via Church–Rosser (common reduct; normal terms reduce only to themselves) — confluence doing the work. **`dec-conv`**: conversion is DECIDABLE given weak normalization + decidable NF equality (routine, threaded). `var≇lam`: a concrete non-conversion proven with NO inputs (two distinct normal terms are inconvertible — the reject half, self-standing); `redex≅reduct`: the accept side. Honest ceiling: normalization (1) is research-scale — β/Σ-β SN is classical, but the UNIVERSE makes it hard (`El c` decodes to Π/Σ, so types GROW under substitution → the reducibility predicate needs a Kripke logical relation). `--safe`, zero axioms. |
| `NbEPDirDTT` | dHoTT-43 | **★ A GENUINELY-DEPENDENT RAW calculus + its FULL SYNTACTIC METATHEORY, `--safe`/zero-axiom.** The honest genuinely-dependent raw route, after finding the code-based `⌜Π⌝` route was NON-dependent (dHoTT-41 #2: the minimal Tarski universe has no `El → U`, so a codomain code can't mention the domain value). Here dependency comes instead from a TYPE-LEVEL LARGE ELIMINATION `𝕀 t A B` (`if t then A else B`): TYPES `Ty` (`𝔹`/`⊥̇`/`𝕀`/`Π̇`) genuinely DEPEND ON TERMS, so `Π̇ 𝔹 (𝕀 (var vz) A B)` is a REAL dependent function type and `⊢app`'s result `subTy (single u) B` is genuinely NON-vacuous (`dep-example : Ty ε` = `(b : 𝔹) → if b then 𝔹 else (𝔹→𝔹)`, a concrete witness). Contents: terms `Tm` + term-dependent types `Ty`; de Bruijn renaming/substitution with the FULL fusion algebra (`ren-ren`/`sub-ren`/`ren-sub`/`sub-sub`/`sub-id` + type versions, all funext-free); the dependent typing relation `_⊢_∷_`; ★ **`ren-⊢`** (RENAMING preserves typing, via `renTy-wk`/`renTy-comm`) + ★ **`sub-⊢`** (SUBSTITUTION preserves typing — the `app` case rests on the genuine, non-vacuous **`subTy-comm`** for the dependent codomain). This is M1–M3b (the full syntactic metatheory of `NbEPDirDepModel`) LIFTED from the non-dependent code route to GENUINE dependency. Honest ceiling: M3c (the set interpretation `⟦_⟧` → consistency) REMAINS — its framework typechecks (`⟦_⟧C`/`⟦_⟧T`/`⟦_⟧M` structure, `tt`/`ff` filled), but `var`/`lam`/`app` need the semantic weakening lemma `⟦renTy vs A⟧(ρ,v)=⟦A⟧ρ` + substitution lemma `⟦subTy(single u)B⟧ρ=⟦B⟧(ρ,⟦u⟧ρ)` (mutual with the interpretation) PLUS derivation-coherence (raw+typing carries a separate `_⊨_` well-formedness derivation Agda can't see as unique) — the standard coherence-heavy DTT-soundness core, a dedicated formalization. Its SEMANTIC analogue (genuinely-dependent CONSISTENCY) is ALREADY proven intrinsically in `NbEPDirDepIR` (dHoTT-41) and `NbEPDirDHoTT3` (dHoTT-42), where intrinsic semantic types sidestep both the substitution lemma and the coherence. |
| `NbEPDirDTTSem` | dHoTT-43c | **★ The SEMANTIC counterpart of `NbEPDirDTT` — genuinely-dependent CONSISTENCY for the type-level-`if` mechanism, closed CLEANLY, `--safe`/zero-axiom.** The raw calculus's set model (M3c) is blocked on a real wall: its interpretation would need to be independent of the well-formedness derivation, but Curry-style typing is NON-UNIQUE (machine-checked: `lam (var vz)` inhabits BOTH `Π̇ 𝔹 𝔹` and `Π̇ ⊥̇ ⊥̇`), so raw M3c needs Church-style annotation + a heterogeneous TI-irr/MI-irr proof + semantic weakening/subst lemmas + threaded funext — a dedicated build. This module proves the SAME dependency mechanism consistent INTRINSICALLY: a TYPE-LEVEL large elimination `IfT A B t` (`if t then A else B`) where TYPES are SEMANTIC (`⟦Γ⟧ → Set`) and `Tm` is defined MUTUALLY with `⟦_⟧` by IR — so `IfT`'s condition is the meta term-interp (no re-typing) and the dependent eliminator `bif`'s result type `IfT A B c` is the semantic `If` (NO substitution lemma, NO coherence). ★ `consistency : Tm ε ⊥T → Empty`; ★ genuinely-dependent terms — `dep-true : Tm ε (IfT _ _ b1)` computes to the then-branch `Two` (`check-then : ⟦dep-true⟧ ⋆ ≡ 1₂`, `refl`) and `dep-false` at `b0` lands in the else-branch `Two → Two` (`check-else`, `refl`) — the TYPE genuinely depends on the boolean. The intrinsic route sidesteps exactly what blocks raw M3c and proves the dependency consistent. |
| `NbEPDirDHoTT` / `2` / `3` | dHoTT-42 (M1–M3) | **★ SOUNDNESS OF THE (directed) dHoTT KERNEL — a DIRECTED model, `--safe`/zero-axiom.** dHoTT's novelty is that the identity type is DIRECTED (`Hom a b := a ⟶* b`, covariant J, NO symmetry), so its model is NOT the standard set model — types carry directed hom-structure. **M1** (`NbEPDirDHoTT`): the meta DIRECTED Tarski universe (IR) — a code decodes to a SET `Êl` + a base one-step relation `Stp`; `Hom` = the refl-trans closure; `transp` (DIRECTED/covariant transport, needs the functorial action); ★ `no-sym : Hom ι̂ t1 t0 → Empty` (an arrow `t0 ⟶ t1` exists, `t1 ⟶ t0` does not). **M2** (`NbEPDirDHoTT2`): object syntax — semantic types are DIRECTED SETS, `Tm` defined MUTUALLY with `⟦_⟧` by IR (dependency + a `Hom`-type `HomT`/`hrfl`, NO syntactic substitution); ★ `consistency : Tm ε ⊥T → Empty`; object `no-sym`. **M3** (`NbEPDirDHoTT3`): the FULL kernel — folds in the object universe `U`/`El` (small Tarski universe of DirSets) with ★ **`El-⇒ : ElT (⌜⇒⌝ c d) ≡ ⇒T (ElT c) (ElT d)`** `refl` (El-conversion DEFINITIONAL), plus the directed-J laws ★ `transpD-rfl` (β-rule) and ★ `transpD-⊙` (functoriality). Dependency + universe + El-conversion + the DIRECTED identity + no-sym + covariant transport, unified and sound. Scope: universe codes non-dependent (dependency lives at the object type level `ΠT`); a syntactic directed-J term + the covariant (natural-transformation) function hom are refinements. |
| `NbEPDirDepIR` | dHoTT-41 (M2–M4) | **★ DEPENDENT CONSISTENCY — `Tm ε ⊥ → Empty` for a genuinely DEPENDENT type theory (dependent Π + empty type), `--safe`/zero-axiom.** The dependency project's payoff, via the standard model as an INDUCTION-RECURSION — sidestepping the semantic substitution lemma entirely. TYPES are SEMANTIC (a type over Γ is a code-family `⟦Γ⟧ → Û` into the meta dependent Tarski universe); the deep term syntax `Tm` is defined MUTUALLY with its interpretation `⟦_⟧` by IR, so **`app`'s result type is the semantic instantiation `b (γ , ⟦u⟧ γ)`** — dependency is meta-level function application, and SUBSTITUTION/CONVERSION are FREE. M2 = `Con`/`⟦_⟧C`/`Ty`; M3 = `Tm` (`vz`/`vs`/`lam`/`app`, NO intro for `⊥̂`)/`⟦_⟧`; M4 = ★ **`consistency : Tm ε (λ _ → ⊥̂) → Empty`** `= λ t → ⟦ t ⟧ tt`. Honest scope: models the DEPENDENCY feature (dependent Π + ⊥); the object-level universe-as-a-type is separate (dHoTT-39 level-stratification, dHoTT-40 El-conversion — here `U` is the meta `Û`). A single calculus with ALL THREE features is the RAW-syntax route (M1 + the syntactic substitution lemma), more faithful and harder. |
| `NbEPDirDep` | dHoTT-41 (M1) | **Dependent soundness — RAW-syntax route, milestone 1: the substitution foundation, `--safe`/zero-axiom.** Goal: `Con(the DEPENDENT kernel)` — the last of the universe's three hard features (level-stratification dHoTT-39, `El`-conversion dHoTT-40, now TERM-DEPENDENCY). NO-QIIT route: RAW well-scoped de Bruijn syntax + a typing RELATION + DEFINED substitution → a set/realizability model over a META dependent Tarski universe. **Key stratification** (why tractable): codes (terms of `U`) denote elements of the FIXED set `Û`, not type-dependent, so their substitution lemma is HOMOGENEOUS and feeds the `El`-type substitution lemma — no heterogeneous transport. Milestone 1: the meta dependent IR universe **`Û`/`Êl`** with the dependent code `π̂ : (a : Û) → (Êl a → Û) → Û` (`Êl (π̂ a b) = (x : Êl a) → Êl (b x)`); raw syntax (`U`/`El`, `⌜⊥⌝`/`⌜Π⌝`, `lam`/`app`); renaming + parallel substitution; the four composition operators; the four fusion lemmas + `sub-id` + **`sub-comm`**. Next milestones: typing relation; environment interpretation of contexts/types/terms; the semantic substitution lemma; consistency. |
| `NbEPDirConv` | dHoTT-40 | **★ Scaling the soundness bridge: a model that RESPECTS El-CONVERSION, `--safe`/zero-axiom.** dHoTT-39 sidestepped the universe's defining feature — `El` DECODES, so `El (⌜⇒⌝ c d) ≅ (El c) ⇒ (El d)`, and a sound model must send convertible types to EQUAL sets. The tool: a META-level Tarski universe by INDUCTION-RECURSION (`Û : Set`, `Êl : Û → Set`, `Êl ⊥̂ = Empty`, `Êl (⇒̂ a b) = Êl a → Êl b` — DEFINITIONAL), so decoding a function-code IS the function set on the nose. Hence CONVERSION-SOUNDNESS on the `El` rules is **`refl`** (`⟦El(⌜⇒⌝ c d)⟧ = Êl(⇒̂ ⟦c⟧ ⟦d⟧) = Êl⟦c⟧ → Êl⟦d⟧ = ⟦(El c) ⇒ (El d)⟧`). Object syntax has a separate `⇒`/`⊥`, Tarski codes, the conversion `_≅_`, and a `conv` term constructor; **`conv-sound : A ≅ B → ⟦A⟧ ≡ ⟦B⟧`** (El rules `refl`, closure `sym`/`trans`/`cong₂`); `⟦conv p t⟧` = `coe` along `conv-sound` (identity on El rules, `coe refl = id`); ★ **`consistency : Tm ∅ (El ⌜⊥⌝) → Empty`** — the empty type reached THROUGH a code and `El` still has no closed inhabitant. Honest scope: non-dependent (types over codes, not terms); the last piece for the real kernel is DEPENDENCY (`⌜Π⌝ c d`, `d` in an extended context), whose soundness needs the semantic SUBSTITUTION LEMMA `⟦d[u]⟧ = ⟦d⟧ ∘ (id,⟦u⟧)`. Complements dHoTT-39 (which had the level-stratification / large universe; here the IR universe is small/same-level for `refl` conversion). |
| `NbEPDirCon` | dHoTT-39 | **★ The SOUNDNESS BRIDGE — a set model of a type theory WITH a universe ⇒ CONSISTENCY, one level up, `--safe`/zero-axiom.** Turns a tower rung into an actual relative-consistency claim. SYNTAX (intrinsically well-typed, de Bruijn): a universe `U`, Tarski codes `Code` (`c⊥` empty, `c→` functions) with `El` decoding, `code`/`var`/`lam`/`app`; `El (c→ a b)` is directly the arrow (NO conversion — the tractable faithful core). MODEL `⟦_⟧` into `Set`, parametric in level `ℓ`: `⟦U⟧ = Set ℓ` (ONE LEVEL UP, `: Set (lsuc ℓ)`), `⟦El c⟧ = Lift ⟦c⟧`, `⟦c⊥⟧ = Empty`, `⟦c→ a b⟧ = ⟦a⟧ → ⟦b⟧`, contexts→products, terms→functions (SOUNDNESS by construction). ★ **`consistency : Tm ∅ (El c⊥) → Empty`** — a closed term of the empty type interprets to an element of the empty set, impossible. LEVEL-PARAMETRIC ⇒ the generic rung: `Interp ℓ` proves the calculus consistent using `Set ℓ`/`Set (lsuc ℓ)` for EVERY `ℓ`, the uniform step whose iteration is `Con(Once_n) ⊢ Once_{n+1}`. Gödel intact (the model needs `Set (lsuc ℓ)` strictly above the object universe's `Set ℓ`, so the theory never models itself). Honest scope: simply-typed-WITH-a-universe (types depend on codes, not terms); the full dependent kernel's soundness (term dependency, `El`-conversion) is the larger proof. |
| `NbEPDirStrict` | dHoTT-38 | **★ A STRICT directed CwF core — transport-free by DATA/LAW separation, universe-parametric (the consistency tower's generic rung), `--safe`/zero-axiom.** The principled redesign (answering "don't work around `Sub`'s proof fields — realign the representation"): separate DATA from LAWS. A `Sub` is LAW-FREE functor data (`ob`/`mor` only) so `_∘_` is plain function composition — `∘-idˡ`/`∘-idʳ`/`∘-assoc` all **`refl`**; a `Ty` is a LAW-FREE covariant family (`fam`/`act` only) so `_[_]` is plain precomposition — `[]-id`/`[]-∘` **`refl`**; hence **`Σ-stable`** `(Σ' A B)[σ] ≡ Σ' (A[σ]) (B[σ ↑ A])` and **`×-stable`** are **`refl`** — NO funext, uip, subst, or `Ty` extensionality wrapper. The lift `σ ↑ A` carries the source `Hom`-proof VERBATIM (`snd m`) because `act (A[σ]) f = act A (mor σ f)` definitionally. This is the SAME strictness-by-construction the syntactic kernel enjoys (dHoTT-20), now on the SEMANTIC side. UNIVERSE-PARAMETRIC (polymorphic in the fibre level `ℓ'`, `Ty Γ ℓ' : Set (ℓ ⊔ lsuc ℓ')`) — the shape of the CONSISTENCY TOWER's generic step (`Once_n` at `ℓ'`, `Once_{n+1}` one level up), so the core is the single level-parametric rung whose instances give `Once ⊂ Once⁺ ⊂ …`. Also the RELATIVE-CONSISTENCY LADDER rung (dHoTT-38b): `Tm` (natural sections) + `_[_]ᵗ`; a reflection universe **`U ℓ'`** over Γ (`: Ty Γ (ℓ ⊔ lsuc ℓ')`) reflecting the level-`ℓ'` types ONE LEVEL UP, with `code`/`El` and ★ **`El-code : El (code A) ≡ A`** (`refl` — the decode is DEFINITIONAL, soundness of the reflection), and ★ **`ladder : El (code (U ℓ')) ≡ U ℓ'`** (the universe of one level classified at the next; level-parametric ⇒ `Once ⊂ Once⁺ ⊂ …` from one construction; Gödel not bypassed — the reflection needs a level strictly above). Honest scope: strictness makes the covariant-stable formers (`Σ`,`×`) `refl`-stable; it does NOT resolve the directed `Π⁺` future-cone — its fibre indexes over the base category's morphisms, which genuinely change under a Cat→Cat substitution (Beck–Chevalley, `NbEPDirPiSub`), real mathematics that survives strictification for general base change (strict only along exact `σ`). The reflection universe is "large"/non-substitution-stable (`El`'s general `act` has one transport; `El-code` is `refl`); the STABLE small universe is Hofmann–Streicher (covariant, intricate — remaining). |
| `NbEPDirDBSNU` | dHoTT-37 | **★ The UNIVERSE's TYPE-LEVEL metatheory, CLOSED — type SN + normalization + DECIDABLE TYPE CONVERSION, `--safe`/zero-axiom.** A real run at the universe. Verified against the kernel's `_⟶_`/`_⟶ᵀ_`: TERM reduction has NO `El` (codes reduce only by ξ), so term SN is STLC+products+inert-codes IN SHAPE — but does NOT shortcut via erasure (the erased simple type is not conversion-stable: a neutral code `app (lam t) u : U` can reduce to a real code `⌜Π⌝ …`, so `El` of the redex vs its reduct erase to `base` vs `⇒` — the very obstruction that makes the universe hard). The genuinely-new difficulty lives entirely in TYPE reduction, where `El` decodes and a type GROWS; this module CLOSES that side. **`snᵀ : (A : Ty) → SNᵀ A`** — the decoding relation is STRONGLY NORMALIZING, by a plain STRUCTURAL induction on the code (`snEl`): predicativity means `El (⌜Π⌝ c d)` decodes over strictly SMALLER codes, so NO measure / WF-recursion / IR. **`nfᵀ`** — a DIRECT normal-form function (`_⟶ᵀ_` is orthogonal: inert codes ⇒ ≤1 redex per `El`, congruences never overlap) with `nfᵀ-red*`/`red-nfᵀ`; hence `A ≅ᵀ B iff nfᵀ A ≡ nfᵀ B` and ★ **`dec-≅ᵀ`** — type conversion is DECIDABLE (with `decCode`/`decTy`). Mirrors dHoTT-25/34 at the type level. Model faithful to `_⟶ᵀ_` (`El-⌜base⌝`/`⌜Π⌝`/`⌜Σ⌝` + `ξ-Π*`/`ξ-Σ*`), non-dependent for focus; the `atom` code models a neutral/variable of type `U`. Honest ceiling: TERM SN and the COUPLED fundamental theorem (reducibility of terms AT `El`-types, following the decoding) are the induction-recursion (Abel–Öhman–Vezzosi) that stands on this type-normalization — genuinely research-scale, no erasure shortcut. |
| `NbEPDirDBSNSig` | dHoTT-36 | **★ STRONG NORMALIZATION for the Π/Σ FRAGMENT (functions AND products) — `sn : Γ⊢A → SN t`, `--safe`, zero axioms.** Closes the NON-UNIVERSE half of the kernel's SN: without `U`/`El`, kernel types are term-free `base`/Π/Σ trees, i.e. simple types with functions and products — so this IS "SN for dependent Π/Σ without a universe." Extends the dHoTT-35 reducibility proof with `_×ₜ_`/`pair`/`fst`/`snd`, `βfst`/`βsnd`, the product candidate `Red (A ×ₜ B) t = Red A (fst t) × Red B (snd t)`, and **`red-pair`** — the pair INTRODUCTION lemma (dual to `abs`), by lexicographic induction on `SN a`/`SN b` with the βfst/βsnd-reducts cleared by `CR3` on the neutral projections. `CR1`/`CR2`/`CR3` and `fund` gain the product cases; the `CR3` arrow case is factored through **`app-nlam-inv`** (a neutral application reduces only in a subterm), which centralises the reduction-reflection. Honest ceiling: the UNIVERSE remains — `El c` decodes to Π/Σ, so types grow under substitution and the logical relation needs an induction-recursion (Abel–Öhman–Vezzosi). This + dHoTT-35 deliver everything below that frontier. |
| `NbEPDirDBSN` | dHoTT-35 | **★ STRONG NORMALIZATION, PROVEN for the simply-typed core — `sn : Γ⊢A → SN t`, `--safe`, zero axioms.** The input `NbEPDirDBDec.dec-conv` assumed, discharged for STLC by Girard–Tait reducibility. A self-contained intrinsically-typed STLC (`ι`/`_⇒_`) with the funext-free substitution calculus (fusion lemmas, `sub-comm`, `ren-comm`); β-reduction `_⟶_`/`_⟶*_` with **`⟶-sub`**, **`⟶-ren`**, and **`⟶-ren-inv`** (reduction REFLECTS through renaming); **`SN`** as accessibility, with `sn-red*`, `SN-appˡ-inv`, `sn-ren`/`SN-ren-inv` (SN both ways under renaming), and ★ **`sn-antisub`** (`SN (sub σ t) → SN t`). The SN CLOSURE THEOREMS: `sn-lam`, `sn-neutral-app`, ★ **`sn-β-exp`** (SN closed under β-EXPANSION, done clean at the accessibility level with NO candidates), and **`nf→SN`** (every β-normal form is SN). Then ★ REDUCIBILITY, Kripke form: **`Red`** by recursion on the type (arrow case over future renamings), **`Red-ren`** (closure under weakening), the candidate conditions **`CR1`/`CR2`/`CR3`** mutual on the type (crux: Girard-neutral = *not a λ*, so a β-redex is neutral and `CR3` applies to it — what makes `abs` go through), the abstraction lemma **`abs`**, the fundamental theorem **`fund`** (every term reducible under a reducible substitution, via `reds-ext` + the λ-case substitution identity `fund-lam-eq`), and hence **`sn`**. Honest ceiling: this is the SIMPLY-TYPED calculus (dependent Π/Σ without a universe reduces to it); the UNIVERSE is the hard extension — `El c` decodes to `Π`/`Σ`, types grow under substitution, so the logical relation needs an induction-recursion (Abel–Öhman–Vezzosi). The classical core is delivered, complete. |
