...
| `NbEPDirIR` | dHoTT-6 | **The real IR's `NatTr`/`Fuse`, directed** — the on-ramp from the POC's directed cata to `formal/Once/IR.agda`. Models the real IR's structured total recursion: `NatTr` as the eight-constructor **syntactic** nat-transformation (`ntId`/`ntK`/`ntFst`/`ntSnd`/`ntCase`/`ntInl`/`ntInr`/`ntPair`) — a *polynomial* transformation, so naturality is automatic (no coherence wall); `⟦_⟧nt` interprets it as a directed IR morphism; `fuseD τ alg = cata G (alg ∘ ⟦τ⟧)` realizes the real IR's law `Fuse alg τ = cata (alg ∘ τ)` definitionally (`fuse-spec`); `fuse-run` is the structural computation as a directed step — **Fuse is total because it IS a directed cata**; `idNat`/`fuse-idNat` show Fuse *generalizes* Cata (the identity `NatTr` reduces to `id`, over One-free `Poly` functors). Scope: `μ`/`Cata`/`Fuse` only — no `Ana`/`Out`/`Hylo`/`ν`. |
