...
--
-- Why this can work where the current design cannot: a caller supplies nat-TI's fuel from the
-- MEASURE OF ITS OWN ARGUMENT (szT wA₀ < suc (szT wA₀)), never from an external budget.  So no
-- caller ever has to prove an arithmetic inequality against a threaded n — which is what produced
-- the 67 bound towers, the + inversion trap, and the §2.5 metas.  And because TI is bound-free,
-- subTI/wkTI keep BOUND-FREE statements (the §4.2‴ blocker dissolves).
--
...
module poc.OCP0009.SpikeHybrid where
...
n≤m+n : (m n : Nat) → n ≤ m + n
n≤m+n zero    n = ≤-refl
n≤m+n (suc m) n = ≤-trans (n≤m+n m n) (help _) where
  help : ∀ n → n ≤ suc n
  help zero = z≤n
  help (suc k) = s≤s (help k)
-- the measure (types only — that is all nat-TI recurses on)
...
-- wkTI: BOUND-FREE statement (no fuel in the type).  Proven below by handing nat-TI fuel taken
-- from szT wA₀ — the caller never threads an external budget.
...
------------------------------------------------------------------------
-- HYBRID NATURALITY: fuel on nat-TI, but the CONCLUSION mentions only bound-free TI.
------------------------------------------------------------------------
force : ∀ {a}{A : Set a}{B : Set} → A → B → B
force _ b = b

envO   : ∀ {Γ Δ}{Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑[ o ] Θc) → CI Θc → CI Δc
-- fuel `n` is INTERNAL to nat-TI's recursion; the conclusion has no bound.
nat-TI : (n : Nat) → ∀ {Γ Δ}{Δc : Con Γ}{Θc : Con Δ}{o}(r : Δc ⊑[ o ] Θc){A}(wA : Δc ⊨ A)
         (wA' : Θc ⊨ renTy ⌜ o ⌝ A)(δ : CI Θc) → szT wA < n
         → TI wA' δ ≡ TI wA (envO r δ)

envO done         ⟨⟩       = ⟨⟩
-- caller supplies fuel from ITS OWN argument's measure — never an external budget.
envO (keep r wA) (δ ∷ᴱ x)  = envO r δ ∷ᴱ coe (congÊl (nat-TI (suc (szT wA)) r wA (ren⊨ r wA) δ ≤-refl)) x
envO (skip r wB) (δ ∷ᴱ x)  = envO r δ

nat-TI (suc n) r ⊨𝔹 wA'              δ b = {!!}
nat-TI (suc n) r ⊨⊥ wA'              δ b = {!!}
nat-TI (suc n) r (⊨𝕀 tb w𝔹 wA wB) wA' δ b = force recA (force recB {!!})
  where recA = nat-TI n r wA {!!} δ {!!}
        recB = nat-TI n r wB {!!} δ {!!}
nat-TI (suc n) r (⊨Π wA wB) wA'      δ b = force recA {!!}
  where recA = nat-TI n r wA {!!} δ {!!}
nat-TI zero r wA wA' δ ()

wkTI wC wA₀ wA ρ v = force recT {!!}
  where recT = nat-TI (suc (szT wA₀)) (wk⊑ _ wC) wA₀ {!!} (ρ ∷ᴱ v) ≤-refl
